<?php
/**
 * This template is used to display the goal with [walkthecounty_goal]
 */

$form        = new WalkTheCounty_Donate_Form( $form_id );
$goal_option = walkthecounty_get_meta( $form->ID, '_walkthecounty_goal_option', true );
...
if ( ( isset( $args['show_goal'] ) && ! filter_var( $args['show_goal'], FILTER_VALIDATE_BOOLEAN ) )
     || empty( $form->ID )
     || ( is_singular( 'walkthecounty_forms' ) && ! walkthecounty_is_setting_enabled( $goal_option ) )
     || ! walkthecounty_is_setting_enabled( $goal_option ) || 0 === $form->goal ) {
	return false;
}

$goal_format         = walkthecounty_get_form_goal_format( $form_id );
$price               = walkthecounty_get_meta( $form_id, '_walkthecounty_set_price', true );
$color               = walkthecounty_get_meta( $form_id, '_walkthecounty_goal_color', true );
 ...
 * @since 1.8.8
 */
$progress = apply_filters( 'walkthecounty_goal_amount_funded_percentage_output', $progress, $form_id, $form );
?>
	...
	<?php if ( ! empty( $show_text ) ) : ?>
        <div class="raised">
			...
			if ( 'amount' === $goal_format ) :
				...
				/**
				 * Filter the walkthecounty currency.
				 *
				 * @since 1.8.17
				 */
				$form_currency = apply_filters( 'walkthecounty_goal_form_currency', walkthecounty_get_currency( $form_id ), $form_id );
				 ...
				 * @since 1.8.17
				 */
				$income_format_args = apply_filters( 'walkthecounty_goal_income_format_args', array(
					'sanitize' => false,
				 ...
				 * @since 1.8.17
				 */
				$goal_format_args = apply_filters( 'walkthecounty_goal_amount_format_args', array(
					'sanitize' => false,
					'currency' => $form_currency,
					'decimal'  => false,
				), $form_id );
				...
				$income_amounts = apply_filters(
					'walkthecounty_goal_raised_amounts',
					array(
				...
				$income = walkthecounty_human_format_large_amount( walkthecounty_format_amount( $income, $income_format_args ), array( 'currency' => $form_currency ) );
				$goal   = walkthecounty_human_format_large_amount( walkthecounty_format_amount( $goal, $goal_format_args ), array( 'currency' => $form_currency ) );

				// Format the human readable donation amount.
				$formatted_income = walkthecounty_currency_filter(
					$income,
					array(
						'form_id' => $form_id,
					)
				...
				$formatted_goal = walkthecounty_currency_filter(
					$goal,
					array(
						'form_id' => $form_id,
					)
				...
				echo sprintf(
					...
					__( '<span class="income" data-amounts="%1$s">%2$s</span> of <span class="goal-text" data-amounts="%3$s">%4$s</span> raised', 'walkthecounty' ),
            ...
            elseif ( 'donation' === $goal_format ) :
				...
				echo sprintf( /* translators: 1: total number of donations completed 2: total number of donations set as goal */
					...
					walkthecounty_format_amount( $income, array( 'decimal' => false ) ),
					walkthecounty_format_amount( $goal, array( 'decimal' => false ) )
				);
