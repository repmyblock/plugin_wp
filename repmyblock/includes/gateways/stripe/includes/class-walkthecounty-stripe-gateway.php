...
/**
 * Check for class WalkTheCounty_Stripe_Gateway exists.
 *
 * @since 2.5.0
 */
if ( ! class_exists( 'WalkTheCounty_Stripe_Gateway' ) ) {

	class WalkTheCounty_Stripe_Gateway {
		 ...
		 * @access public
		 *
		 * @var WalkTheCounty_Stripe_Payment_Method
		 */
		public $payment_method;

		/**
		 * WalkTheCounty_Stripe_Gateway constructor.
		 *
		 * @since  2.5.0
		 * @access public
		 *
		...
		public function __construct() {
			...
			$this->secret_key = walkthecounty_stripe_get_secret_key();

			// Set API Version.
			$this->set_api_version();

			// Call Payment Intent Class to utilize.
			$this->payment_intent = new WalkTheCounty_Stripe_Payment_Intent();
			$this->payment_method = new WalkTheCounty_Stripe_Payment_Method();

			add_action( "walkthecounty_gateway_{$this->id}", array( $this, 'process_payment' ) );
			...
			try {
				...
				walkthecounty_record_gateway_error(
					...
					sprintf(
						...
						__( 'Unable to set Stripe API Version. Details: %s', 'walkthecounty' ),
						$e->getMessage()
					)
				);
		...
		public function send_back_to_checkout() {
			walkthecounty_send_back_to_checkout( '?payment-mode=' . $this->id );
		}
		...
		public function get_token_details( $id, $args = array() ) {
			...
			try {
				...
				$args = wp_parse_args( $args, walkthecounty_stripe_get_connected_account_options() );
				...
				// Something went wrong outside of Stripe.
				walkthecounty_record_gateway_error(
					__( 'Stripe Token Error', 'walkthecounty' ),
					sprintf(
						/* translators: %s Exception Message Body */
						__( 'Unable to retrieve token. Details: %s', 'walkthecounty' ),
						$e->getMessage()
					)
				);
				walkthecounty_set_error( 'stripe_error', __( 'An error occurred while processing the donation. Please try again.', 'walkthecounty' ) );
		...
		public function get_source_details( $id ) {
			...
			try {
				...
				return \Stripe\Source::retrieve( $id, walkthecounty_stripe_get_connected_account_options() );
				...
				// Something went wrong outside of Stripe.
				walkthecounty_record_gateway_error(
					__( 'Stripe Source Error', 'walkthecounty' ),
					sprintf(
						/* translators: %s Exception Message Body */
						__( 'Unable to retrieve source. Details: %s', 'walkthecounty' ),
						$e->getMessage()
					)
				);
				walkthecounty_set_error( 'stripe_error', __( 'An error occurred while processing the donation. Please try again.', 'walkthecounty' ) );
		...
		public function prepare_source( $args ) {
			...
			try {
				...
				return \Stripe\Source::create( $args, walkthecounty_stripe_get_connected_account_options() );
				...
				walkthecounty_record_gateway_error(
					...
					sprintf(
						/* translators: %s Exception Message Body */
						__( 'Unable to create source. Details: %s', 'walkthecounty' ),
						$e->getMessage()
					)
				);
				walkthecounty_set_error( 'stripe_error', __( 'An error occurred while processing the donation. Please try again.', 'walkthecounty' ) );
		...
		public function get_customer_card( $stripe_customer, $id ) {
			...
			if ( walkthecounty_stripe_is_checkout_enabled() && 'stripe' === $this->id ) {
			...
			if ( ! $card_exists ) {
				try {
					...
					walkthecounty_record_gateway_error(
						__( 'Stripe Card Error', 'walkthecounty' ),
						sprintf(
							/* translators: %s Exception Error Message */
							__( 'The Stripe Gateway returned an error while processing a donation. Details: %s', 'walkthecounty' ),
							$e->getMessage()
						)
					);
		...
		public function save_stripe_customer_id( $stripe_customer_id, $payment_id ) {
			...
			if ( class_exists( 'WalkTheCounty_DB_Donor_Meta' ) ) {

				$donor_id = walkthecounty_get_payment_donor_id( $payment_id );

				// Get the WalkTheCounty donor.
				$donor = new WalkTheCounty_Donor( $donor_id );

				// Update donor meta.
				$donor->update_meta( walkthecounty_stripe_get_customer_key(), $stripe_customer_id );

			} elseif ( is_user_logged_in() ) {

				// Support saving to legacy method of user method.
				update_user_meta( get_current_user_id(), walkthecounty_stripe_get_customer_key(), $stripe_customer_id );
		...
		public function log_error( $exception ) {
			...
			$log_message = __( 'The Stripe payment gateway returned an error while processing the donation.', 'walkthecounty' ) . '<br><br>';
			...
			if ( ! empty( $exception_message ) ) {
				...
				walkthecounty_set_error( 'stripe_request_error', $exception_message );
			} else {
		...
		public function verify_payment( $payment_id, $stripe_customer_id, $charge ) {
			...
			if ( $payment_id && ( ! empty( $stripe_customer_id ) || ! empty( $charge ) ) ) {
				 ...
				 * @since 2.5.0
				 */
				do_action( 'walkthecounty_stripe_verify_payment', $payment_id, $stripe_customer_id, $charge );
				...
				walkthecounty_set_error( 'payment_not_recorded', __( 'Your donation could not be recorded, please contact the site administrator.', 'walkthecounty' ) );
		...
		public function prepare_metadata( $donation_id = 0 ) {
			return walkthecounty_stripe_prepare_metadata( $donation_id );
		}
		...
		public function create_charge( $donation_id, $charge_args ) {
			...
			try {
				...
				$charge_args = apply_filters( "walkthecounty_{$this->id}_create_charge_args", $charge_args );
				...
				if ( ! defined( 'WALKTHECOUNTY_STRIPE_VERSION' ) ) {
					...
					$charge_args['application_fee_amount'] = walkthecounty_stripe_get_application_fee_amount( $charge_args['amount'] );
				}
				...
				if ( ! empty( $charge ) ) {
					walkthecounty_insert_payment_note( $donation_id, 'Stripe Charge ID: ' . $charge->id );
					walkthecounty_set_payment_transaction_id( $donation_id, $charge->id );
				}
				...
				walkthecounty_record_gateway_error(
					__( 'Stripe Charge Error', 'walkthecounty' ),
					sprintf(
						/* translators: %s Exception Error Message */
						__( 'Unable to create a successful charge. Details: %s', 'walkthecounty' ),
						$e
					)
				);
				walkthecounty_set_error( 'stripe_charge_error', __( 'Error processing donation with Stripe. Please try again.', 'walkthecounty' ) );
				return false;
			} // End try().
		}
		...
		public function create_3d_secure_source( $donation_id, $source_id ) {
			...
			$customer_id     = walkthecounty_get_payment_meta( $donation_id, '_walkthecounty_stripe_customer_id', true );
			$donation_amount = walkthecounty_donation_amount( $donation_id );
			...
			$source_args = array(
				...
				'redirect'             => array(
					'return_url' => add_query_arg(
						array(
							'walkthecounty-listener' => 'stripe_three_d_secure',
			...
			if ( ! empty( $source->id ) ) {
				walkthecounty_insert_payment_note( $donation_id, 'Stripe 3D Secure Source ID: ' . $source->id );
			}

			// Save 3D secure source id to donation.
			walkthecounty_update_payment_meta( $donation_id, '_walkthecounty_stripe_3dsecure_source_id', $source->id );
		...
		public function is_3d_secure_required( $source_object ) {
			...
			$is_3d_secure_enabled = walkthecounty_is_setting_enabled( walkthecounty_get_option( 'stripe_enable_three_d_secure_payments', '' ) );

			if ( $is_3d_secure_enabled ) {
				return apply_filters(
					'walkthecounty_stripe_3d_secure_required',
					(
