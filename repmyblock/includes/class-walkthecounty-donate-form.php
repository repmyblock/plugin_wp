 ...
 * @subpackage  Classes/WalkTheCounty_Donate_Form
 * @copyright   Copyright (c) 2015, WalkTheCountyWP
...
/**
 * WalkTheCounty_Donate_Form Class.
 *
 ...
 * @property $post_type
 */
class WalkTheCounty_Donate_Form {
	 ...
	 * Class Constructor
	 *
	 * Set up the WalkTheCounty Donate Form Class.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	/**
	 * WalkTheCountyn the donation form data, let's set the variables
	 *
	 * @since  1.5
	 * @access private
	 *
	...
	private function setup_donation_form( $donation_form ) {
		...
		if (
			...
			|| 'walkthecounty_forms' !== $donation_form->post_type
		) {
			return false;
		}


		WalkTheCounty_Forms_Query::update_meta_cache( array( $donation_form->ID ) );
	...
	public function __get( $key ) {
		...
		return new WP_Error( 'walkthecounty-form-invalid-property', sprintf( esc_html__( 'Can\'t get property %s.', 'walkthecounty' ), $key ) );
	...
	public function create( $data = array() ) {
		...
		$defaults = array(
			...
			'post_status' => 'draft',
			'post_title'  => __( 'New Donation Form', 'walkthecounty' ),
		);
		 ...
		 */
		do_action( 'walkthecounty_form_pre_create', $args );
		 ...
		 * @param array $args The post object arguments used for creation.
		 */
		do_action( 'walkthecounty_form_post_create', $id, $args );
	...
	public function get_price() {
		...
		if ( ! isset( $this->price ) ) {
			...
			$this->price = walkthecounty_maybe_sanitize_amount(
		 ...
		 */
		return apply_filters( 'walkthecounty_get_set_price', $this->price, $this->ID );
	}
	...
	public function get_minimum_price() {
		...
		if ( ! isset( $this->minimum_price ) ) {
			...
			$this->minimum_price = walkthecounty_get_meta( $this->ID, '_walkthecounty_custom_amount_range_minimum', true );

			// WalkTheCounty backward < 2.1
			if ( empty( $this->minimum_price ) ) {
				$this->minimum_price = walkthecounty_get_meta( $this->ID, '_walkthecounty_custom_amount_minimum', true );
			}

			if ( ! $this->is_custom_price_mode() ) {
				$this->minimum_price = walkthecounty_get_lowest_price_option( $this->ID );
			}

		}

		return apply_filters( 'walkthecounty_get_set_minimum_price', $this->minimum_price, $this->ID );
	}
	...
	public function get_maximum_price() {
		...
		if ( ! isset( $this->maximum_price ) ) {
			$this->maximum_price = walkthecounty_get_meta( $this->ID, '_walkthecounty_custom_amount_range_maximum', true, 999999.99 );

			if ( ! $this->is_custom_price_mode() ) {
				$this->maximum_price = walkthecounty_get_highest_price_option( $this->ID );
			}
		}

		return apply_filters( 'walkthecounty_get_set_maximum_price', $this->maximum_price, $this->ID );
	}
	...
	public function get_prices() {
		...
		if ( ! isset( $this->prices ) ) {
			...
			$this->prices = walkthecounty_get_meta( $this->ID, '_walkthecounty_donation_levels', true );
		 ...
		 */
		return apply_filters( 'walkthecounty_get_donation_levels', $this->prices, $this->ID );
	...
	public function get_level_info( $price_id ) {
		...
		foreach ( $levels as $level ) {
			if ( $price_id === $level['_walkthecounty_id']['level_id'] ) {
				$level_info = $level;
				break;
			}
		}
	...
	public function get_goal() {
		...
		if ( ! isset( $this->goal ) ) {
			...
			$goal_format = walkthecounty_get_form_goal_format( $this->ID );

			if( ! $this->has_goal() ) {
				$this->goal = '';

			} elseif ( 'donation' === $goal_format ) {
				$this->goal = walkthecounty_get_meta( $this->ID, '_walkthecounty_number_of_donation_goal', true );

			} elseif ( 'donors' === $goal_format ) {
				$this->goal = walkthecounty_get_meta( $this->ID, '_walkthecounty_number_of_donor_goal', true );

			} elseif( in_array( $goal_format, array( 'amount', 'percentage' ) ) ) {
				$this->goal = walkthecounty_get_meta( $this->ID, '_walkthecounty_set_goal', true );
			}
		}

		return apply_filters( 'walkthecounty_get_set_goal', $this->goal, $this->ID );
	...
	public function is_single_price_mode() {
		...
		$option = walkthecounty_get_meta( $this->ID, '_walkthecounty_price_option', true );
		$ret    = 0;
		 ...
		 */
		return (bool) apply_filters( 'walkthecounty_single_price_option_mode', $ret, $this->ID );
	...
	public function is_custom_price_mode() {
		...
		$option = walkthecounty_get_meta( $this->ID, '_walkthecounty_custom_amount', true );
		$ret    = 0;

		if ( walkthecounty_is_setting_enabled( $option ) ) {
			$ret = 1;
		}
		 ...
		 * @param int|string $ID  The ID of the donation form.
		 */
		return (bool) apply_filters( 'walkthecounty_custom_price_option_mode', $ret, $this->ID );
	...
	public function is_custom_price( $amount ) {
		$result = false;
		$amount = walkthecounty_maybe_sanitize_amount( $amount );
		...
		if ( $this->is_custom_price_mode() ) {
			...
			if ( 'set' === $this->get_type() ) {
				...
				$level_amounts = array_map( 'walkthecounty_maybe_sanitize_amount', wp_list_pluck( $this->get_prices(), '_walkthecounty_amount' ) );
		 ...
		 * @since 1.8.18
		 */
		return (bool) apply_filters( 'walkthecounty_is_custom_price', $result, $amount, $this->ID );
	}
	...
	public function has_variable_prices() {
		 ...
		 * @param int|string $ID  The ID of the donation form.
		 */
		return (bool) apply_filters( 'walkthecounty_has_variable_prices', $ret, $this->ID );
	...
	public function get_type() {
		...
		if ( ! isset( $this->type ) ) {
			...
			$this->type = walkthecounty_get_meta( $this->ID, '_walkthecounty_price_option', true );
		...
		return apply_filters( 'walkthecounty_get_form_type', $this->type, $this->ID );
	...
	public function get_form_classes( $args ) {
		...
		$float_labels_option = walkthecounty_is_float_labels_enabled( $args )
			? 'float-labels-enabled'
			: '';

		$form_classes_array = apply_filters( 'walkthecounty_form_classes', array(
			'walkthecounty-form',
			'walkthecounty-form-' . $this->ID,
			'walkthecounty-form-type-' . $this->get_type(),
			$float_labels_option,
		), $this->ID, $args );
	...
	public function get_form_wrap_classes( $args ) {
		$custom_class = array(
			'walkthecounty-form-wrap',
		);

		if ( $this->is_close_donation_form() ) {
			$custom_class[] = 'walkthecounty-form-closed';
		} else {
			$display_option = ( isset( $args['display_style'] ) && ! empty( $args['display_style'] ) )
				? $args['display_style']
				: walkthecounty_get_meta( $this->ID, '_walkthecounty_payment_display', true );

			$custom_class[] = "walkthecounty-display-{$display_option}";

			// If admin want to show only button for form then user inbuilt modal functionality.
			if ( 'button' === $display_option ) {
				$custom_class[] = 'walkthecounty-display-button-only';
			}
		}
		 ...
		 * @since 1.0
		 */
		$form_wrap_classes_array = (array) apply_filters( 'walkthecounty_form_wrap_classes', $custom_class, $this->ID, $args );
	...
	public function get_sales() {
		...
		if ( ! isset( $this->sales ) ) {
			...
			if ( '' == walkthecounty_get_meta( $this->ID, '_walkthecounty_form_sales', true ) ) {
				add_post_meta( $this->ID, '_walkthecounty_form_sales', 0 );
			} // End if

			$this->sales = walkthecounty_get_meta( $this->ID, '_walkthecounty_form_sales', true );
	...
	public function increase_sales( $quantity = 1 ) {
		...
		$sales       = walkthecounty_get_form_sales_stats( $this->ID );
		$quantity    = absint( $quantity );
		$total_sales = $sales + $quantity;

		if ( $this->update_meta( '_walkthecounty_form_sales', $total_sales ) ) {
	...
	public function get_earnings() {
		...
		if ( ! isset( $this->earnings ) ) {
			...
			if ( '' == walkthecounty_get_meta( $this->ID, '_walkthecounty_form_earnings', true ) ) {
				add_post_meta( $this->ID, '_walkthecounty_form_earnings', 0 );
			}

			$this->earnings = walkthecounty_get_meta( $this->ID, '_walkthecounty_form_earnings', true );
	...
	/**
	 * Increase the earnings by the walkthecountyn amount
	 *
	 * @since  1.0
	...
	public function increase_earnings( $amount = 0, $payment_id = 0 ) {
		...
		$earnings   = walkthecounty_get_form_earnings_stats( $this->ID );
		 ...
		 * @param int   $payment_id Donation ID.
		 */
		$amount = apply_filters( 'walkthecounty_increase_form_earnings_amount', $amount, $this->ID, $payment_id );

		$new_amount = $earnings + (float) $amount;

		if ( $this->update_meta( '_walkthecounty_form_earnings', $new_amount ) ) {
	...
	/**
	 * Decrease the earnings by the walkthecountyn amount
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function decrease_earnings( $amount, $payment_id = 0 ) {
		...
		if ( $earnings > 0 ) {
			 ...
			 * @param int   $payment_id Donation ID.
			 */
			$amount = apply_filters( 'walkthecounty_decrease_form_earnings_amount', $amount, $this->ID, $payment_id );
	...
	public function is_close_donation_form() {
		$is_closed = ( 'closed' === walkthecounty_get_meta( $this->ID, '_walkthecounty_form_status', true, 'open' ) );

		// If manual upgrade not completed, proceed with backward compatible code.
		if ( ! walkthecounty_has_upgrade_completed( 'v210_verify_form_status_upgrades' ) ) {
		...
		return apply_filters(
			'walkthecounty_is_close_donation_form',
			$is_closed,
			$this
		);
	...
	public function has_goal() {
		return walkthecounty_is_setting_enabled(
			WalkTheCounty()->form_meta->get_meta(
				$this->ID,
	...
	private function update_meta( $meta_key = '', $meta_value = '' ) {
		...
		if ( walkthecounty_update_meta( $this->ID, $meta_key, $meta_value ) ) {
			return true;
		}
	...
	private function bc_210_is_close_donation_form() {
		...
		$close_form      = false;
		$is_goal_enabled = walkthecounty_is_setting_enabled( walkthecounty_get_meta( $this->ID, '_walkthecounty_goal_option', true, 'disabled' ) );

		// Proceed, if the form goal is enabled.
		if ( $is_goal_enabled ) {

			$close_form_when_goal_achieved = walkthecounty_is_setting_enabled( walkthecounty_get_meta( $this->ID, '_walkthecounty_close_form_when_goal_achieved', true, 'disabled' ) );

			// Proceed, if close form when goal achieved option is enabled.
			if ( $close_form_when_goal_achieved ) {

				$form        = new WalkTheCounty_Donate_Form( $this->ID );
				$goal_format = walkthecounty_get_form_goal_format( $this->ID );
				...
				switch ( $goal_format ) {
					case 'donation':
						...
						break;
					case 'donors':
						$closed = $form->get_goal() <= walkthecounty_get_form_donor_count( $this->ID );
						break;
					default :
