 ...
 * @subpackage  Classes/WalkTheCounty_DB_Logs
...
/**
 * WalkTheCounty_DB_Logs Class
 *
 * This class is for interacting with the log database table.
 *
 * @since 2.0
 */
class WalkTheCounty_DB_Logs extends WalkTheCounty_DB {

	/**
	 * WalkTheCounty_DB_Logs constructor.
	 *
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'walkthecounty_logs';
		$this->primary_key = 'ID';
		$this->version     = '1.0';
	...
	public function get_log_by( $log_id = 0, $by = 'id' ) {
		...
		switch ( $by ) {
			...
			default:
				$log = apply_filters( "walkthecounty_get_log_by_{$by}", $log, $log_id );
		}
	...
	public function get_logs( $args = array() ) {
		...
		// Get log.
		if ( ! ( $logs = WalkTheCounty_Cache::get( 'walkthecounty_logs', true, $sql_query ) ) ) {
			$logs = $wpdb->get_results( $sql_query );
			WalkTheCounty_Cache::set( 'walkthecounty_logs', $logs, 3600, true, $sql_query );
		}
	...
	public function count( $args = array() ) {
		...
		if ( ! ( $count = WalkTheCounty_Cache::get( 'walkthecounty_logs_count', true, $sql_query ) ) ) {
			$count = $wpdb->get_var( $sql_query );
			WalkTheCounty_Cache::set( 'walkthecounty_logs_count', $count, 3600, true, $args );
		}
