<?php
/**
 * WalkTheCounty DB Meta
 *
 * @package     WalkTheCounty
 * @subpackage  Classes/WalkTheCounty_DB_Meta
...
class WalkTheCounty_DB_Meta extends WalkTheCounty_DB {
	/**
	 * Post type
	 *
	 * @since  2.0
	...
	/**
	 * WalkTheCounty_DB_Meta constructor.
	 *
	 * @since 2.0
	 */
	 ...
	 *
	 * For internal use only. Use WalkTheCounty_Payment->add_meta() for public usage.
	 *
	 * @access  private
	 * @since   2.0
	 *
	 ...
	 *
	 * For internal use only. Use WalkTheCounty_Payment->update_meta() for public usage.
	 *
				...
				foreach ( $joins as $join ) {
					if ( false !== strpos( $clause, $join ) ) {
						...
						foreach ( $clause as $key => $clause_part ) {
							...
							preg_match( '/' . $wpdb->prefix . 'walkthecounty_' . $this->meta_type . 'meta AS (.*) ON/', $clause_part, $alias_table_name );
	...
	private function delete_cache( $id, $meta_type = '' ) {
		...
		$group = array(
			'payment'  => 'walkthecounty-donations', // Backward compatibility
			'donation' => 'walkthecounty-donations',
			'donor'    => 'walkthecounty-donors',
			'customer' => 'walkthecounty-donors', // Backward compatibility for pre upgrade in 2.0
		);

		if ( array_key_exists( $meta_type, $group ) ) {
			WalkTheCounty_Cache::delete_group( $id, $group[ $meta_type ] );
