 ...
 * @subpackage  Classes/WalkTheCounty_DB_Comments
...
/**
 * WalkTheCounty_DB_Comments Class
 *
 * This class is for interacting with the comment database table.
 *
 * @since 2.3.0
 */
class WalkTheCounty_DB_Comments extends WalkTheCounty_DB {

	/**
	 * WalkTheCounty_DB_Comments constructor.
	 *
	...
	public function __construct() {
		...
		$wpdb->walkthecounty_comments = $this->table_name = $wpdb->prefix . 'walkthecounty_comments';
	...
	public function add( $data = array() ) {
		...
		// Strip out backslash for apostrophe and sanitize comment using walkthecounty_clean.
	...
	public function get_comment_by( $comment_id = 0, $by = 'id' ) {
		...
		switch ( $by ) {
			...
			default:
				$comment = apply_filters( "walkthecounty_get_comment_by_{$by}", $comment, $comment_id );
		}
	...
	public function get_sql( $args = array() ) {
		...
		if ( ! empty( $args['meta_query'] ) ) {
			...
			$meta_query        = $meta_query_object->get_sql( 'walkthecounty_comment', $this->table_name, 'comment_ID' );
