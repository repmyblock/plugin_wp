 ...
 * @subpackage  Classes/WalkTheCounty_DB_Donors
...
/**
 * WalkTheCounty_DB_Donors Class
 *
 * This class is for interacting with the donor database table.
 *
 * @since 1.0
 */
class WalkTheCounty_DB_Donors extends WalkTheCounty_DB {

	/**
	 * WalkTheCounty_DB_Donors constructor.
	 *
	 * Set up the WalkTheCounty DB Donor class.
	 *
	 * @since  1.0
	 * @access public
	 */
	public function __construct() {
		...
		$wpdb->donors      = $this->table_name = "{$wpdb->prefix}walkthecounty_donors";
		$this->primary_key = 'id';
		$this->version     = '1.0';
	...
	public function update( $row_id, $data = array(), $where = '' ) {
		...
		if ( $status ) {
			WalkTheCounty_Cache::delete_group( $row_id, 'walkthecounty-donors' );
		}
	...
	public function insert( $data, $type = '' ) {
		...
		if ( $donor_id ) {
			WalkTheCounty_Cache::delete_group( $donor_id, 'walkthecounty-donors' );
		}
	 ...
	 * the payment meta and logs. Use walkthecounty_donor_delete() instead.
	 *
	...
	public function delete( $_id_or_email = false ) {
		...
		if ( $donor->id > 0 ) {
			 ...
			 * @since 1.8.14
			 */
			WalkTheCounty()->donor_meta->delete_all_meta( $donor->id );
	...
	public function get_donor_by( $field = 'id', $value = 0 ) {
		...
		// Get donors.
		$donor = new WalkTheCounty_Donors_Query( $args );
		...
		isset( $donor->id ) && WalkTheCounty_Donors_Query::update_meta_cache( array( $donor->id  ) );
	...
	public function get_donors( $args = array() ) {
		...
		$donors = new WalkTheCounty_Donors_Query( $args );
	...
	public function count( $args = array() ) {
		...
		$cache_key = md5( 'walkthecounty_donors_count' . serialize( $args ) );
		$count     = WalkTheCounty_Cache::get_group( $cache_key, 'donors' );

		if ( is_null( $count ) ) {
			$donors = new WalkTheCounty_Donors_Query( $args );
			$count  = $donors->get_donors();

			WalkTheCounty_Cache::set_group( $cache_key, $count, 'donors', 3600 );
		}
	...
	private function bc_200_params() {
		...
		if (
			...
			$wpdb->query( $wpdb->prepare( "SHOW TABLES LIKE %s", "{$wpdb->prefix}walkthecounty_customers" ) )
		) {
			$wpdb->donors = $this->table_name = "{$wpdb->prefix}walkthecounty_customers";
		}
	}
