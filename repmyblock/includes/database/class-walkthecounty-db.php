 ...
 * @subpackage  Classes/WalkTheCounty_DB
...
/**
 * WalkTheCounty_DB Class
 *
 * This class is for interacting with the database table.
 *
 * @since 1.0
 */
abstract class WalkTheCounty_DB {
	 ...
	 * Class Constructor
	 *
	 * Set up the WalkTheCounty DB Class.
	 *
	 * @since  1.0
	 * @access public
	 */
	...
	public function insert( $data, $type = '' ) {
		 ...
		 * @param array $data
		 */
		do_action( "walkthecounty_pre_insert_{$type}", $data );
		...
		// Reorder $column_formats to match the order of columns walkthecountyn in $data
		 ...
		 * @param array $data
		 */
		do_action( "walkthecounty_post_insert_{$type}", $wpdb->insert_id, $data );
	...
	/**
	 * Check if the walkthecountyn table exists
	 *
	 * @since  1.3.2
	 * @access public
	 *
	...
	/**
	 * WalkTheCountyn a ID, make sure it's a positive number, greater than zero before inserting or adding.
	 *
	 * @access private
	 * @since  2.0
	 *
	...
	public function sanitize_id( $id ) {
		...
		// We were walkthecountyn a non positive number.
		if ( absint( $id ) !== $id ) {
			return false;
		}
	...
	public function handle_switch_blog( $new_blog_id, $prev_blog_id ) {
		...
		if ( $this instanceof WalkTheCounty_DB_Meta ) {
