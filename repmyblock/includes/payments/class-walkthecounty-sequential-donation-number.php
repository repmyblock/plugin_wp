...
class WalkTheCounty_Sequential_Donation_Number {
	/**
	 * Instance.
	 *
	 * @since  2.1.0
	 ...
	 * @var string
	 */
	private $donation_title_prefix = 'walkthecounty-donation-';
	 ...
	 * @since  2.1.0
	 * @access public
	 * @return WalkTheCounty_Sequential_Donation_Number
	 */
	...
	public function __save_donation_title( $donation_id, $post, $existing_donation_updated ) {
		// Bailout
		if (
			! walkthecounty_is_setting_enabled( walkthecounty_get_option( 'sequential-ordering_status', 'disabled' ) )
			|| $existing_donation_updated
			|| 'walkthecounty_payment' !== $post->post_type
		) {
			return;
		}
		...
		// Add prefix.
		if ( $prefix = walkthecounty_get_option( 'sequential-ordering_number_prefix', '' ) ) {
			$serial_code = $prefix . $serial_code;
		}

		// Add suffix.
		if ( $suffix = walkthecounty_get_option( 'sequential-ordering_number_suffix', '' ) ) {
		...
		$serial_code = apply_filters(
			'walkthecounty_set_sequential_donation_title',
			walkthecounty_time_do_tags( $serial_code ),
			$donation_id,
			$post,
			$existing_donation_updated,
			array(
		...
		try {
			...
			walkthecounty_update_option( 'sequential-ordering_number', ( $serial_number + 1 ) );
		} catch ( Exception $e ) {
			error_log( "WalkTheCountyWP caught exception: {$e->getMessage()}" );
		}
	}
	...
	public function __set_donation_number( $donation_id ) {
		...
		if (
			get_option( '_walkthecounty_reset_sequential_number' ) &&
			( $number = walkthecounty_get_option( 'sequential-ordering_number', 0 ) )
		) {
			if ( WalkTheCounty()->sequential_donation_db->get_id_auto_increment_val() <= $number ) {
				delete_option( '_walkthecounty_reset_sequential_number' );
			}
		...
		return apply_filters(
			'walkthecounty_set_sequential_donation_number',
			WalkTheCounty()->sequential_donation_db->insert( $table_data ),
			$table_data
		);
	}
	...
	public function __remove_serial_number( $donation_id ) {
		return WalkTheCounty()->sequential_donation_db->delete( $this->get_serial_number( $donation_id ) );
	}
	...
	public function set_number_padding( $serial_number ) {
		if ( $number_padding = walkthecounty_get_option( 'sequential-ordering_number_padding', 0 ) ) {
	 ...
	 * @access public
	 *
	 * @param int|WalkTheCounty_Payment|WP_Post $donation
	 * @param array            $args
	 *
	 * @return string
	 */
	public function get_serial_code( $donation, $args = array() ) {
		...
		if( ! is_numeric( $donation ) ) {
			if( $donation instanceof WalkTheCounty_Payment ) {
		 ...
		 */
		return apply_filters( 'walkthecounty_get_donation_serial_code', $serial_code, $donation, $args, $donation_number );
	}
	...
	public function get_serial_number( $donation_id_or_serial_code ) {
		if ( is_numeric( $donation_id_or_serial_code ) ) {
			return WalkTheCounty()->sequential_donation_db->get_column_by( 'id', 'payment_id', $donation_id_or_serial_code );
		}
	...
	public function get_donation_id( $donation_number_or_serial_code ) {
		...
		if ( is_numeric( $donation_number_or_serial_code ) ) {
			return WalkTheCounty()->sequential_donation_db->get_column_by(
				'payment_id',
				'id',
	...
	public function get_max_number() {
		global $wpdb;
		$table_name = WalkTheCounty()->sequential_donation_db->table_name;
