...
/**
 * WalkTheCounty_Payments_Query Class
 *
 ...
 * @since 1.0
 */
class WalkTheCounty_Payments_Query extends WalkTheCounty_Stats {
	...
	/**
	 * The args to pass to the walkthecounty_get_payments() query
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			...
			'donor'           => null,
			'status'          => walkthecounty_get_payment_status_keys(),
			'meta_key'        => null,
			'year'            => null,
			'month'           => null,
			'day'             => null,
	...
	private function set_filters() {
		 ...
		 * @since 1.0
		 *
		 * @param WalkTheCounty_Payments_Query $this Payments query object.
		 */
		do_action( 'walkthecounty_pre_get_payments', $this );
	}
	...
	private function unset_filters() {
		 ...
		 * @param WalkTheCounty_Payments_Query $this Payments query object.
		 */
		do_action( 'walkthecounty_post_get_payments', $this );
	}
	...
	public function get_payments() {
		...
		$results        = array();
		$this->payments = array();
		$cache_key      = WalkTheCounty_Cache::get_key( 'walkthecounty_payment_query', $this->args, false );
		$this->payments = WalkTheCounty_Cache::get_db_query( $cache_key );
		...
		$custom_output = array(
			'payments',
			'walkthecounty_payments',
		);

		if ( $query->have_posts() ) {

			// Update meta cache only if query is not for all donations.
			// @see https://github.com/impress-org/walkthecounty/issues/4104
			if (
			...
			if ( ! in_array( $this->args['output'], $custom_output ) ) {
				...
				while ( $query->have_posts() ) {
					...
					$this->payments[] = apply_filters( 'walkthecounty_payment', $payment, $payment_id, $this );
				}
		...
		WalkTheCounty_Cache::set_db_query( $cache_key, $results );
	...
	public function get_payment_by_group() {
			...
			if ( $this->args['count'] ) {
				...
				switch ( $this->args['group_by'] ) {
					case 'post_status':
						...
						/* @var WalkTheCounty_Payment $donation */
						foreach ( walkthecounty_get_payment_status_keys() as $status ) {
				...
				/* @var $donation WalkTheCounty_Payment */
		 ...
		 * @since 1.8.17
		 */
		return apply_filters( 'walkthecounty_get_payment_by_group', $result, $this );
	}
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'amount':
				...
				$this->__set( 'meta_key', '_walkthecounty_payment_total' );
				break;
			...
			case 'donation_form':
				...
				$this->__set( 'meta_key', '_walkthecounty_payment_form_title' );
				break;
	...
	public function custom_orderby( $order, $query ) {
		...
		if ( ! empty( $query->query['post_type'] ) ) {
			...
			if ( ! in_array( 'walkthecounty_payment', $post_types ) || ! isset( $query->query['orderby'] ) || is_array( $query->query['orderby'] ) ) {
				return $order;
			}
	...
	public function user() {
		...
		if ( is_numeric( $this->args['user'] ) ) {
			...
			$donor_id = WalkTheCounty()->donors->get_column_by( 'id','user_id', $this->args['user'] );
	...
	public function donor() {
		...
		$donor_meta_type = WalkTheCounty()->donor_meta->meta_type;

		$this->__set( 'meta_query', array(
			'key'   => "_walkthecounty_payment_{$donor_meta_type}_id",
	...
	public function search() {
		...
		if ( ! empty( $this->args['search_in_notes'] ) ) {
			...
			$notes = walkthecounty_get_payment_notes( 0, $search );
			...
			$key         = $is_email ? '_walkthecounty_payment_donor_email' : '_walkthecounty_payment_purchase_key';
			$search_meta = array(
				'key'     => $key,
				'value'   => $search,
				'compare' => 'LIKE',
			...
			if ( is_object( $post ) && $post->post_type == 'walkthecounty_payment' ) {
			...
			$search = str_replace( '#', '', $search );
			$this->__set( 'walkthecounty_forms', $search );
			$this->__unset( 's' );
			...
			if ( is_array( $search_parts ) && 2 === count( $search_parts ) ) {
				$search_meta = array(
					'relation' => 'AND',
					array(
						'key'     => '_walkthecounty_donor_billing_first_name',
						'value'   => $search_parts[0],
						'compare' => '=',
					),
					array(
						'key'     => '_walkthecounty_donor_billing_last_name',
	 ...
	 * @return void
	 */
	public function walkthecounty_forms() {

		if ( empty( $this->args['walkthecounty_forms'] ) ) {
			return;
		}

		$compare = '=';

		if ( is_array( $this->args['walkthecounty_forms'] ) ) {
			$compare = 'IN';
		}
		...
		$this->__set(
			...
			array(
				...
				'value'   => $this->args['walkthecounty_forms'],
				'compare' => $compare,
			)
		);

		$this->__unset( 'walkthecounty_forms' );
	...
	private function get_sql() {
		...
		$where = "WHERE {$wpdb->posts}.post_type = 'walkthecounty_payment'";
		...
		if ( ! empty( $this->args['meta_query'] ) ) {
			...
			$where          = WalkTheCounty()->payment_meta->__rename_meta_table_name( $where, 'posts_where' );
		}
	...
	public static function update_meta_cache( $donation_ids ) {
		...
		update_meta_cache( WalkTheCounty()->payment_meta->get_meta_type(), $donation_ids );
	}
}
