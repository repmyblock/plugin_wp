 ...
 * @since 1.0
 */
function walkthecounty_is_test_mode() {

	$ret = walkthecounty_is_setting_enabled( walkthecounty_get_option( 'test_mode' ) );

	return (bool) apply_filters( 'walkthecounty_is_test_mode', $ret );
 ...
 * @since 1.0
 */
function walkthecounty_get_current_page_url() {
	 ...
	 * @since 1.0
	 */
	return apply_filters( 'walkthecounty_get_current_page_url', $current_uri );
 ...
 * @since 1.0
 */
function walkthecounty_is_cc_verify_enabled() {
	 ...
	 * @since 1.0
	 */
	return (bool) apply_filters( 'walkthecounty_is_cc_verify_enabled', $ret );
}
 ...
 * @since 1.0
 */
function walkthecounty_get_timezone_id() {
...
function walkthecounty_get_ip() {
	 ...
	 * @since 1.0
	 */
	$ip = apply_filters( 'walkthecounty_get_ip', $ip );

	// Filter empty values.
	if ( false !== strpos( $ip, ',' ) ) {
		$ip = walkthecounty_clean( explode( ',', $ip ) );
		$ip = array_filter( $ip );
		$ip = implode( ',', $ip );
	} else {
		$ip = walkthecounty_clean( $ip );
	}
 ...
 * @since 1.0
 *
 * @uses  WalkTheCounty()->session->set()
 */
function walkthecounty_set_purchase_session( $purchase_data = array() ) {
	WalkTheCounty()->session->set( 'walkthecounty_purchase', $purchase_data );
	WalkTheCounty()->session->set( 'walkthecounty_email', $purchase_data['user_email'] );
}
 ...
 * @uses  WalkTheCounty()->session->get()
 * @since 1.0
 */
function walkthecounty_get_purchase_session() {
	return WalkTheCounty()->session->get( 'walkthecounty_purchase' );
}
 ...
 * @since 1.8.17
 */
function walkthecounty_get_receipt_session() {
	return WalkTheCounty()->session->get( 'receipt_access' );
}
 ...
 * @since 1.8.17
 */
function walkthecounty_get_history_session() {
	return (bool) WalkTheCounty()->session->get( 'history_access' );
}
 ...
 * @since 1.8.14
 */
function walkthecounty_payment_gateway_item_title( $payment_data ) {

	$form_id   = intval( $payment_data['post_data']['walkthecounty-form-id'] );
	$item_name = isset( $payment_data['post_data']['walkthecounty-form-title'] ) ? $payment_data['post_data']['walkthecounty-form-title'] : '';
	$price_id  = isset( $payment_data['post_data']['walkthecounty-price-id'] ) ? $payment_data['post_data']['walkthecounty-price-id'] : '';

	// Verify has variable prices.
	if ( walkthecounty_has_variable_prices( $form_id ) && ! empty( $price_id ) ) {

		$item_price_level_text = walkthecounty_get_price_option_name( $form_id, $price_id, 0, false );
	 ...
	 * @since 1.8.14
	 */
	return apply_filters( 'walkthecounty_payment_gateway_item_title', $item_name, $form_id, $payment_data );
}
 ...
 * @since       1.8.12
 */
function walkthecounty_payment_gateway_donation_summary( $donation_data, $name_and_email = true, $length = 255 ) {

	$form_id  = isset( $donation_data['post_data']['walkthecounty-form-id'] ) ? $donation_data['post_data']['walkthecounty-form-id'] : '';
	$price_id = isset( $donation_data['post_data']['walkthecounty-price-id'] ) ? $donation_data['post_data']['walkthecounty-price-id'] : '';

	// Form title.
	$summary = ( ! empty( $donation_data['post_data']['walkthecounty-form-title'] ) ? $donation_data['post_data']['walkthecounty-form-title'] : ( ! empty( $form_id ) ? wp_sprintf( __( 'Donation Form ID: %d', 'walkthecounty' ), $form_id ) : __( 'Untitled donation form', 'walkthecounty' ) ) );

	// Form multilevel if applicable.
	if ( ! empty( $price_id ) && 'custom' !== $price_id ) {
		$summary .= ': ' . walkthecounty_get_price_option_name( $form_id, $donation_data['post_data']['walkthecounty-price-id'] );
	}
	...
	return apply_filters( 'walkthecounty_payment_gateway_donation_summary', $summary );
}
 ...
 * @since 1.0
 */
function walkthecounty_get_host() {
 ...
 *
 * There is a hook walkthecounty_deprecated_function_run that will be called that can be used
 ...
 *
 * @uses do_action() Calls 'walkthecounty_deprecated_function_run' and passes the function name, what to use instead,
 *       and the version the function was deprecated in.
 * @uses apply_filters() Calls 'walkthecounty_deprecated_function_trigger_error' and expects boolean value of true to do
 *       trigger or false to not trigger error.
 */
function _walkthecounty_deprecated_function( $function, $version, $replacement = null, $backtrace = null ) {

	/**
	 * Fires while walkthecounty deprecated function call occurs.
	 *
	 ...
	 * @since 1.0
	 */
	do_action( 'walkthecounty_deprecated_function_run', $function, $replacement, $version );

	$show_errors = current_user_can( 'manage_options' );

	// Allow plugin to filter the output error trigger.
	if ( WP_DEBUG && apply_filters( 'walkthecounty_deprecated_function_trigger_error', $show_errors ) ) {
		if ( ! is_null( $replacement ) ) {
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since WalkTheCountyWP version %2$s! Use %3$s instead.', 'walkthecounty' ), $function, $version, $replacement ) );
			trigger_error( print_r( $backtrace, 1 ) ); // Limited to previous 1028 characters, but since we only need to move back 1 in stack that should be fine.
			// Alternatively we could dump this to a file.
		} else {
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since WalkTheCountyWP version %2$s with no alternative available.', 'walkthecounty' ), $function, $version ) );
			trigger_error( print_r( $backtrace, 1 ) );// Limited to previous 1028 characters, but since we only need to move back 1 in stack that should be fine.
			// Alternatively we could dump this to a file.
		}
	}
...
/**
 * WalkTheCounty Get Admin ID
 *
 * Helper function to return the ID of the post for admin usage
 *
 * @return string $post_id
 */
function walkthecounty_get_admin_post_id() {
 ...
 * @since 1.0
 */
function walkthecounty_get_php_arg_separator_output() {
 ...
 * @since 1.0
 */
function walkthecounty_month_num_to_name( $n ) {
 ...
 * @since 1.0
 */
function walkthecounty_is_func_disabled( $function ) {
 ...
 * @return string
 */
function walkthecounty_svg_icons( $icon ) {
...
function modify_nav_menu_meta_box_object( $post_type ) {
	if ( isset( $post_type->name ) && $post_type->name == 'walkthecounty_forms' ) {
		$post_type->labels->name = esc_html__( 'Donation Forms', 'walkthecounty' );
	}
 ...
 * @since 1.8.14
 */
function walkthecounty_donation_metabox_menu() {

	// Get Current Screen.
	$screen = get_current_screen();

	// Proceed, if current screen is navigation menus.
	if ( 'nav-menus' === $screen->id && walkthecounty_is_setting_enabled( walkthecounty_get_option( 'forms_singular' ) ) && ! get_user_option( 'walkthecounty_is_donation_forms_menu_updated' ) ) {
		...
		if ( ! in_array( 'add-post-type-walkthecounty_forms', $hidden_meta_boxes, true ) ) {
			return false;
		}

		// Exclude 'Donation Form' value from hidden meta box's list.
		$hidden_meta_boxes = array_diff( $hidden_meta_boxes, array( 'add-post-type-walkthecounty_forms' ) );

		// Get current user ID.
		$user = wp_get_current_user();

		update_user_option( $user->ID, 'metaboxhidden_nav-menus', $hidden_meta_boxes, true );
		update_user_option( $user->ID, 'walkthecounty_is_donation_forms_menu_updated', true, true );
	}
}

add_action( 'current_screen', 'walkthecounty_donation_metabox_menu' );
...
if ( ! function_exists( 'array_column' ) ) {
	...
	function array_column( $input = null, $columnKey = null, $indexKey = null ) {
		...
		if ( $argc < 2 ) {
			trigger_error( sprintf( esc_html__( 'array_column() expects at least 2 parameters, %s walkthecountyn.', 'walkthecounty' ), $argc ), E_USER_WARNING );

			return null;
		}

		if ( ! is_array( $params[0] ) ) {
			trigger_error( sprintf( esc_html__( 'array_column() expects parameter 1 to be array, %s walkthecountyn.', 'walkthecounty' ), gettype( $params[0] ) ), E_USER_WARNING );

			return null;
		}

		if ( ! is_int( $params[1] ) && ! is_float( $params[1] ) && ! is_string( $params[1] ) && $params[1] !== null && ! ( is_object( $params[1] ) && method_exists( $params[1], '__toString' ) ) ) {
			trigger_error( esc_html__( 'array_column(): The column key should be either a string or an integer.', 'walkthecounty' ), E_USER_WARNING );

			return false;
		}

		if ( isset( $params[2] ) && ! is_int( $params[2] ) && ! is_float( $params[2] ) && ! is_string( $params[2] ) && ! ( is_object( $params[2] ) && method_exists( $params[2], '__toString' ) ) ) {
			trigger_error( esc_html__( 'array_column(): The index key should be either a string or an integer.', 'walkthecounty' ), E_USER_WARNING );
 ...
 * @since 1.3.2
 */
function walkthecounty_can_view_receipt( $donation_id ) {
	...
	$walkthecounty_receipt_args['id'] = $donation_id;

	// Add backward compatibility.
	if ( ! is_numeric( $donation_id ) ) {
		$walkthecounty_receipt_args['id'] = walkthecounty_get_donation_id_by_key( $donation_id );
	}
	...
	if ( current_user_can( 'export_walkthecounty_reports' ) ) {
		 ...
		 * @since 2.3.1
		 */
		return apply_filters( 'walkthecounty_can_admin_view_receipt', true );
	}

	if ( is_user_logged_in() || current_user_can( 'view_walkthecounty_sensitive_data' ) ) {

		// Proceed only, if user is logged in or can view sensitive WalkTheCounty data.
		$donor = WalkTheCounty()->donors->get_donor_by( 'user_id', get_current_user_id() );

	} elseif ( ! is_user_logged_in() ) {

		// Check whether it is purchase session?
		// This condition is to show receipt to donor after donation.
		$purchase_session = walkthecounty_get_purchase_session();

		if (
			! empty( $purchase_session )
			&& absint( $purchase_session['donation_id'] ) === absint( $donation_id )
		) {
			$donor = WalkTheCounty()->donors->get_donor_by( 'email', $purchase_session['user_email'] );
		}

		// Check whether it is receipt access session?
		$receipt_session    = walkthecounty_get_receipt_session();
		$email_access_token = ! empty( $_COOKIE['walkthecounty_nl'] ) ? walkthecounty_clean( $_COOKIE['walkthecounty_nl'] ) : false;

		if (
			! empty( $receipt_session ) ||
			(
				walkthecounty_is_setting_enabled( walkthecounty_get_option( 'email_access' ) ) &&
				! empty( $email_access_token )
			)
		) {
			$donor = ! empty( $email_access_token )
				? WalkTheCounty()->donors->get_donor_by_token( $email_access_token )
				: false;
		}
	}
	...
	if ( is_object( $donor ) ) {
		...
		if ( ! $is_donor_donated ) {
			WalkTheCounty()->session->set( 'donor_donation_mismatch', true );
		}
	}

	return (bool) apply_filters( 'walkthecounty_can_view_receipt', $can_view_receipt, $donation_id );
 ...
 * @since 1.8.0
 *
 * @todo  update this function to query walkthecounty addon and additional
 *
 ...
 * and license validation for WalkTheCounty add-ons (`true` or `false`). Does not include
 * MU plugins.
 */
function walkthecounty_get_plugins( $args = array() ) {
	$plugins             = get_plugins();
	...
	foreach ( $plugins as $plugin_path => $plugin_data ) {
		...
		// Is the plugin a WalkTheCounty add-on?
		if (
			false !== strpos( $dirname, 'walkthecounty-' )
			&& (
				false !== strpos( $plugin_data['PluginURI'], 'walkthecountywp.com' )
				|| array_intersect( $author, array( 'WordImpress', 'WalkTheCountyWP' ) )
			)
		) {
			// Plugin is a WalkTheCounty-addon.
			$plugins[ $plugin_path ]['Type'] = 'add-on';

			$license_active = WalkTheCounty_License::get_license_by_plugin_dirname( $dirname );

			// Does a valid license exist?
			$plugins[ $plugin_path ]['License'] = $license_active && 'valid' === $license_active['license'];

		} else {
			// Plugin is not a WalkTheCounty add-on.
	...
	if( ! empty( $args['only_premium_add_ons'] ) ) {
		$premium_addons_list = walkthecounty_get_premium_add_ons();

		foreach ( $plugins as $key => $plugin ){
			$addon_shortname = str_replace( 'walkthecounty-', '', $plugin['Dir'] );
			...
			if(
				'add-on' !== $plugin['Type']
				|| ( false === strpos( $plugin['PluginURI'], 'walkthecountywp.com' ) && ! $is_premium )
			) {
				unset( $plugins[$key] );
			}
		}
 ...
 * @since 1.8
 */
function walkthecounty_is_terms_enabled( $form_id ) {
	$form_option = walkthecounty_get_meta( $form_id, '_walkthecounty_terms_option', true );

	if ( walkthecounty_is_setting_enabled( $form_option, 'global' ) && walkthecounty_is_setting_enabled( walkthecounty_get_option( 'terms' ) ) ) {
		return true;
 ...
 */
function walkthecounty_delete_donation_stats( $date_range = '', $args = array() ) {

	// Delete all cache.
	$status = WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_options_like( 'walkthecounty_stats' ) );
	 ...
	 * @since 1.8.7
	 */
	do_action( 'walkthecounty_delete_donation_stats', $status, $date_range, $args );
 ...
 * @since 2.0
 */
function walkthecounty_is_add_new_form_page() {
	$status = false;

	if ( false !== strpos( $_SERVER['REQUEST_URI'], '/wp-admin/post-new.php?post_type=walkthecounty_forms' ) ) {
		$status = true;
	}
 ...
 *       If you want to get meta for donors then use get_meta of WalkTheCounty_Donor and
 *       If you want to get meta for logs then use get_meta of WalkTheCounty_Logging->logmeta_db.
 *
 * @param int    $id
 ...
 * @since 1.8.8
 */
function walkthecounty_get_meta( $id, $meta_key = '', $single = false, $default = false, $meta_type = '' ) {
	switch ( $meta_type ) {
		case 'donation':
			$meta_value = WalkTheCounty()->payment_meta->get_meta( $id, $meta_key, $single );
			break;

		case 'form':
			$meta_value = WalkTheCounty()->form_meta->get_meta( $id, $meta_key, $single );
			break;

		case 'donor':
			$meta_value = WalkTheCounty()->donor_meta->get_meta( $id, $meta_key, $single );
			break;
	 ...
	 * @since 1.8.8
	 */
	$meta_value = apply_filters( 'walkthecounty_get_meta', $meta_value, $id, $meta_key, $default, $meta_value );
 ...
 * @since 1.8.8
 */
function walkthecounty_update_meta( $id, $meta_key, $meta_value, $prev_value = '', $meta_type = '' ) {
	switch ( $meta_type ) {
		case 'donation':
			$status = WalkTheCounty()->payment_meta->update_meta( $id, $meta_key, $meta_value, $prev_value );
			break;

		case 'form':
			$status = WalkTheCounty()->form_meta->update_meta( $id, $meta_key, $meta_value, $prev_value );
			break;

		case 'donor':
			$status = WalkTheCounty()->donor_meta->update_meta( $id, $meta_key, $meta_value, $prev_value );
			break;
	 ...
	 * @since 1.8.8
	 */
	return apply_filters( 'walkthecounty_update_meta', $status, $id, $meta_key, $meta_value, $meta_type );
}
 ...
 * @since 1.8.8
 */
function walkthecounty_delete_meta( $id, $meta_key, $meta_value = '', $meta_type = '' ) {
	switch ( $meta_type ) {
		case 'donation':
			$status = WalkTheCounty()->payment_meta->delete_meta( $id, $meta_key, $meta_value );
			break;

		case 'form':
			$status = WalkTheCounty()->form_meta->delete_meta( $id, $meta_key, $meta_value );
			break;

		case 'donor':
			$status = WalkTheCounty()->donor_meta->delete_meta( $id, $meta_key, $meta_value );
			break;
	 ...
	 * @since 1.8.8
	 */
	return apply_filters( 'walkthecounty_delete_meta', $status, $id, $meta_key, $meta_value, $meta_type );
}
 ...
 * @since  1.0
 */
function walkthecounty_has_upgrade_completed( $upgrade_action = '' ) {
	// Bailout.
	...
	$is_fresh_install = ! WalkTheCounty_Cache_Setting::get_option( 'walkthecounty_version' );
	if ( $is_fresh_install ) {
		return true;
	}
 ...
 * @since 1.8
 */
function walkthecounty_maybe_resume_upgrade() {
	$doing_upgrade = get_option( 'walkthecounty_doing_upgrade', false );
 ...
 * @since  1.0
 */
function walkthecounty_set_upgrade_complete( $upgrade_action = '' ) {
	 ...
	 * @since 1.8.12
	 */
	do_action( 'walkthecounty_set_upgrade_completed', $upgrade_action, $completed_upgrades );

	return update_option( 'walkthecounty_completed_upgrades', $completed_upgrades, false );
}
 ...
 * @since  1.0
 */
function walkthecounty_get_completed_upgrades() {
	return (array) WalkTheCounty_Cache_Setting::get_option( 'walkthecounty_completed_upgrades' );
}
 ...
 * @global wpdb  $wpdb
 */
function __walkthecounty_v20_bc_table_details( $type ) {
	global $wpdb;
	$table = array();
	...
	switch ( $type ) {
		...
		case 'payment':
			...
			$table['column']['id'] = WalkTheCounty()->payment_meta->get_meta_type() . '_id';
	}
...
/**
 * Remove the WalkTheCounty transaction pages from WP search results.
 *
 * @param WP_Query $query
 *
 * @since 1.8.13
 */
function walkthecounty_remove_pages_from_search( $query ) {

	if ( ! $query->is_admin && $query->is_search && $query->is_main_query() ) {

		$transaction_failed = walkthecounty_get_option( 'failure_page', 0 );
		$success_page       = walkthecounty_get_option( 'success_page', 0 );

		$args = apply_filters(
			'walkthecounty_remove_pages_from_search',
			array(
				$transaction_failed,
				$success_page,
			),
...
add_action( 'pre_get_posts', 'walkthecounty_remove_pages_from_search', 10, 1 );
 ...
 */
function walkthecounty_array_insert_before( $key, array &$array, $new_key, $new_value ) {
 ...
 * @see   array_insert_before()
 */
function walkthecounty_array_insert_after( $key, array &$array, $new_key, $new_value ) {
	if ( array_key_exists( $key, $array ) ) {
		$new = array();
		foreach ( $array as $k => $value ) {
			$new[ $k ] = $value;
 ...
 * @since 1.8.13
 */
function walkthecounty_list_pluck( $list, $field, $index_key = null ) {
...
function add_donor_meta( $donor_id, $meta_key, $meta_value, $unique = false ) {
	return add_metadata( 'walkthecounty_customer', $donor_id, $meta_key, $meta_value, $unique );
}
...
function delete_donor_meta( $donor_id, $meta_key, $meta_value = '' ) {
	return delete_metadata( 'walkthecounty_customer', $donor_id, $meta_key, $meta_value );
}
...
function get_donor_meta( $donor_id, $key = '', $single = false ) {
	return get_metadata( 'walkthecounty_customer', $donor_id, $key, $single );
}
...
function update_donor_meta( $donor_id, $meta_key, $meta_value, $prev_value = '' ) {
	return update_metadata( 'walkthecounty_customer', $donor_id, $meta_key, $meta_value, $prev_value );
}


/**
 * WalkTheCounty recalculate income and donation of the donation from ID
 *
 ...
 * @since 1.8.13
 */
function walkthecounty_recount_form_income_donation( $form_id = 0 ) {
	...
	if ( ! empty( $form_id ) ) {
		 ...
		 */
		$args = apply_filters(
			'walkthecounty_recount_form_stats_args',
			array(
		...
		walkthecounty_update_meta( $form_id, '_walkthecounty_form_sales', $totals['sales'] );
		walkthecounty_update_meta( $form_id, '_walkthecounty_form_earnings', walkthecounty_sanitize_amount_for_db( $totals['earnings'] ) );
	}// End if().
}
 ...
 * @since 1.8.17
 */
function walkthecounty_get_attribute_str( $attributes, $default_attributes = array() ) {
	$attribute_str = '';
 ...
 * @since 1.8.17
 */
function walkthecounty_get_wp_upload_dir() {
 ...
 * @since 1.8.17
 */
function walkthecounty_get_core_settings_json( $file_name ) {
	$upload_dir = walkthecounty_get_wp_upload_dir();
	$file_path  = $upload_dir . '/' . $file_name;

	if ( is_wp_error( $file_path ) || empty( $file_path ) ) {
		WalkTheCounty_Admin_Settings::add_error( 'walkthecounty-import-csv', __( 'Please upload or provide a valid JSON file.', 'walkthecounty' ) );
	}
 ...
 * @since 1.8.17
 */
function walkthecounty_get_limit_display_donations() {
	return walkthecounty_get_option( 'limit_display_donations', 1 );
}
 ...
 * @since 1.8.17
 */
function walkthecounty_donation_history_table_end() {
	$email = WalkTheCounty()->session->get( 'walkthecounty_email' );
	?>
	<tfoot>
		<tr>
			<td colspan="9999">
				<div class="walkthecounty-security-wrap">
					<div class="walkthecounty-security-column walkthecounty-security-description-wrap">
						<?php
						echo sprintf( __( 'For security reasons, please confirm your email address (%s) to view your complete donation history.', 'walkthecounty' ), $email );
						?>
					</div>
					<div class="walkthecounty-security-column walkthecounty-security-button-wrap">
						<a href="#" data-email="<?php echo $email; ?>" id="walkthecounty-confirm-email-btn"
						   class="walkthecounty-confirm-email-btn walkthecounty-btn">
							<?php _e( 'Confirm Email', 'walkthecounty' ); ?>
						</a>
						<span><?php _e( 'Email Sent!', 'walkthecounty' ); ?></span>
					</div>
				</div>
			</td>
		</tr>
 ...
 * @since  1.8.18
 */
function walkthecounty_doing_it_wrong( $function, $message, $version ) {
 ...
 * @since 1.8.18
 */
function walkthecounty_ignore_user_abort() {
	ignore_user_abort( true );

	if ( ! walkthecounty_is_func_disabled( 'set_time_limit' ) && ! ini_get( 'safe_mode' ) ) {
		set_time_limit( 0 );
	}
}
 ...
 * @since 2.0.2
 */
function walkthecounty_get_total_post_type_count( $post_type = '', $args = array() ) {
	global $wpdb;
	$where = '';
 ...
 * @since  2.0.5
 */
function walkthecounty_maybe_define_constant( $name, $value ) {
	if ( ! defined( $name ) ) {
		define( $name, $value );
	}
}
 ...
 * @since 2.1.0
 */
function walkthecounty_time_do_tags( $string, $timestamp = 0 ) {
	 ...
	 * @since 2.1.0
	 */
	return apply_filters( 'walkthecounty_time_do_tags', $formatted_string, $string, $timestamp );
}
 ...
 * @since 2.1
 */
function walkthecounty_is_company_field_enabled( $form_id ) {
	$form_setting_val   = walkthecounty_get_meta( $form_id, '_walkthecounty_company_field', true );
	$global_setting_val = walkthecounty_get_option( 'company_field' );

	if ( ! empty( $form_setting_val ) ) {
		if ( walkthecounty_is_setting_enabled( $form_setting_val, array( 'required', 'optional' ) ) ) {
			return true;
		} elseif ( 'global' === $form_setting_val && walkthecounty_is_setting_enabled(
			$global_setting_val,
			array(
				'required',
				'optional',
	...
	} elseif ( walkthecounty_is_setting_enabled( $global_setting_val, array( 'required', 'optional' ) ) ) {
		return true;
 ...
 * @since 2.1
 */
function walkthecounty_is_anonymous_donation_field_enabled( $form_id ) {
	$form_setting_val   = walkthecounty_get_meta( $form_id, '_walkthecounty_anonymous_donation', true, 'global' );
	$global_setting_val = walkthecounty_get_option( 'anonymous_donation', 'disabled' );

	if ( ! empty( $form_setting_val ) ) {
		if ( walkthecounty_is_setting_enabled( $form_setting_val ) ) {
			return true;
		} elseif ( 'global' === $form_setting_val && walkthecounty_is_setting_enabled( $global_setting_val ) ) {
			return true;
		} else {
			return false;
		}
	} elseif ( walkthecounty_is_setting_enabled( $global_setting_val ) ) {
		return true;
	}
 ...
 * @since 2.1
 */
function walkthecounty_is_donor_comment_field_enabled( $form_id ) {
	$form_setting_val   = walkthecounty_get_meta( $form_id, '_walkthecounty_donor_comment', true, 'global' );
	$global_setting_val = walkthecounty_get_option( 'donor_comment', 'disabled' );
 ...
 *
 * @param string $banner_addon_name WalkTheCounty add-on name.
 *
 * @return array
 * @since 2.1.0
 */
function __walkthecounty_get_active_by_user_meta( $banner_addon_name ) {
	global $wpdb;

	// Get the option key.
	$option_name = WalkTheCounty_Addon_Activation_Banner::get_banner_user_meta_key( $banner_addon_name );
	$data        = array();

	if ( empty( $GLOBALS['walkthecounty_addon_activated_by_user'] ) ) {
		$GLOBALS['walkthecounty_addon_activated_by_user'] = array();
		...
		$activation_banners = $wpdb->get_results(
			"
					...
					AND option_name LIKE '%walkthecounty_addon%'
					",
			ARRAY_A
		);

		if ( ! empty( $activation_banners ) ) {
			$GLOBALS['walkthecounty_addon_activated_by_user'] = array_combine(
	...
	if ( array_key_exists( $option_name, $GLOBALS['walkthecounty_addon_activated_by_user'] ) ) {
		$data = maybe_unserialize( $GLOBALS['walkthecounty_addon_activated_by_user'][ $option_name ] );
	}
 ...
 * @since 2.1.3
 */
function walkthecounty_get_nonce_life() {
	/**
 ...
 * @since 2.1.3
 */
function walkthecounty_get_nonce_field( $action, $name, $referer = false ) {
	return str_replace(
		"id=\"{$name}\"",
		'',
 ...
 *
 * @param int|WalkTheCounty_Donate_Form $form Form ID or Form Object.
 *
 * @return array
 * @since 2.1
 */
function walkthecounty_goal_progress_stats( $form ) {

	if ( ! $form instanceof WalkTheCounty_Donate_Form ) {
		$form = new WalkTheCounty_Donate_Form( $form );
	}

	$goal_format = walkthecounty_get_form_goal_format( $form->ID );
	 ...
	 * @since 1.8.8
	 */
	$total_goal = apply_filters( 'walkthecounty_goal_amount_target_output', round( walkthecounty_maybe_sanitize_amount( $form->goal ), 2 ), $form->ID, $form );
	...
	switch ( $goal_format ) {
		case 'donation':
			 ...
			 * @since 2.1
			 */
			$actual = apply_filters( 'walkthecounty_goal_donations_raised_output', $form->sales, $form->ID, $form );
			break;
		case 'donors':
			/**
			 ...
			 * @param int              $form_id Donation Form ID.
			 * @param WalkTheCounty_Donate_Form $form    instances of WalkTheCounty_Donate_Form.
			 *
			 * @return int $donors Total number of donors that donated to the form.
			 * @since 2.1.3
			 */
			$actual = apply_filters( 'walkthecounty_goal_donors_target_output', walkthecounty_get_form_donor_count( $form->ID ), $form->ID, $form );
			break;
		default:
			/**
			 ...
			 * @since 1.8.8
			 */
			$actual = apply_filters( 'walkthecounty_goal_amount_raised_output', $form->earnings, $form->ID, $form );
			break;
	}
	 ...
	 * @since 1.8.8
	 */
	$progress = apply_filters( 'walkthecounty_goal_amount_funded_percentage_output', $progress, $form->ID, $form );
	...
	switch ( $goal_format ) {
		...
		case 'amount' === $goal_format:
			$actual     = walkthecounty_currency_filter( walkthecounty_format_amount( $actual ) );
			$total_goal = walkthecounty_currency_filter( walkthecounty_format_amount( $total_goal ) );
			break;

		default:
			$actual     = walkthecounty_format_amount( $actual, array( 'decimal' => false ) );
			$total_goal = walkthecounty_format_amount( $total_goal, array( 'decimal' => false ) );
			break;
	}
	 ...
	 * @since 2.1
	 */
	return apply_filters( 'walkthecounty_goal_progress_stats', $stats_array );
}
 ...
 * @since 2.1.4
 */
function walkthecounty_get_admin_messages_key() {
	$messages = empty( $_GET['walkthecounty-messages'] ) ? array() : walkthecounty_clean( $_GET['walkthecounty-messages'] );

	// backward compatibility.
	if ( ! empty( $_GET['walkthecounty-message'] ) ) {
		$messages[] = walkthecounty_clean( $_GET['walkthecounty-message'] );
	}
	 ...
	 * @since 2.1.4
	 */
	return (array) apply_filters( 'walkthecounty_get_admin_messages_key', $messages );
}
 ...
 * @since 2.1.4
 */
function walkthecounty_get_user_agent() {

	// Get User Agent.
	$user_agent = ! empty( $_SERVER['HTTP_USER_AGENT'] ) ? walkthecounty_clean( $_SERVER['HTTP_USER_AGENT'] ) : ''; // WPCS: input var ok.
 ...
 * @since 2.2.0
 */
function walkthecounty_setcookie( $name, $value, $expire = 0, $secure = false ) {
	if ( ! headers_sent() ) {
		setcookie(
			$name,
			$value,
			...
			COOKIE_DOMAIN,
			$secure,
			apply_filters( 'walkthecounty_cookie_httponly', false, $name, $value, $expire, $secure )
		);
	}
}
 ...
 * @since 2.2.0
 */
function walkthecounty_get_formatted_address( $address = array() ) {
	$formatted_address = '';
	 ...
	 * @since 2.2.0
	 */
	$address_format = apply_filters( 'walkthecounty_address_format_template', "{street_address}\n{city}, {state} {postal_code}\n{country}" );
	...
	/**
	 * WalkTheCounty get formatted address.
	 *
	 ...
	 * @since 2.2.0
	 */
	$formatted_address = apply_filters( 'walkthecounty_get_formatted_address', $formatted_address, $address_format, $address );
 ...
 * @since 2.2.0
 */
function walkthecounty_get_safe_asset_url( $url ) {
	 ...
	 * @since 2.2.0
	 */
	return apply_filters( 'walkthecounty_get_safe_asset_url', $url );
}

/**
 * WalkTheCounty get formatted date.
 ...
 * @since 2.3.0
 */
function walkthecounty_get_formatted_date( $date, $format = 'Y-m-d', $current_format = '' ) {
	$current_format = empty( $current_format ) ? walkthecounty_date_format() : $current_format;
	 ...
	 * @since 2.3.0
	 */
	return apply_filters( 'walkthecounty_get_formatted_date', $formatted_date, array( $date, $format, $current_format ) );
}
 ...
 * @since 2.3.1
 */
function walkthecounty_get_receipt_link( $donation_id ) {

	return sprintf(
		'<a href="%1$s">%2$s</a>',
		esc_url( walkthecounty_get_receipt_url( $donation_id ) ),
		esc_html__( 'View the receipt in your browser &raquo;', 'walkthecounty' )
	);
 ...
 * @since 2.0
 */
function walkthecounty_get_receipt_url( $donation_id ) {
 ...
 * @since 2.4.1
 */
function walkthecounty_get_view_receipt_link( $donation_id ) {

	return sprintf(
		'<a href="%1$s">%2$s</a>',
		walkthecounty_get_view_receipt_url( $donation_id ),
		esc_html__( 'View the receipt in your browser &raquo;', 'walkthecounty' )
	);
 ...
 * @since 2.4.1
 */
function walkthecounty_get_view_receipt_url( $donation_id ) {

	$receipt_url = esc_url_raw(
		add_query_arg(
			array(
				'action'     => 'view_in_browser',
				'_walkthecounty_hash' => walkthecounty_get_payment_key( $donation_id ),
			),
			walkthecounty_get_history_page_uri()
		)
	);
 ...
 * @since 2.4.1
 */
function walkthecounty_display_donation_receipt( $args ) {

	global $walkthecounty_receipt_args;

	$walkthecounty_receipt_args = $args;
	...
	if ( 'view_in_browser' !== $receipt_type ) {
		...
		$is_email_access = walkthecounty_is_setting_enabled( $email_access ) && ! WalkTheCounty()->email_access->token_exists;
		...
		if ( ! $donation_id ) {
			...
			if ( $is_email_access ) {
				...
				echo WalkTheCounty_Notices::print_frontend_notice( $args['error'], false, 'error' );
			}
		...
		if ( ! ( $user_can_view = walkthecounty_can_view_receipt( $donation_id ) ) ) {

			if ( true === WalkTheCounty()->session->get( 'donor_donation_mismatch' ) ) {
				 ...
				 * @since 2.3.1
				 */
				$donor_mismatch_text = apply_filters( 'walkthecounty_receipt_donor_mismatch_notice_text', __( 'You are trying to access invalid donation receipt. Please try again.', 'walkthecounty' ) );

				echo WalkTheCounty_Notices::print_frontend_notice(
					$donor_mismatch_text,
					false,
					'error'
				);
				...
				$walkthecounty_login_redirect = walkthecounty_get_current_page_url();

				WalkTheCounty_Notices::print_frontend_notice(
					apply_filters(
						'walkthecounty_must_be_logged_in_error_message',
						__( 'You must be logged in to view this donation receipt.', 'walkthecounty' )
					)
				);

				walkthecounty_get_template_part( 'shortcode', 'login' );
			}
		 ...
		 */
		if ( ! apply_filters( 'walkthecounty_user_can_view_receipt', $user_can_view, $args ) ) {
			return WalkTheCounty_Notices::print_frontend_notice( $args['error'], false, 'error' );
		}
	} else{
		$donation_id =  walkthecounty_get_donation_id_by_key( $get_data['donation_id'] );
		$walkthecounty_receipt_args['id'] = $donation_id;
	}

	walkthecounty_get_template_part( 'shortcode', 'receipt' );
 ...
 * @since 2.5.0
 */
function walkthecounty_get_addon_readme_url( $plugin_slug, $by_plugin_name = false ) {

	if ( $by_plugin_name ) {
		$plugin_slug = WalkTheCounty_License::get_short_name( $plugin_slug );
	}

	$website_url = trailingslashit( WalkTheCounty_License::get_website_url() );
	 ...
	 */
	$url = apply_filters(
		'walkthecounty_addon_readme_file_url',
...
/**
 * Refresh all walkthecountywp license.
 *
 ...
 * @since  2.5.0
 */
function walkthecounty_refresh_licenses( $wp_check_updates = true ) {
	$walkthecounty_licenses = get_option( 'walkthecounty_licenses', array() );
	$walkthecounty_addons   = walkthecounty_get_plugins( array( 'only_premium_add_ons' => true ) );

	if ( ! $walkthecounty_licenses && ! $walkthecounty_addons ) {
		return array();
	}

	$license_keys = $walkthecounty_licenses ? implode( ',', array_keys( $walkthecounty_licenses ) ) : '';

	$unlicensed_walkthecounty_addon = $walkthecounty_addons
		? array_values(
			array_diff(
				array_map(
					function ( $plugin_name ) {
						return trim( str_replace( 'WalkTheCounty - ', '', $plugin_name ) );
					},
					wp_list_pluck( $walkthecounty_addons, 'Name', true )
				),
				wp_list_pluck( $walkthecounty_licenses, 'item_name', true )
			)
		)
		: array();
	...
	$tmp = WalkTheCounty_License::request_license_api( array(
		...
		'unlicensed' => implode( ',', $unlicensed_walkthecounty_addon ),
	), true );
	...
	// @see https://github.com/impress-org/walkthecounty/issues/4176
	...
	foreach ( $check_licenses as $key => $data ) {
		...
		if ( ! $data['success'] ) {
			unset( $walkthecounty_licenses[ $key ] );
			continue;
		}

		$walkthecounty_licenses[ $key ] = $data;
	}
	...
	update_option( 'walkthecounty_licenses', $walkthecounty_licenses, 'no' );
	update_option( 'walkthecounty_get_versions', $tmp_update_plugins, 'no' );

	$refresh         = WalkTheCounty_License::refresh_license_status();
	$refresh['time'] = current_time( 'timestamp', 1 );

	update_option( 'walkthecounty_licenses_refreshed_last_checked', $refresh, 'no' );
	...
	return array(
		'walkthecounty_licenses'     => $walkthecounty_licenses,
		'walkthecounty_get_versions' => $tmp_update_plugins,
	);
}
 ...
 * @since 2.5.0
 */
function walkthecounty_check_addon_updates( $_transient_data ){
	if ( ! is_object( $_transient_data ) ) {
		$_transient_data = new stdClass;
	}

	$update_plugins = get_option( 'walkthecounty_get_versions', array() );
	$check_licenses = get_option( 'walkthecounty_licenses', array() );

	if ( ! $update_plugins ) {
		$data = walkthecounty_refresh_licenses( false );

		if(
			empty( $data['walkthecounty_get_versions'] )
			|| is_wp_error( $data )
		) {
			return $_transient_data;
		}

		$update_plugins = $data['walkthecounty_get_versions'];
	}
	...
	foreach ( $update_plugins as $key => $data ) {
		...
		foreach ( $plugins as $plugin ) {
			...
			$tmp_plugin = WalkTheCounty_License::get_plugin_by_slug( $plugin['slug'] );
