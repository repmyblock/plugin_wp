...
/**
 * WalkTheCounty_API Class
 *
 * Renders API returns as a JSON/XML array
 *
 * @since 1.1
 */
class WalkTheCounty_API {
	...
	/**
	 * Instance of WalkTheCounty Stats class
	 *
	 * @var object
	 * @access private
	 * @since  1.1
	...
	public function __construct() {
		...
		$this->versions = array(
			'v1' => 'WALKTHECOUNTY_API_V1',
		);

		foreach ( $this->get_versions() as $version => $class ) {
			require_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/api/class-walkthecounty-api-' . $version . '.php';
		}
		...
		add_action( 'walkthecounty_process_api_key', array( $this, 'process_api_key' ) );
		...
		$this->log_requests = apply_filters( 'walkthecounty_api_log_requests', $this->log_requests );

		// Setup WalkTheCounty_Payment_Stats instance
		$this->stats = new WalkTheCounty_Payment_Stats();
	...
	public function add_endpoint() {
		add_rewrite_endpoint( 'walkthecounty-api', EP_ALL );
	}
	...
	public function get_default_version() {
		...
		$version = get_option( 'walkthecounty_default_api_version' );

		if ( defined( 'WALKTHECOUNTY_API_VERSION' ) ) {
			$version = WALKTHECOUNTY_API_VERSION;
		} elseif ( ! $version ) {
			$version = 'v1';
		}
	...
	private function set_queried_version() {
		...
		$version = $wp_query->query_vars['walkthecounty-api'];
		...
		if ( strpos( $version, '/' ) ) {
			...
			$wp_query->query_vars['walkthecounty-api'] = str_replace( $version . '/', '', $wp_query->query_vars['walkthecounty-api'] );
	 ...
	 * @uses   WalkTheCounty_API::get_user()
	 * @uses   WalkTheCounty_API::invalid_key()
	 * @uses   WalkTheCounty_API::invalid_auth()
	 * @since  1.1
	 * @return bool
	 */
	private function validate_request() {
		...
		if ( ! empty( $wp_query->query_vars['walkthecounty-api'] ) && ( $wp_query->query_vars['walkthecounty-api'] !== 'forms' || ! empty( $wp_query->query_vars['token'] ) ) ) {
		...
		} elseif ( ! empty( $wp_query->query_vars['walkthecounty-api'] ) && $wp_query->query_vars['walkthecounty-api'] === 'forms' ) {
			$this->is_valid_request = true;
	...
	public function get_user( $key = '' ) {
		...
		$user = WalkTheCounty_Cache::get( md5( 'walkthecounty_api_user_' . $key ), true );

		if ( false === $user ) {
			$user = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = %s AND meta_value=%s LIMIT 1", $key, 'walkthecounty_user_public_key' ) );
			WalkTheCounty_Cache::set( md5( 'walkthecounty_api_user_' . $key ), $user, DAY_IN_SECONDS, true );
		}
	...
	public function get_user_public_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'walkthecounty_api_user_public_key' . $user_id );
		$user_public_key = WalkTheCounty_Cache::get( $cache_key, true );

		if ( empty( $user_public_key ) ) {
			$user_public_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'walkthecounty_user_public_key' AND user_id = %d", $user_id ) );
			WalkTheCounty_Cache::set( $cache_key, $user_public_key, HOUR_IN_SECONDS, true );
		}
	...
	public function get_user_secret_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'walkthecounty_api_user_secret_key' . $user_id );
		$user_secret_key = WalkTheCounty_Cache::get( $cache_key, true );

		if ( empty( $user_secret_key ) ) {
			$user_secret_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'walkthecounty_user_secret_key' AND user_id = %d", $user_id ) );
			WalkTheCounty_Cache::set( $cache_key, $user_secret_key, HOUR_IN_SECONDS, true );
		}
	 ...
	 *
	 * @access private
	 * @uses   WalkTheCounty_API::output()
	 * @since  1.1
	 */
	private function missing_auth() {
		$error          = array();
		$error['error'] = __( 'You must specify both a token and API key.', 'walkthecounty' );
	...
	private function invalid_auth() {
		$error          = array();
		$error['error'] = __( 'Your request could not be authenticated.', 'walkthecounty' );
	...
	private function invalid_key() {
		$error          = array();
		$error['error'] = __( 'Invalid API key.', 'walkthecounty' );
	...
	private function invalid_version() {
		$error          = array();
		$error['error'] = __( 'Invalid API version.', 'walkthecounty' );
	...
	public function process_query() {
		...
		// Check for walkthecounty-api var. Get out if not present
		if ( empty( $wp_query->query_vars['walkthecounty-api'] ) ) {
			return;
		}
		...
		if ( ! defined( 'WALKTHECOUNTY_DOING_API' ) ) {
			define( 'WALKTHECOUNTY_DOING_API', true );
		}
		...
		$this->data = apply_filters( 'walkthecounty_api_output_data', $data, $this->endpoint, $this );
	...
	public function set_query_mode() {
		...
		$accepted = apply_filters( 'walkthecounty_api_valid_query_modes', array(
			'stats',
			'forms',
			'donors',
			'donations',
		...
		$query = isset( $wp_query->query_vars['walkthecounty-api'] ) ? $wp_query->query_vars['walkthecounty-api'] : null;
		...
		if ( ! in_array( $query, $accepted ) ) {
			$error['error'] = __( 'Invalid query.', 'walkthecounty' );
	...
	public function per_page() {
		...
		return apply_filters( 'walkthecounty_api_results_per_page', $per_page );
	}
	...
	public function get_dates( $args = array() ) {
		 ...
		 */
		return apply_filters( 'walkthecounty_api_stat_dates', $dates );
	}
	...
	public function get_donors( $donor = null ) {
		...
		$donors = array();
		$error  = array();
		if ( ! user_can( $this->user_id, 'view_walkthecounty_sensitive_data' ) && ! $this->override ) {
			return $donors;
		}
		...
		$donor_query = WalkTheCounty()->donors->get_donors( array(
			'number' => $per_page,
			'offset' => $offset,
			$field   => $donor,
		) );
		...
		if ( $donor_query ) {
			...
			foreach ( $donor_query as $donor_obj ) {
				...
				$title_prefix = WalkTheCounty()->donor_meta->get_meta( $donor_obj->id, '_walkthecounty_donor_title_prefix', true );
				...
				$donor = new WalkTheCounty_Donor( $donor_obj->id );
			...
			$error['error'] = sprintf(
				/* translators: %s: donor */
				__( 'Donor %s not found.', 'walkthecounty' ),
				$donor
			);
			...
			$error['error'] = __( 'No donors found.', 'walkthecounty' );
	 ...
	 * @since  1.1
	 *
	 * @param int $form WalkTheCounty Form ID.
	 *
	...
	public function get_forms( $form = null ) {
		...
		if ( $form == null ) {
			...
			if ( get_post_type( $form ) == 'walkthecounty_forms' ) {
				...
				$error['error'] = sprintf( /* translators: %s: form */
					__( 'Form %s not found.', 'walkthecounty' ), $form );
	...
	/**
	 * WalkTheCountyn a walkthecounty_forms post object, generate the data for the API output
	 *
	 * @since  1.1
	 *
	 * @param  object $form_info The WalkTheCounty Form's Post Object.
	 *
	...
	private function get_form_data( $form_info ) {
		...
		$form['info']['content']       = walkthecounty_get_meta( $form_info->ID, '_walkthecounty_form_content', true );
		...
		if ( walkthecounty_is_setting_enabled( walkthecounty_get_option( 'categories', 'disabled' ) ) ) {
			$form['info']['category'] = get_the_terms( $form_info, 'walkthecounty_forms_category' );
			$form['info']['tags']     = get_the_terms( $form_info, 'walkthecounty_forms_tag' );
		}
		...
		$goal_option = walkthecounty_get_meta( $form_info->ID, '_walkthecounty_goal_option', true );
		$goal_amount = walkthecounty_get_meta( $form_info->ID, '_walkthecounty_set_goal', true );
		if ( walkthecounty_is_setting_enabled( $goal_option ) && $goal_amount ) {
			$total_income                         = walkthecounty_get_form_earnings_stats( $form_info->ID );
			$goal_percentage_completed            = ( $total_income < $goal_amount ) ? round( ( $total_income / $goal_amount ) * 100, 2 ) : 100;
			$form['goal']['amount']               = isset( $goal_amount ) ? walkthecounty_format_decimal( array( 'amount' => $goal_amount, 'currency' => $currency  ) ) : '';
			$form['goal']['percentage_completed'] = isset( $goal_percentage_completed ) ? $goal_percentage_completed : '';
		}

		if ( user_can( $this->user_id, 'view_walkthecounty_reports' ) || $this->override ) {
			$form['stats']['total']['donations']           = walkthecounty_get_form_sales_stats( $form_info->ID );
			$form['stats']['total']['earnings']            = walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_form_earnings_stats( $form_info->ID ), 'currency' => $currency  ) );
			$form['stats']['monthly_average']['donations'] = walkthecounty_get_average_monthly_form_sales( $form_info->ID );
			$form['stats']['monthly_average']['earnings']  = walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_average_monthly_form_earnings( $form_info->ID ), 'currency' => $currency  ) );
		}

		$counter = 0;
		if ( walkthecounty_has_variable_prices( $form_info->ID ) ) {
			foreach ( walkthecounty_get_variable_prices( $form_info->ID ) as $price ) {
				$counter ++;
				// multi-level item
				$level                                     = isset( $price['_walkthecounty_text'] ) ? $price['_walkthecounty_text'] : 'level-' . $counter;
				$form['pricing'][ sanitize_key( $level ) ] = walkthecounty_format_decimal( array( 'amount' => $price['_walkthecounty_amount'], 'currency' => $currency ) );

			}
		} else {
			$form['pricing']['amount'] = walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_form_price( $form_info->ID ), 'currency' => $currency ) );
		}
		...
		if ( user_can( $this->user_id, 'view_walkthecounty_sensitive_data' ) || $this->override ) {
			 ...
			 * @since 1.1
			 */
			do_action( 'walkthecounty_api_sensitive_data' );

		}

		return apply_filters( 'walkthecounty_api_forms_form', $form );
	...
	public function get_stats( $args = array() ) {
		...
		if ( $args['type'] == 'donations' ) {
			...
			if ( $args['form'] == null ) {
				if ( $args['date'] == null ) {
					...
					if ( $args['enddate'] < $args['startdate'] ) {
						$error['error'] = __( 'The end date must be later than the start date.', 'walkthecounty' );
					}

					// Ensure both the start and end date are specified
					if ( empty( $args['startdate'] ) || empty( $args['enddate'] ) ) {
						$error['error'] = __( 'Invalid or no date range specified.', 'walkthecounty' );
					}
					...
					while ( $y <= $dates['year_end'] ) :
						...
						while ( $i <= $month_end ) :
							...
							while ( $d <= $num_of_days ) :
								$sale_count = walkthecounty_get_sales_by_date( $d, $i, $y );
					...
					if ( $args['date'] == 'this_quarter' || $args['date'] == 'last_quarter' ) {
						...
						while ( $month <= $dates['m_end'] ) :
							$donations_count += walkthecounty_get_sales_by_date( null, $month, $dates['year'] );
							$month ++;
						endwhile;

						$donations['donations'][ $args['date'] ] = $donations_count;
					} else {
						$donations['donations'][ $args['date'] ] = walkthecounty_get_sales_by_date( $dates['day'], $dates['m_start'], $dates['year'] );
					}
				}// End if().
				...
				if ( get_post_type( $args['form'] ) == 'walkthecounty_forms' ) {
					...
					$error['error'] = sprintf( /* translators: %s: form */
						__( 'Form %s not found.', 'walkthecounty' ), $args['form'] );
				}
			}// End if().
			...
			if ( $args['form'] == null ) {
				if ( $args['date'] == null ) {
					...
					while ( $y <= $dates['year_end'] ) :
						...
						while ( $i <= $month_end ) :
							...
							while ( $d <= $num_of_days ) :
								$earnings_stat = walkthecounty_get_earnings_by_date( $d, $i, $y );
								$date_key      = date( 'Ymd', strtotime( $y . '/' . $i . '/' . $d ) );
								if ( ! isset( $earnings['earnings'][ $date_key ] ) ) {
									$earnings['earnings'][ $date_key ] = 0;
								}

								$earnings['earnings'][ $date_key ] += walkthecounty_format_decimal( array( 'amount' => $earnings_stat, 'currency' => $currency ) );
								$total                             += $earnings_stat;
								$d ++;
							endwhile;
					...
					$earnings['totals'] = walkthecounty_format_decimal( array( 'amount' => $total, 'currency' => $currency ) );
				} else {
					if ( $args['date'] == 'this_quarter' || $args['date'] == 'last_quarter' ) {
						...
						while ( $month <= $dates['m_end'] ) :
							$earnings_count += walkthecounty_get_earnings_by_date( null, $month, $dates['year'] );
							$month ++;
						endwhile;

						$earnings['earnings'][ $args['date'] ] = walkthecounty_format_decimal( array( 'amount' => $earnings_count, 'currency' => $currency ) );
					} else {
						$earnings['earnings'][ $args['date'] ] = walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_earnings_by_date( $dates['day'], $dates['m_start'], $dates['year'] ), 'currency' => $currency ) );
					}
				}// End if().
			} elseif ( $args['form'] == 'all' ) {
				$forms = get_posts( array(
				...
				foreach ( $forms as $form_info ) {
					$earnings['earnings'][ $i ] = array(
						$form_info->post_name => walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_form_earnings_stats( $form_info->ID ), 'currency' => $currency ) ),
					);
					$i ++;
				}
			} else {
				if ( get_post_type( $args['form'] ) == 'walkthecounty_forms' ) {
					...
					$earnings['earnings'][0] = array(
						$form_info->post_name => walkthecounty_format_decimal( array( 'amount' => $this->stats->get_earnings(
							$args['form'],
							is_numeric( $args['startdate'] )
								? strtotime( $args['startdate'] )
								: $args['startdate'],
			...
			$donors                             = new WalkTheCounty_DB_Donors();
	...
	public function get_recent_donations( $args = array() ) {
		...
		if ( isset( $wp_query->query_vars['id'] ) ) {
			$query   = array();
			$query[] = new WalkTheCounty_Payment( $wp_query->query_vars['id'] );
		} elseif ( isset( $wp_query->query_vars['purchasekey'] ) ) {
			$query   = array();
			$query[] = walkthecounty_get_payment_by( 'key', $wp_query->query_vars['purchasekey'] );
		} elseif ( isset( $wp_query->query_vars['email'] ) ) {
			$args  = array(
				'fields'     => 'ids',
				'meta_key'   => '_walkthecounty_payment_donor_email',
		...
		if ( $query ) {
			...
			foreach ( $query as $payment ) {
				...
				if ( is_numeric( $payment ) ) {
					$payment      = new WalkTheCounty_Payment( $payment );
				}
				...
				$donations['donations'][ $i ]['total']          = walkthecounty_format_decimal( array( 'donation_id' => $payment->ID, 'dp' => true ) );
				$donations['donations'][ $i ]['status']         = walkthecounty_get_payment_status( $payment, true );
				...
				$price    = isset( $payment_meta['form_id'] ) ? walkthecounty_get_form_price( $payment_meta['form_id'] ) : false;
				$price_id = isset( $payment_meta['price_id'] ) ? $payment_meta['price_id'] : null;

				$donations['donations'][ $i ]['form']['id']    = $form_id;
				$donations['donations'][ $i ]['form']['name']  = get_the_title( $payment_meta['form_id'] );
				$donations['donations'][ $i ]['form']['price'] = walkthecounty_format_decimal( array( 'amount' => $price, 'currency' => walkthecounty_get_option('currency' ), 'dp' => true ) );

				if ( walkthecounty_has_variable_prices( $form_id ) ) {
					if ( isset( $payment_meta['price_id'] ) ) {
						$price_name                                         = walkthecounty_get_price_option_name( $form_id, $payment_meta['price_id'], $payment->ID );
						$donations['donations'][ $i ]['form']['price_name'] = $price_name;
						$donations['donations'][ $i ]['form']['price_id']   = $price_id;
						$donations['donations'][ $i ]['form']['price']      = walkthecounty_format_decimal( array( 'amount' => walkthecounty_get_price_option_amount( $form_id, $price_id ), 'currency' => walkthecounty_get_option('currency' ), 'dp' => true ) );
					}
				}
				...
				if( ! empty( $payment_meta ) ) {
					...
					foreach ( $payment_meta as $meta_key => $meta_value ) {
						...
						$exceptions = array(
							...
							'price_id',
							'user_info',
							'_walkthecounty_payment_purchase_key',
							'key',
							'email',
							'date',
							'currency',
						...
						// Meta key can contain price value like _walkthecounty_fee_amount, so convert them to standard format.
						if( walkthecounty_is_amount_sanitized( $meta_value ) ) {
							$meta_value = walkthecounty_format_decimal( array( 'amount' => $meta_value, 'currency' => walkthecounty_get_option('currency' ), 'dp' => true ) );
						}
		...
		return apply_filters( 'walkthecounty_api_donations_endpoint', $donations );
	}
	...
	public function get_output_format() {
		...
		return apply_filters( 'walkthecounty_api_output_format', $format );
	}
	...
	private function log_request( $data = array() ) {
		...
		$query = array(
			'walkthecounty-api'    => $wp_query->query_vars['walkthecounty-api'],
		...
		$log_meta = array(
			...
			'request_ip' => walkthecounty_get_ip(),
			'user'       => $this->user_id,
			'key'        => isset( $wp_query->query_vars['key'] ) ? $wp_query->query_vars['key'] : null,
			'token'      => isset( $wp_query->query_vars['token'] ) ? $wp_query->query_vars['token'] : null,
	...
	public function output( $status_code = 200 ) {
		 ...
		 * @param WalkTheCounty_API $this   The WalkTheCounty_API object.
		 * @param string   $format Output format, XML or JSON. Default is JSON.
		 */
		do_action( 'walkthecounty_api_output_before', $this->data, $this, $format );

		switch ( $format ) :

			case 'xml' :

				require_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/libraries/array2xml.php';
				$xml = Array2XML::createXML( 'walkthecounty', $this->data );
				echo $xml->saveXML();
			...
			default :
				 ...
				 * @param WalkTheCounty_API $this The WalkTheCounty_API object.
				 */
				do_action( "walkthecounty_api_output_{$format}", $this->data, $this );
		 ...
		 * @param string   $format Output format, XML or JSON. Default is JSON.
		 */
		do_action( 'walkthecounty_api_output_after', $this->data, $this, $format );
	...
	function user_key_field( $user ) {
		...
		if ( ( walkthecounty_get_option( 'api_allow_user_keys', false ) || current_user_can( 'manage_walkthecounty_settings' ) ) && current_user_can( 'edit_user', $user->ID ) ) {
			...
			<table class="form-table">
				...
				<tr>
					<th>
						<?php _e( 'WalkTheCountyWP API Keys', 'walkthecounty' ); ?>
					</th>
					<td>
						<?php
						...
						?>
						<?php if ( empty( $user->walkthecounty_user_public_key ) ) { ?>
							<input name="walkthecounty_set_api_key" type="checkbox" id="walkthecounty_set_api_key" />
							<span class="description"><?php _e( 'Generate API Key', 'walkthecounty' ); ?></span>
						<?php } else { ?>
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Public key:', 'walkthecounty' ); ?>
								&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="publickey" value="<?php echo esc_attr( $public_key ); ?>" />
							<br />
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Secret key:', 'walkthecounty' ); ?>
								&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="privatekey" value="<?php echo esc_attr( $secret_key ); ?>" />
							<br />
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Token:', 'walkthecounty' ); ?>
								&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="token" value="<?php echo esc_attr( $this->get_token( $user->ID ) ); ?>" />
							<br />
							<input name="walkthecounty_revoke_api_key" type="checkbox" id="walkthecounty_revoke_api_key" />
							<span class="description"><label for="walkthecounty_revoke_api_key"><?php _e( 'Revoke API Keys', 'walkthecounty' ); ?></label></span>
						<?php } ?>
					</td>
				</tr>
				</tbody>
	...
	public function process_api_key( $args ) {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'walkthecounty-api-nonce' ) ) {
		...
		if ( empty( $args['user_id'] ) ) {
			wp_die( __( 'User ID Required.', 'walkthecounty' ), __( 'Error', 'walkthecounty' ), array(
				'response' => 401,
			) );
		}
		...
		$process = isset( $args['walkthecounty_api_process'] ) ? strtolower( $args['walkthecounty_api_process'] ) : false;

		if ( $user_id == get_current_user_id() && ! walkthecounty_get_option( 'allow_user_api_keys' ) && ! current_user_can( 'manage_walkthecounty_settings' ) ) {
			wp_die( sprintf( /* translators: %s: process */
				__( 'You do not have permission to %s API keys for this user.', 'walkthecounty' ), $process ), __( 'Error', 'walkthecounty' ), array(
				'response' => 403,
			) );
		} elseif ( ! current_user_can( 'manage_walkthecounty_settings' ) ) {
			wp_die( sprintf( /* translators: %s: process */
				__( 'You do not have permission to %s API keys for this user.', 'walkthecounty' ), $process ), __( 'Error', 'walkthecounty' ), array(
				'response' => 403,
			) );
		...
		switch ( $process ) {
			case 'generate':
				if ( $this->generate_api_key( $user_id ) ) {
					WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( 'walkthecounty_total_api_keys' ) );
					wp_redirect( add_query_arg( 'walkthecounty-messages[]', 'api-key-generated', 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-tools&tab=api' ) );
					exit();
				} else {
					wp_redirect( add_query_arg( 'walkthecounty-messages[]', 'api-key-failed', 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-tools&tab=api' ) );
					exit();
				}
				break;
			case 'regenerate':
				...
				WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( 'walkthecounty_total_api_keys' ) );
				wp_redirect( add_query_arg( 'walkthecounty-messages[]', 'api-key-regenerated', 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-tools&tab=api' ) );
				exit();
				break;
			case 'revoke':
				$this->revoke_api_key( $user_id );
				WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( 'walkthecounty_total_api_keys' ) );
				wp_redirect( add_query_arg( 'walkthecounty-messages[]', 'api-key-revoked', 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-tools&tab=api' ) );
				exit();
				break;
			default;
				break;
	...
	public function generate_api_key( $user_id = 0, $regenerate = false ) {
		...
		if( ! empty( $_POST['from'] ) && 'profile' === $_POST['from'] ) {
			...
			if( ! empty( $_POST['walkthecounty_set_api_key'] ) ) {
				// Generate API Key from User Profile page.
				$new_public_key = $this->generate_public_key( $user->user_email );
				$new_secret_key = $this->generate_private_key( $user->ID );
			} elseif ( ! empty( $_POST['walkthecounty_revoke_api_key'] ) ) {
		...
		update_user_meta( $user_id, $new_public_key, 'walkthecounty_user_public_key' );
		update_user_meta( $user_id, $new_secret_key, 'walkthecounty_user_secret_key' );
	...
	public function revoke_api_key( $user_id = 0 ) {
		...
		if ( ! empty( $public_key ) ) {
			WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( md5( 'walkthecounty_api_user_' . $public_key ) ) );
			WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( md5( 'walkthecounty_api_user_public_key' . $user_id ) ) );
			WalkTheCounty_Cache::delete( WalkTheCounty_Cache::get_key( md5( 'walkthecounty_api_user_secret_key' . $user_id ) ) );
	...
	private function get_default_sales_stats() {
		...
		$donations['donations']['totals']        = walkthecounty_get_total_donations();
	...
	private function get_default_earnings_stats() {
		...
		$earnings                              = array();
		$earnings['earnings']['today']         = walkthecounty_format_decimal( array(
			'amount'   => $this->stats->get_earnings( 0, 'today' ),
			'currency' => $currency,
		) );
		$earnings['earnings']['current_month'] = walkthecounty_format_decimal( array(
			'amount'   => $this->stats->get_earnings( 0, 'this_month' ),
			'currency' => $currency,
		) );
		$earnings['earnings']['last_month']    = walkthecounty_format_decimal( array(
			'amount'   => $this->stats->get_earnings( 0, 'last_month' ),
			'currency' => $currency,
		) );
		$earnings['earnings']['totals']        = walkthecounty_format_decimal( array(
			'amount'   => walkthecounty_get_total_earnings(),
			'currency' => $currency,
		) );
	...
	public function api_key_backwards_compat( $check, $object_id, $meta_key, $single ) {
		...
		if ( $meta_key !== 'walkthecounty_user_public_key' && $meta_key !== 'walkthecounty_user_secret_key' ) {
			return $check;
		}
		...
		switch ( $meta_key ) {
			case 'walkthecounty_user_public_key':
				$return = WalkTheCounty()->api->get_user_public_key( $object_id );
				break;
			case 'walkthecounty_user_secret_key':
				$return = WalkTheCounty()->api->get_user_secret_key( $object_id );
				break;
		}
