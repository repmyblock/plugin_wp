...
/**
 * WalkTheCounty_API_V2 Class
 *
 * The base version API class
 *
 * @since  2.1
 */
class WalkTheCounty_API_V2 {
	/**
	 * API base prefix
	 *
	 * @since  2.1
	 ...
	 * @var string
	 */
	private $rest_base = 'walkthecounty-api/v2';
	...
	public function localize_script() {
		$data = array(
			'root' => esc_url_raw( WalkTheCounty_API_V2::get_rest_api() ),
			'rest_base' => $this->rest_base
		);

		if ( is_admin() ) {
			wp_localize_script( 'walkthecounty-admin-scripts', 'walkthecountyApiSettings', $data );
		} else {
			wp_localize_script( 'walkthecounty', 'walkthecountyApiSettings', $data );
		}
	}
	...
	public function get_forms_data( $request ) {
		...
		if ( ! isset( $parameters['id'] ) || empty( $parameters['id'] ) ) {
			return array( 'error' => 'no_parameter_walkthecountyn' );
		}

		return walkthecounty_form_shortcode( $parameters );
	}
	...
	public function get_donation_grid( $request ) {
		...
		return walkthecounty_form_grid_shortcode( $parameters );
	}
	...
	public function get_donor_wall( $request ) {
		...
		return WalkTheCounty_Donor_Wall::get_instance()->render_shortcode( $parameters );
	}
...
WalkTheCounty_API_V2::get_instance();
