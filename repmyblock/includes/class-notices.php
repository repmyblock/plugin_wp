...
/**
 * WalkTheCounty_Notices Class
 *
 * @since 1.8.9
 */
class WalkTheCounty_Notices {
	/**
	 * List of notices
	 * @var array
	 * @since  1.8.9
	...
	public function __construct() {
		...
		add_action( 'walkthecounty_dismiss_notices', array( $this, 'dismiss_notices' ) );

		add_action( 'walkthecounty_frontend_notices', array( $this, 'render_frontend_notices' ), 999 );
		add_action( 'walkthecounty_pre_form_output', array( $this, 'render_frontend_form_notices' ), 10, 1 );
		add_action( 'walkthecounty_ajax_donation_errors', array( $this, 'render_frontend_notices' ) );
		 ...
		 * @since 1.8.14
		 */
		add_filter( 'walkthecounty_register_notice_args', array( $this, 'bc_deprecated_params' ) );
		add_filter( 'walkthecounty_frontend_errors_args', array( $this, 'bc_deprecated_params' ) );
		add_filter( 'walkthecounty_frontend_notice_args', array( $this, 'bc_deprecated_params' ) );
	}
	...
	public function register_notice( $notice_args ) {
		 ...
		 * @since 1.8.14
		 */
		$notice_args = apply_filters( 'walkthecounty_register_notice_args', $notice_args );
	...
	public function render_admin_notices() {
		...
		// We don't want to annoy admins with notices on important screens like WP or WalkTheCountyWP updates, etc.
		if (
			'update-core' === $wp_screen->id
			|| 'walkthecounty_forms_page_walkthecounty-addons' === $wp_screen->id
			|| 'walkthecounty_forms_page_walkthecounty-updates' === $wp_screen->id
		) {
			return;
		}
		...
		foreach ( self::$notices as $notice_id => $notice ) {
			...
			$css_id = ( false === strpos( $notice['id'], 'walkthecounty' ) ? "walkthecounty-{$notice['id']}" : $notice['id'] );

			$css_class = 'walkthecounty-notice notice ' . ( empty( $notice['dismissible'] ) ? 'non' : 'is' ) . "-dismissible {$notice['type']} notice-{$notice['type']}";
			$output    .= sprintf(
				...
				$css_id,
				$css_class,
				walkthecounty_clean( $notice['dismissible'] ),
				$notice['dismissible_type'],
				$notice['dismiss_interval'],
				$notice['id'],
				empty( $notice['dismissible_type'] ) ? '' : wp_create_nonce( "walkthecounty_edit_{$notice_id}_notice" ),
	...
	/**
	 * Render walkthecounty frontend notices.
	 *
	 * @since  1.8.9
	 * @access public
	 *
	...
	public function render_frontend_notices( $form_id = 0 ) {
		...
		if ( ! isset( $_POST['walkthecounty_ajax'] ) && $request_form_id !== $form_id ) {
			return;
		}
	...
	public function render_frontend_form_notices( $form_id ) {
		$display_option = walkthecounty_get_meta( $form_id, '_walkthecounty_payment_display', true );

		if ( 'modal' === $display_option ) {
			add_action( 'walkthecounty_payment_mode_top', array( $this, 'render_frontend_notices' ) );
		} else {
			add_action( 'walkthecounty_pre_form', array( $this, 'render_frontend_notices' ), 11 );
		}
	}
		...
		if ( self::$has_auto_dismissible_notice ) :
			...
			<script>
				jQuery(document).ready(function () {
					...
					window.setTimeout(
						function () {
							jQuery('.walkthecounty-notice[data-dismissible="auto"]').slideUp();
						},
						5000
					);
				})
		...
		if ( self::$has_dismiss_interval_notice ) :
			...
			<script>
				jQuery(document).ready(function () {
					...
					$body.on('click', '.walkthecounty_dismiss_notice', function (e) {
						var $parent            = jQuery(this).parents('.walkthecounty-notice'),
							custom_notice_data = {
					...
					$body.on('click', 'button.notice-dismiss', function (e, custom_notice_data) {
						...
						var data = {
							'walkthecounty-action': 'dismiss_notices',
	...
	public function __reveal_notices(){
		...
		<script>
			jQuery(document).ready(function($){
				...
				window.setTimeout(
					function(){
						var walkthecounty_notices = $('.walkthecounty-notice');

						if( walkthecounty_notices.length ) {
							walkthecounty_notices.slideDown();
						}
					},
					1000
				);
	...
	public function dismiss_notices() {
		...
		if (
			...
			! check_ajax_referer( "walkthecounty_edit_{$notice_id}_notice", '_wpnonce' )
		) {
			wp_send_json_error();
		}

		$notice_key = WalkTheCounty()->notices->get_notice_key( $notice_id, $_post['dismiss_interval'] );
		if ( 'user' === $_post['dismissible_type'] ) {
			$current_user = wp_get_current_user();
			$notice_key   = WalkTheCounty()->notices->get_notice_key( $notice_id, $_post['dismiss_interval'], $current_user->ID );
		}

		$notice_dismiss_time = ! empty( $_post['dismiss_interval_time'] ) ? $_post['dismiss_interval_time'] : null;

		// Save option to hide notice.
		WalkTheCounty_Cache::set( $notice_key, true, $notice_dismiss_time, true );
		 ...
		 * @since 2.2.0
		 */
		do_action( 'walkthecounty_notice_dismissed', $_post );
	...
	public function get_notice_key( $notice_id, $dismiss_interval = null, $user_id = 0 ) {
		$notice_key = "_walkthecounty_notice_{$notice_id}";
	...
	public function get_dismiss_link( $notice_args ) {
		$notice_args = wp_parse_args(
			$notice_args,
			array(
				'title'                 => __( 'Click here', 'walkthecounty' ),
				'dismissible_type'      => '',
				'dismiss_interval'      => '',
				'dismiss_interval_time' => null,
			)
		...
		return sprintf(
			'<a href="#" class="walkthecounty_dismiss_notice" data-dismissible-type="%1$s" data-dismiss-interval="%2$s" data-dismiss-interval-time="%3$s">%4$s</a>',
	...
	public function is_notice_dismissed( $notice ) {
		...
		if ( 'user' === $notice['dismissible_type'] ) {
			$current_user = wp_get_current_user();
			$notice_key   = WalkTheCounty()->notices->get_notice_key( $notice['id'], $notice['dismiss_interval'], $current_user->ID );
		}

		$notice_data = WalkTheCounty_Cache::get( $notice_key, true );
	...
	public static function print_frontend_errors( $errors ) {
		...
		// Note: we will remove walkthecounty_errors class in future.
		$classes = apply_filters( 'walkthecounty_error_class', array( 'walkthecounty_notices', 'walkthecounty_errors' ) );
		...
		foreach ( $errors as $error_id => $error ) {
			 ...
			 * @since 1.8.14
			 */
			$notice_args = apply_filters( 'walkthecounty_frontend_errors_args', $notice_args );

			echo sprintf(
				'<div class="walkthecounty_error walkthecounty_notice" id="walkthecounty_error_%1$s" data-dismissible="%2$s" data-dismiss-interval="%3$d">
						<p><strong>%4$s</strong>: %5$s</p>
					</div>',
				$error_id,
				walkthecounty_clean( $notice_args['dismissible'] ),
	...
	public static function print_frontend_notice( $message, $echo = true, $notice_type = 'warning', $notice_args = array() ) {
		 ...
		 * @since 1.8.14
		 */
		$notice_args = apply_filters( 'walkthecounty_frontend_notice_args', $notice_args );

		$close_icon = 'manual' === $notice_args['dismiss_type'] ?
			sprintf(
				'<img class="notice-dismiss walkthecounty-notice-close" src="%s" />',
				esc_url( WALKTHECOUNTY_PLUGIN_URL . 'assets/dist/images/close.svg' )
		...
		// Note: we will remove walkthecounty_errors class in future.
		$error = sprintf(
			'<div class="walkthecounty_notices walkthecounty_errors" id="walkthecounty_error_%1$s">
						<p class="walkthecounty_notice walkthecounty_%1$s" data-dismissible="%2$s" data-dismiss-interval="%3$d" data-dismiss-type="%4$s">
							%5$s
						</p>
						%6$s
					</div>',
			...
			walkthecounty_clean( $notice_args['dismissible'] ),
			absint( $notice_args['dismiss_interval'] ),
			walkthecounty_clean( $notice_args['dismiss_type'] ),
			$message,
			$close_icon
	...
	public function print_admin_notices( $notice_args = array() ) {
		...
		$output    = '';
		$css_id    = ! empty( $notice_args['id'] ) ? $notice_args['id'] : uniqid( 'walkthecounty-inline-notice-' );
		$css_class = "notice-{$notice_args['notice_type']} walkthecounty-notice notice inline";
