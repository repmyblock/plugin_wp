 ...
 */
function walkthecounty_get_currency( $donation_or_form_id = null, $args = array() ) {

	// Get currency from donation
	if ( is_numeric( $donation_or_form_id ) && 'walkthecounty_payment' === get_post_type( $donation_or_form_id ) ) {
		$currency = walkthecounty_get_meta( $donation_or_form_id, '_walkthecounty_payment_currency', true );

		if ( empty( $currency ) ) {
			$currency = walkthecounty_get_option( 'currency', 'USD' );
		}
	} else {
		$currency = walkthecounty_get_option( 'currency', 'USD' );
	}
	 ...
	 * @since 1.0
	 */
	return apply_filters( 'walkthecounty_currency', $currency, $donation_or_form_id, $args );
}
 ...
 * @return string The currency code
 */
function walkthecounty_get_currency_position() {

	$currency_pos = walkthecounty_get_option( 'currency_position', 'before' );

	return apply_filters( 'walkthecounty_currency_position', $currency_pos );
}
 ...
 */
function walkthecounty_get_currencies_list() {
	$currencies = WalkTheCounty_Cache_Setting::get_option( 'currencies' );
	 ...
	 */
	return (array) apply_filters( 'walkthecounty_currencies', $currencies );
}
 ...
 * @return array $currencies A list of the available currencies
 */
function walkthecounty_get_currencies( $info = 'admin_label' ) {

	$currencies = walkthecounty_get_currencies_list();
 ...
 * @return array
 */
function walkthecounty_currency_symbols( $decode_currencies = false ) {
	$currencies = walkthecounty_get_currencies( 'symbol' );
	 ...
	 * @param array $currencies
	 */
	return apply_filters( 'walkthecounty_currency_symbols', $currencies );
}


/**
 * WalkTheCounty Currency Symbol
 *
 * WalkTheCountyn a currency determine the symbol to use. If no currency walkthecountyn, site default is used. If no symbol is determine,
 ...
 */
function walkthecounty_currency_symbol( $currency = '', $decode_currency = false ) {

	if ( empty( $currency ) ) {
		$currency = walkthecounty_get_currency();
	}

	$currencies = walkthecounty_currency_symbols( $decode_currency );
	 ...
	 */
	return apply_filters( 'walkthecounty_currency_symbol', $symbol, $currency );
}
 ...
 * @return string
 */
function walkthecounty_get_currency_name( $currency_code ) {
	$currency_name  = '';
	$currency_names = walkthecounty_get_currencies();
	 ...
	 * @param string $currency_code
	 */
	return apply_filters( 'walkthecounty_currency_name', $currency_name, $currency_code );
}
 ...
 * @return mixed|string
 */
function walkthecounty_currency_filter( $price = '', $args = array() ) {
	...
	if ( isset( $func_args[1] ) && is_string( $func_args[1] ) ) {
		...
		walkthecounty_doing_it_wrong( __FUNCTION__, 'Pass second argument as Array.', WALKTHECOUNTY_VERSION );
	}
	...
	if ( empty( $args['currency_code'] ) || ! array_key_exists( (string) $args['currency_code'], walkthecounty_get_currencies() ) ) {
		$args['currency_code'] = walkthecounty_get_currency( $args['form_id'] );
	}

	$args['position'] = walkthecounty_get_option( 'currency_position', 'before' );
	...
	$args['symbol'] = walkthecounty_currency_symbol( $args['currency_code'], $args['decode_currency'] );
	 ...
	 * @since 1.8.17
	 */
	$formatted = apply_filters( 'walkthecounty_currency_filter', $formatted, $args, $price );
	 ...
	 *           filter name will be walkthecounty_usd_currency_filter_before
	 *
	 *           and if currency is USD and currency position is after then
	 *           filter name will be walkthecounty_usd_currency_filter_after
	 */
	$formatted = apply_filters(
		'walkthecounty_' . strtolower( $args['currency_code'] ) . "_currency_filter_{$args['position']}",
		$formatted,
		$args['currency_code'],
		$price,
		$args
 ...
 * @return array
 */
function walkthecounty_get_zero_based_currencies() {
	 ...
	 * @since 2.3.0
	 */
	return apply_filters( 'walkthecounty_get_zero_based_currencies', $zero_based_currencies );
}
 ...
 * @see   https://github.com/impress-org/walkthecounty/issues/2191
 *
 * @param string $currency Currency code
 *
 * @return bool
 */
function walkthecounty_is_zero_based_currency( $currency = '' ) {

	$zero_based_currency = walkthecounty_get_zero_based_currencies();
 ...
 * @return bool
 */
function walkthecounty_is_right_to_left_supported_currency( $currency = '' ) {
	$zero_based_currency = apply_filters(
		'walkthecounty_right_to_left_supported_currency',
		array(
			'IRR',
			'RIAL',
			'MAD',
