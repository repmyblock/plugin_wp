...
if ( ! class_exists( 'WalkTheCounty_License' ) ) :

	/**
	 * WalkTheCounty_License Class
	 *
	 * This class simplifies the process of adding license information
	 * to new WalkTheCounty add-ons.
	 *
	 * @since 1.0
	 */
	class WalkTheCounty_License {
		 ...
		 * @var    string
		 */
		private static $site_url = 'https://walkthecountywp.com/';
		 ...
		 * @var    string
		 */
		private static $api_url = 'https://walkthecountywp.com/edd-sl-api/';
		 ...
		 * @var null|string
		 */
		private static $account_url = 'https://walkthecountywp.com/my-account/';
		 ...
		 * @var null|string
		 */
		private static $downloads_url = 'https://walkthecountywp.com/my-downloads/';
		 ...
		 * @var null|string
		 */
		private static $checkout_url = 'https://walkthecountywp.com/checkout/';

		/**
		 * Class Constructor
		 *
		 * Set up the WalkTheCounty License Class.
		 *
		 * @access public
		 *
		 * @param string $_file
		...
		public static function get_short_name( $plugin_name ) {
			$plugin_name = trim( str_replace( 'WalkTheCounty - ', '', $plugin_name ) );
			$plugin_name = 'walkthecounty_' . preg_replace( '/[^a-zA-Z0-9_\s]/', '', str_replace( ' ', '_', strtolower( $plugin_name ) ) );
		...
		public function get_license_info( $edd_action = '', $response_in_array = false ) {
			...
			walkthecounty_doing_it_wrong( __FUNCTION__, 'Use self::request_license_api instead', '2.5.0' );

			// Data to send to the API.
			$api_params = array(
				'edd_action' => $edd_action, // never change from "edd_" to "walkthecounty_"!
		...
		static function is_subscription( $license_key = '' ) {
			...
			$subscriptions = get_option( 'walkthecounty_subscriptions' );
			$subscription  = array();
		...
		public static function request_license_api( $api_params = array(), $response_in_array = false ) {
			...
			if ( empty( $api_params['edd_action'] ) ) {
				return new WP_Error( 'walkthecounty-invalid-edd-action', __( 'Valid edd_action not defined', 'walkthecounty' ) );
			}
			...
			$response = wp_remote_post(
				self::$api_url,
				apply_filters(
					'walkthecounty_request_license_api_args',
					array(
						'timeout'   => 15,
						'sslverify' => false,
						'body'      => $api_params,
		...
		public static function get_license_by_plugin_dirname( $plugin_dirname ) {
			...
			if ( ! empty( $walkthecounty_licenses ) ) {
				foreach ( $walkthecounty_licenses as $walkthecounty_license ) {

					// Logic to match all access pass license to add-on.
					$compares = $walkthecounty_license['is_all_access_pass']
						? $walkthecounty_license['download']
						...
						: array( array( 'plugin_slug' => ! empty( $walkthecounty_license['plugin_slug'] ) ? $walkthecounty_license['plugin_slug'] : '' ) );

					foreach ( $compares as $compare ) {
						if ( ! empty( $compare['plugin_slug'] ) && $plugin_dirname === $compare['plugin_slug'] ) {
							$license = $walkthecounty_license;
							break;
						}
					}
				}
		...
		public static function get_plugin_by_slug( $plugin_slug ) {
			$walkthecounty_plugins   = walkthecounty_get_plugins();
			$matching_list  = wp_list_pluck( $walkthecounty_plugins, 'Dir', 'Path' );
			$is_match_found = array_search( $plugin_slug, $matching_list, true );

			return $is_match_found ? $walkthecounty_plugins[ $is_match_found ] : array();
		}
		...
		public static function build_plugin_name_from_slug( $plugin_slug ) {
			$plugin_name = str_replace( array( '-', 'walkthecounty ' ), array( ' ', 'WalkTheCounty - ' ), $plugin_slug );
		...
		public static function render_licenses_list() {
			...
			$walkthecounty_licenses          = get_option( 'walkthecounty_licenses', array() );
			$licenses_without_addon = $walkthecounty_licenses;
			...
			foreach ( $walkthecounty_licenses as $key => $walkthecounty_license ) {
				if ( $walkthecounty_license['is_all_access_pass'] ) {
					$all_access_pass_licenses[ $key ] = $walkthecounty_license;

					unset( $licenses_without_addon[$key] );

					foreach ( $walkthecounty_license['download'] as $download ) {
			...
			if( ! empty( $walkthecounty_plugins ) ) {
				foreach ( $walkthecounty_plugins as $walkthecounty_plugin ) {
					if ( in_array( $walkthecounty_plugin['Dir'], $all_access_pass_addon_list ) ) {
						continue;
					}

					$addon_license = self::get_license_by_plugin_dirname( $walkthecounty_plugin['Dir'] );
					...
					$html["{$html_arr_key}"] .= self::html_by_plugin( $walkthecounty_plugin );
				}
			}
		...
		public static function html_by_plugin( $plugin ) {
			...
			$plugin['License'] = $license = wp_parse_args(
				$license, array(
					'item_name'     => str_replace( 'walkthecounty-', '', $plugin['Dir'] ),
			...
			?>
			<div class="walkthecounty-addon-wrap">
				<div class="walkthecounty-addon-inner">
			...
			<div class="walkthecounty-addon-wrap">
				<div class="walkthecounty-addon-inner">
					...
					foreach ( $addons as $addon ) {
						...
						$plugin['Name'] = false !== strpos( $plugin['Name'], 'WalkTheCounty' )
							? $plugin['Name']
		...
		private static function html_license_row( $license, $plugin = array() ) {
			...
			?>
			<div class="walkthecounty-license-row walkthecounty-clearfix">
				<div class="walkthecounty-license-notice-container"></div>
				<div class="walkthecounty-license-top walkthecounty-clearfix">

					<div class="walkthecounty-license-top-column walkthecounty-license-key-field-wrap">

						<div class="walkthecounty-license__key<?php echo $license_key ? ' walkthecounty-has-license-key' : ''; ?>">
							<?php $value = $license_key ? walkthecounty_hide_char( $license['license_key'], 5 ) : ''; ?>
							<label for="walkthecounty-license-addon-key-field" class="walkthecounty-license-top-header"><?php _e( 'License Key', 'walkthecounty' ); ?></label>
							<input id="walkthecounty-license-addon-key-field" type="text" autocomplete="off" value="<?php echo $value; ?>"<?php echo $value ? ' readonly' : ''; ?>>
							<?php if ( ! $license_key ) : ?>
								<button class="walkthecounty-button__license-activate button-primary" data-addon="<?php echo $addon_dir; ?>">
									<?php _e( 'Activate', 'walkthecounty' ); ?>
								</button>
							<?php elseif ( $license_is_inactive ): ?>
								<button class="walkthecounty-button__license-reactivate button-primary" data-addon="<?php echo $addon_dir; ?>" data-license="<?php echo $license['license_key'] ?>">
									<?php _e( 'Reactivate', 'walkthecounty' ); ?>
								</button>
							<?php else : ?>

								<?php
								echo sprintf(
									'<button class="walkthecounty-license__deactivate button button-secondary" data-license-key="%2$s" data-item-name="%3$s"  data-plugin-dirname="%5$s" data-nonce="%4$s">%1$s</button>',
									__( 'Deactivate', 'walkthecounty' ),
									$license['license_key'],
									$license['item_name'],
									wp_create_nonce( "walkthecounty-deactivate-license-{$license['item_name']}" ),
									! empty( $license['plugin_slug'] ) ? $license['plugin_slug'] : ''
								); ?>

							<?php endif; ?>

							<div class="walkthecounty-license__status">
								<?php
								echo $license_key && ! $license_is_inactive
									? sprintf(
										'<span class="dashicons dashicons-%2$s"></span>&nbsp;%1$s',
										$is_license_expired
											? __( 'License is expired. Please activate your license key.', 'walkthecounty' )
											: __( 'License is active and you are receiving updates and support.', 'walkthecounty' ),
										$is_license_expired
											? 'no'
											: 'yes'
									)
									: sprintf(
										...
										__( 'License is inactive.', 'walkthecounty' ),
										$license_is_inactive
											? sprintf(
												__( 'Please <a href="%1$s" target="_blank">Visit your dashboard</a> to check this license details and activate this license to receive updates and support.', 'walkthecounty' ),
												self::get_account_url()
											)
											: __( 'Please activate your license key.', 'walkthecounty' )
									);
								?>
							</div>
						</div>
					...
					<div class="walkthecounty-license-top-column walkthecounty-license-info-field-wrap">
						<h3 class="walkthecounty-license-top-header"><?php _e( 'License Information', 'walkthecounty' ); ?></h3>
						<?php
						...
						if ( $license_key ) : ?>
							...
							<?php if ( $license_key ) : ?>
								<?php
								echo sprintf(
									'<p class="walkthecounty-license-renewal-date"><span class="dashicons dashicons-calendar-alt"></span> <strong>%1$s</strong> %2$s</p>',
									$is_license_expired ? __( 'Expired:' ) : __( 'Renews:' ),
									date( walkthecounty_date_format(), $expires_timestamp )
								);
								?>
							<?php endif; ?>
							...
							<?php if ( $is_license_expired ) {
								...
								echo sprintf(
									...
									$license['renew_url'],
									__( 'Renew to manage sites', 'walkthecounty' )
								);
							} elseif ( $license_key ) {
								if ( ! $license['activations_left'] ) {
									echo sprintf(
										'<span class="walkthecounty-license-activations-left">%1$s</span>',
										__( 'No activation remaining', 'walkthecounty' )
									);
								} else {
									echo sprintf(
										'<span class="walkthecounty-license-activations-left"><span class="walkthecounty-license-activations-remaining-icon wp-ui-highlight">%1$s</span> %2$s</span>',
										$license['activations_left'],
										_n( 'Activation Remaining', 'Activations Remaining', $license['activations_left'], 'walkthecounty' )
									);
								}
							} ?>
						<?php else : ?>

							<p class="walkthecounty-field-description"><?php _e( 'This is an unlicensed add-on and is not receiving updates or support. Please activate your license key to fix the issue.', 'walkthecounty' ); ?></p>

						<?php endif; ?>
					</div>

					<div class="walkthecounty-license-top-column">
						<h3 class="walkthecounty-license-top-header"><?php _e( 'License Actions', 'walkthecounty' ); ?></h3>
						...
						if ( ! $license_key ) : ?>
							<?php
							echo sprintf(
								'<a class="walkthecounty-button button-secondary" href="%1$s" target="_blank">%2$s</a>',
								$license['purchase_link'],
								__( 'Purchase License', 'walkthecounty' )
							);
							?>
						<?php endif; ?>

						<?php
						// Manage license link on WalkTheCountyWP.com
						if ( ! $is_license_expired && $license_key ) {
							echo sprintf(
								'<a href="%1$spurchase-history/?license_id=%2$s&action=manage_licenses&payment_id=%3$s" target="_blank" class="walkthecounty-license-action-link">%4$s</a>',
								trailingslashit( WalkTheCounty_License::get_website_url() ),
								$license['license_id'],
								$license['payment_id'],
								__( 'Manage License', 'walkthecounty' )
							);
							echo sprintf(
								'<a href="%1$spriority-support/" target="_blank" class="walkthecounty-license-action-link">%2$s</a>',
								trailingslashit( WalkTheCounty_License::get_website_url() ),
								__( 'Access Support', 'walkthecounty' )
							);
						}
						?>
		...
		public static function html_plugin_row( $plugin ) {
			...
			ob_start();
			?>
			<div class="walkthecounty-addon-info-wrap walkthecounty-clearfix">

				<div class="walkthecounty-addon-info-left">
					<span class="walkthecounty-addon-name">
						<?php echo $plugin['Name']; ?>
					</span>
					<span class="walkthecounty-addon-version">
						<?php echo sprintf( '%1$s %2$s', __( 'Version' ), $plugin['Version'] ); ?>
					</span>
				</div>

				<div class="walkthecounty-addon-info-right">
					<?php
					echo sprintf(
						'<a href="%1$s" class="walkthecounty-ajax-modal walkthecounty-addon-view-changelog" title="%3$s">%2$s</a>',
						walkthecounty_modal_ajax_url(
							array(
								'url'            => filter_var( $plugin['ChangeLogSlug'], FILTER_VALIDATE_URL )
									? urldecode_deep( $plugin['ChangeLogSlug'] )
									: urlencode_deep( walkthecounty_get_addon_readme_url( $plugin['ChangeLogSlug'] ) ),
								'show_changelog' => 1,
							)
						),
						__( 'View Changelog', 'walkthecounty' ),
					...
					if ( in_array( $plugin['Status'], array( 'active', 'inactive' ) ) ) {
						echo sprintf(
							'<span class="walkthecounty-addon-activation-status walkthecounty-addon-activation-status__%1$s">%1$s</span>',
							'active' === $plugin['Status'] ? __( 'activated', 'walkthecounty' ) : __( 'installed', 'walkthecounty' )
						);
					}

					printf(
						'<%3$s class="walkthecounty-button button button-secondary button-small" href="%1$s"%4$s><span class="dashicons dashicons-download"></span>%2$s</%3$s>',
						$plugin['DownloadURL'],
						__( 'Download', 'walkthecounty' ),
		...
		public static function refresh_license_status() {
			return get_option(
				'walkthecounty_licenses_refreshed_last_checked',
				array(
					'compare' => date( 'Ymd' ),
