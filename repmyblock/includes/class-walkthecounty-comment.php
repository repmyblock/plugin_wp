...
class WalkTheCounty_Comment {
	 ...
	 * @since  2.3.0
	 * @access public
	 * @var WalkTheCounty_DB_Comment_Meta
	 */
	public $db_meta;
	...
	private function init() {
		$this->db      = new WalkTheCounty_DB_Comments();
		$this->db_meta = new WalkTheCounty_DB_Comment_Meta();
	...
	public static function add( $comment_args = array() ) {
		 ...
		 */
		do_action(
			"walkthecounty_pre_{$action_type}_{$comment_args['comment_type']}_note",
		 ...
		 */
		do_action(
			"walkthecounty_{$action_type}_{$comment_args['comment_type']}_note",
	...
	public static function delete( $comment_id ) {
		...
		$comment = WalkTheCounty()->comment->db->get_by( 'comment_ID', $comment_id );
		 ...
		 * @since 1.0
		 */
		do_action( "walkthecounty_pre_delete_{$comment_type}_note", $comment_id, $comment_parent );

		$ret = WalkTheCounty()->comment->db->delete( $comment_id );

		// Delete comment meta.
		WalkTheCounty()->comment->db_meta->delete_all_meta( $comment_id );
		 ...
		 * @since 1.0
		 */
		do_action( "walkthecounty_post_delete_{$comment_type}_note", $comment_id, $comment_parent, $ret );
	...
	public static function get( $comment_args ) {
		...
		$comments = $wpdb->get_results( WalkTheCounty()->comment->db->get_sql( $comment_args ) );
	...
	/**
	 * Remove WalkTheCounty Comments from the wp_count_comments function
	 *
	 * @since  2.2.0
	 * @access public
	 *
	...
	public function remove_comments_from_comment_counts( $stats, $post_id ) {
		...
		if ( apply_filters( 'walkthecounty_count_payment_notes_in_comments', false ) ) {
			return $stats;
		}

		$stats = WalkTheCounty_Cache::get_group( "comments-{$post_id}", 'counts' );
		...
		WalkTheCounty_Cache::set_group( "comments-{$post_id}", $stats, 'counts' );
	...
	public function __get_comment_author( $author, $comment_id, $comment ) {
		if ( in_array( $comment->comment_type, $this->comment_types ) ) {
			switch ( $comment->comment_type ) {
				case 'walkthecounty_payment_note':
					if ( get_comment_meta( $comment_id, '_walkthecounty_donor_id', true ) ) {
						$author = walkthecounty_get_donor_name_by( $comment->comment_post_ID );
					}
			}
		}
	...
	public static function get_comment_types( $comment_types ) {
		...
		foreach ( $comment_types as $comment_type ) {
			$_comment_types[] = "walkthecounty_{$comment_type}_note";
		}
			...
			switch ( $comment_type ) {
				case 'payment':
					...
					$comments = get_comments( wp_parse_args(
						...
						array(
							...
							'order'   => 'ASC',
							'search'  => $search,
							'type'    => 'walkthecounty_payment_note',
						)
					) );
					break;
				...
				case 'donor':
					$comment_args['meta_query'] = ! empty( $comment_args['meta_query'] )
						...
						: array(
							array(
								'key'   => "_walkthecounty_{$comment_type}_id",
								'value' => $id,
							),
						);
	...
	private static function _bc_add( $comment_args = array() ) {
		...
		if ( ! walkthecounty_has_upgrade_completed( 'v230_move_donor_note' ) ) {
			...
			if ( empty( $id ) || empty( $note ) || empty( $comment_type ) ) {
				return new WP_Error( 'walkthecounty_invalid_required_param', __( 'This comment has invalid ID or comment text or comment type', 'walkthecounty' ) );
			}
			 ...
			 * @since 1.0
			 */
			do_action( "walkthecounty_pre_{$action_type}_{$comment_type}_note", $id, $note );
			...
			$comment_args = wp_parse_args(
				...
				array(
					...
					'comment_type'         => "walkthecounty_{$comment_type}_note",
			...
			update_comment_meta( $comment_id, "_walkthecounty_{$comment_type}_id", $id );
			 ...
			 * @since 1.0
			 */
			do_action( "walkthecounty_{$action_type}_{$comment_type}_note", $comment_id, $id, $note );
		}
	...
	private static function _bc_delete( $comment_args ) {
		...
		if ( ! walkthecounty_has_upgrade_completed( 'v230_move_donor_note' ) ) {
			 ...
			 * @since 1.0
			 */
			do_action( "walkthecounty_pre_delete_{$comment_type}_note", $comment_id, $id );
			 ...
			 * @since 1.0
			 */
			do_action( "walkthecounty_post_delete_{$comment_type}_note", $comment_id, $id, $ret );
		}
