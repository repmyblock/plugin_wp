 ...
 * @copyright   Copyright (c) 2017, WalkTheCounty
 * @license     https://opensource.org/licenses/gpl-license GNU Public License
 * @since       2.0
 */
class WalkTheCounty_Translations {
	/**
	 * Instance.
	 *
	 * @since  2.0
	...
	public function load_translated_texts() {
		...
		self::$text_translations = apply_filters(
			'walkthecounty_translated_texts',
	...
	public static function add_text( $args = array() ) {
		...
		try {
			...
			if ( empty( $args['text'] ) ) {
				/* @var WP_Error $error */
				$error = new WP_Error( 'EMPTY_TEXT', __( 'Empty string is not allowed.', 'walkthecounty' ), $args );
				throw new Exception( $error->get_error_message( 'EMPTY_TEXT' ) );

			} elseif ( empty( $args['id'] ) ) {
				/* @var WP_Error $error */
				$error = new WP_Error( 'EMPTY_ID', __( 'Empty ID is not allowed.', 'walkthecounty' ), $args );
				...
				/* @var WP_Error $error */
				$error = new WP_Error( 'TEXT_ID_ALREADY_EXIST', __( 'Text ID without a group already exists.', 'walkthecounty' ), $args );
				...
				/* @var WP_Error $error */
				$error = new WP_Error( 'TEXT_ID_WITHIN_GROUP_ALREADY_EXIST', __( 'Text ID within a group already exists.', 'walkthecounty' ), $args );
		 ...
		 * @since 2.0
		 */
		self::$text_configs = apply_filters( 'walkthecounty_texts', self::$text_configs );
	...
	public static function get_text( $args = array() ) {
		...
		/**
		 * Filter the walkthecounty text
		 *
		 * @since 2.0
		 */
		$text = apply_filters( 'walkthecounty_text', $text, $args, self::$text_configs, self::$text_translations );
...
// Setup translations.
WalkTheCounty_Translations::get_instance()->setup();
