 ...
 * @subpackage  Classes/WalkTheCounty_Donor
...
/**
 * WalkTheCounty_Donor Class
 *
 * This class handles customers.
 *
 * @since 1.0
 */
class WalkTheCounty_Donor {
	 ...
	 * @access protected
	 *
	 * @var    WalkTheCounty_DB_Donors
	 */
	protected $db;

	/**
	 * WalkTheCounty_Donor constructor.
	 *
	...
	private function setup_donor( $donor ) {
		...
		// Get cached donors.
		$donor_vars = WalkTheCounty_Cache::get_group( $donor->id, 'walkthecounty-donors' );
		...
		if ( is_null( $donor_vars ) ) {
			foreach ( $donor as $key => $value ) {
				...
				switch ( $key ) {
					...
					// https://github.com/impress-org/walkthecounty/issues/3632
					case 'notes':
						break;
			...
			WalkTheCounty_Cache::set_group( $donor->id, get_object_vars( $this ), 'walkthecounty-donors' );
		} else {
	...
	public function setup_address() {
		...
		$addresses = ! empty( $addresses )
			...
			: $wpdb->get_results( $wpdb->prepare( "
				...
				LIKE '%%%s%%'
				AND {$meta_type}_id=%d
				", 'walkthecounty_donor_address', $this->id ), ARRAY_N );
		...
		foreach ( $addresses as $address ) {
			$address[0] = str_replace( '_walkthecounty_donor_address_', '', $address[0] );
	...
	private function get_addresses_from_meta_cache() {
		...
		if ( ! empty( $meta ) ) {
			foreach ( $meta as $meta_key => $meta_value ) {
				if ( false === strpos( $meta_key, 'walkthecounty_donor_address' ) ) {
					continue;
				}
	...
	public function get_donor_address( $args = array() ) {
		...
		if ( ! walkthecounty_has_upgrade_completed( 'v20_upgrades_user_address' ) ) {

			// Backward compatibility for user id param.
			return wp_parse_args( (array) get_user_meta( $this->user_id, '_walkthecounty_user_address', true ), $default_address );
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'walkthecounty-donor-invalid-property', sprintf( esc_html__( 'Can\'t get property %s.', 'walkthecounty' ), $key ) );
	...
	public function create( $data = array() ) {
		 ...
		 */
		do_action( 'walkthecounty_donor_pre_create', $args );
		 ...
		 */
		do_action( 'walkthecounty_donor_post_create', $created, $args );
	...
	public function update( $data = array() ) {
		 ...
		 */
		do_action( 'walkthecounty_donor_pre_update', $this->id, $data );
		 ...
		 * @param array $data     Donor attributes.
		 */
		do_action( 'walkthecounty_donor_post_update', $updated, $this->id, $data );
	...
	public function attach_payment( $payment_id = 0, $update_stats = true ) {
		 ...
		 * @param int $donor_id   Donor id.
		 */
		do_action( 'walkthecounty_donor_pre_attach_payment', $payment_id, $this->id );
		...
		if ( $payment_added ) {
			...
			if ( $update_stats ) {
				$payment_amount = walkthecounty_donation_amount( $payment_id, array( 'type' => 'stats' ) );
		 ...
		 * @param int  $donor_id      Donor id.
		 */
		do_action( 'walkthecounty_donor_post_attach_payment', $payment_added, $payment_id, $this->id );
	...
	public function remove_payment( $payment_id = 0, $update_stats = true ) {
		 ...
		 */
		do_action( 'walkthecounty_donor_pre_remove_payment', $payment_id, $this->id );
		 ...
		 * @param int  $donor_id        Donor id.
		 */
		do_action( 'walkthecounty_donor_post_remove_payment', $payment_removed, $payment_id, $this->id );
	...
	public function increase_purchase_count( $count = 1 ) {
		 ...
		 * @param int $donor_id Donor id.
		 */
		do_action( 'walkthecounty_donor_pre_increase_donation_count', $count, $this->id );
		 ...
		 * @param int $donor_id       Donor id.
		 */
		do_action( 'walkthecounty_donor_post_increase_donation_count', $this->purchase_count, $count, $this->id );
	...
	public function decrease_donation_count( $count = 1 ) {
		 ...
		 * @param int $donor_id Customer id.
		 */
		do_action( 'walkthecounty_donor_pre_decrease_donation_count', $count, $this->id );
		 ...
		 * @param int $donor_id       Donor id.
		 */
		do_action( 'walkthecounty_donor_post_decrease_donation_count', $this->purchase_count, $count, $this->id );
	...
	public function increase_value( $value = 0.00 ) {
		 ...
		 * @param int   $donor_id Customer id.
		 */
		do_action( 'walkthecounty_donor_pre_increase_value', $value, $this->id );
		 ...
		 * @param int   $donor_id       Donor id.
		 */
		do_action( 'walkthecounty_donor_post_increase_value', $this->purchase_value, $value, $this->id );
	...
	public function decrease_value( $value = 0.00 ) {
		 ...
		 * @param int   $donor_id Donor id.
		 */
		do_action( 'walkthecounty_donor_pre_decrease_value', $value, $this->id );
		 ...
		 * @param int   $donor_id       Donor id.
		 */
		do_action( 'walkthecounty_donor_post_decrease_value', $this->purchase_value, $value, $this->id );
	...
	public function get_total_donation_amount( $args = array() ) {
		 ...
		 */
		return apply_filters( 'walkthecounty_get_total_donation_amount', $this->purchase_value, $this->id, $args );
	}
	...
	public function add_note( $note = '' ) {
		...
		$formatted_new_note = apply_filters( 'walkthecounty_customer_add_note_string', $note_string );
		$notes              .= "\n\n" . $formatted_new_note;
		 ...
		 * @param int    $donor_id           Donor id.
		 */
		do_action( 'walkthecounty_donor_pre_add_note', $formatted_new_note, $this->id );

		if ( ! walkthecounty_has_upgrade_completed( 'v230_move_donor_note' ) ) {
			// Backward compatibility.
			$updated = $this->update( array( 'notes' => $notes ) );
		} else {
			$updated = WalkTheCounty()->comment->db->add(
				array(
		 ...
		 * @param int    $donor_id           Donor id.
		 */
		do_action( 'walkthecounty_donor_post_add_note', $this->notes, $formatted_new_note, $this->id );
	...
	private function get_raw_notes() {
		$all_notes = '';
		$comments = WalkTheCounty()->comment->db->get_results_by( array( 'comment_parent' => $this->id ) );
	...
	public function get_meta( $meta_key = '', $single = true ) {
		return WalkTheCounty()->donor_meta->get_meta( $this->id, $meta_key, $single );
	}
	...
	public function add_meta( $meta_key = '', $meta_value, $unique = false ) {
		return WalkTheCounty()->donor_meta->add_meta( $this->id, $meta_key, $meta_value, $unique );
	}
	...
	public function update_meta( $meta_key = '', $meta_value, $prev_value = '' ) {
		return WalkTheCounty()->donor_meta->update_meta( $this->id, $meta_key, $meta_value, $prev_value );
	}
	...
	public function delete_meta( $meta_key = '', $meta_value = '' ) {
		return WalkTheCounty()->donor_meta->delete_meta( $this->id, $meta_key, $meta_value );
	}
	...
	private function sanitize_columns( $data ) {
		...
		foreach ( $columns as $key => $type ) {
			...
			switch ( $type ) {
				...
				case '%f':
					// Convert what was walkthecountyn to a float
	...
	public function add_email( $email = '', $primary = false ) {
		...
		$existing = new WalkTheCounty_Donor( $email );
		...
		do_action( 'walkthecounty_donor_pre_add_email', $email, $this->id, $this );

		// Add is used to ensure duplicate emails are not added
		$ret = (bool) $this->add_meta( 'additional_email', $email );

		do_action( 'walkthecounty_donor_post_add_email', $email, $this->id, $this );
	...
	public function remove_email( $email = '' ) {
		...
		do_action( 'walkthecounty_donor_pre_remove_email', $email, $this->id, $this );

		$ret = (bool) $this->delete_meta( 'additional_email', $email );

		do_action( 'walkthecounty_donor_post_remove_email', $email, $this->id, $this );
	...
	public function set_primary_email( $new_primary_email = '' ) {
		...
		do_action( 'walkthecounty_donor_pre_set_primary_email', $new_primary_email, $this->id, $this );

		$existing = new WalkTheCounty_Donor( $new_primary_email );
		...
		do_action( 'walkthecounty_donor_post_set_primary_email', $new_primary_email, $this->id, $this );
	...
	public function add_address( $address_type, $address ) {
		...
		// Set meta key prefix.
		global $wpdb;
		$meta_key_prefix = "_walkthecounty_donor_address_{$address_type}_{address_name}";
		$meta_type       = WalkTheCounty()->donor_meta->meta_type;
		...
		if ( $is_multi_address ) {
			if ( is_null( $multi_address_id ) ) {
				...
				$multi_address_id = $wpdb->get_var( $wpdb->prepare( "
						...
						LIMIT 1
						", "_walkthecounty_donor_address_{$address_type}_line1", $this->id ) );
			...
			$meta_key_prefix = "_walkthecounty_donor_address_{$address_type}_{address_name}_{$multi_address_id}";
		}

		// Save donor address.
		foreach ( $address as $type => $value ) {
			$meta_key = str_replace( '{address_name}', $type, $meta_key_prefix );
			WalkTheCounty()->donor_meta->update_meta( $this->id, $meta_key, $value );
		}
	...
	public function remove_address( $address_id ) {
		...
		// Set meta key prefix.
		$meta_key_prefix = "_walkthecounty_donor_address_{$address_type}_%";
		...
		// Delete cache.
		WalkTheCounty_Cache::delete_group( $this->id, 'walkthecounty-donors' );
	...
	public function split_donor_name( $id ) {
		...
		$donor      = new WalkTheCounty_Donor( $id );
	...
	public function get_first_name() {
		$first_name = $this->get_meta( '_walkthecounty_donor_first_name' );
		if ( ! $first_name ) {
	...
	public function get_last_name() {
		$first_name = $this->get_meta( '_walkthecounty_donor_first_name' );
		$last_name  = $this->get_meta( '_walkthecounty_donor_last_name' );
	...
	public function get_company_name() {
		$company_name = $this->get_meta( '_walkthecounty_donor_company' );
	...
	public function get_last_donation_date( $formatted = false ) {
		...
		$completed_data = walkthecounty_get_payment_completed_date( $last_donation );

		if ( $formatted ) {
			return date_i18n( walkthecounty_date_format(), strtotime( $completed_data ) );
		}
	...
	public function get_donor_initals() {
		...
		return apply_filters(
			'get_donor_initals',
			walkthecounty_get_name_initial( array(
