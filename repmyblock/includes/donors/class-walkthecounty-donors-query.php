...
/**
 * WalkTheCounty_Donors_Query Class
 *
 ...
 * @since 1.8.14
 */
class WalkTheCounty_Donors_Query {

	/**
	 * The args to pass to the walkthecounty_get_donors() query
	 *
	 * @since  1.8.14
	 * @access public
	 *
	...
	public function __construct( $args = array() ) {
		...
		$this->table_name      = WalkTheCounty()->donors->table_name;
		$this->meta_table_name = WalkTheCounty()->donor_meta->table_name;
		$this->meta_type       = WalkTheCounty()->donor_meta->meta_type;
	...
	public function get_donors() {
		 ...
		 * @since 1.8.14
		 *
		 * @param WalkTheCounty_Donors_Query $this Donors query object.
		 */
		do_action( 'walkthecounty_pre_get_donors', $this );

		$cache_key = WalkTheCounty_Cache::get_key( 'walkthecounty_donor', $this->get_sql(), false );

		// Get donors from cache.
		$this->donors = WalkTheCounty_Cache::get_db_query( $cache_key );
		...
		if ( is_null( $this->donors ) ) {
			...
			WalkTheCounty_Cache::set_db_query( $cache_key, $this->donors );
		}
		 ...
		 * @param WalkTheCounty_Donors_Query $this Donors query object.
		 */
		do_action( 'walkthecounty_post_get_donors', $this );
	...
	private function get_where_query() {
		...
		$where[] = $this->get_where_walkthecounty_forms();
	...
	private function get_order_query() {
		$table_columns = WalkTheCounty()->donors->get_columns();
	...
	/**
	 * Set walkthecounty_forms where clause.
	 *
	 * @todo   : add phpunit test
	 *
	 * @since  2.1.0
	 ...
	 * @return string
	 */
	private function get_where_walkthecounty_forms() {
		global $wpdb;
		$where = '';

		if ( ! empty( $this->args['walkthecounty_forms'] ) ) {
			if ( ! is_array( $this->args['walkthecounty_forms'] ) ) {
				$this->args['walkthecounty_forms'] = explode( ',', $this->args['walkthecounty_forms'] );
			}

			$form_ids        = implode( ',', array_map( 'intval', $this->args['walkthecounty_forms'] ) );
	...
	public static function update_meta_cache( $donor_ids ){
		...
		update_meta_cache( WalkTheCounty()->donor_meta->get_meta_type(), $donor_ids );
	}
