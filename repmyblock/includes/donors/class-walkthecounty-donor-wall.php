 ...
 * @subpackage  Classes/WalkTheCounty_Donors_Gravatars
...
/**
 * WalkTheCounty_Donor_Wall Class
 *
 * This class handles donors.
 *
 * @since 2.2.0
 */
class WalkTheCounty_Donor_Wall {
	 ...
	 * @since  2.2.0
	 * @access public
	 * @return WalkTheCounty_Donor_Wall
	 */
	...
	public function setup_actions() {
		...
		add_shortcode( 'walkthecounty_donor_wall', array( $this, 'render_shortcode' ) );

		add_action( 'wp_ajax_walkthecounty_get_donor_comments', array( $this, 'ajax_handler' ) );
		add_action( 'wp_ajax_nopriv_walkthecounty_get_donor_comments', array( $this, 'ajax_handler' ) );
	...
	public function render_shortcode( $atts ) {
		...
		if ( $donations ) {
			...
			foreach ( $donations as $donation ) {
				// WalkTheCounty/templates/shortcode-donor-wall.php.
				walkthecounty_get_template( 'shortcode-donor-wall', array( $donation, $walkthecounty_settings, $atts ) );
			}
		...
		$more_btn_html = sprintf(
			'<input type="hidden" class="walkthecounty-donor-wall-shortcode-attrs" data-shortcode="%1$s">',
			rawurlencode( http_build_query( $atts ) )
		);

		if ( $this->has_donations( $temp_atts ) ) {
			$more_btn_html .= sprintf(
				'<button class="walkthecounty-donor__load_more walkthecounty-button-with-loader"><span class="walkthecounty-loading-animation"></span>%1$s</button>',
				$atts['loadmore_text']
			);
		}
		...
		$html = $html
			? sprintf(
				'<div class="walkthecounty-wrap walkthecounty-grid-ie-utility"><div class="walkthecounty-grid walkthecounty-grid--%1$s">%2$s</div>%3$s</div>',
	...
	public function parse_atts( $atts ) {
		$atts = shortcode_atts(
			array(
				...
				'comment_length'  => 140,
				'only_comments'   => false,
				'readmore_text'   => esc_html__( 'Read more', 'walkthecounty' ),
				'loadmore_text'   => esc_html__( 'Load more', 'walkthecounty' ),
				'avatar_size'     => 60,
				'orderby'         => 'post_date',
				'order'           => 'DESC',
	...
	public function get_donors( $donor_query ) {
		$donor_query = new WalkTheCounty_Donors_Query( $donor_query );
	...
	public function ajax_handler() {
		$shortcode_atts = wp_parse_args( walkthecounty_clean( rawurldecode( $_POST['data'] ) ) ); // @codingStandardsIgnoreLine
		...
		if ( ! empty( $results ) ) {
			...
			if ( ! empty( $temp ) ) {
				foreach ( $temp as $donation_id => $donation_data ) {
					...
					$temp[ $donation_id ]['name_initial'] = walkthecounty_get_name_initial( array(
						'firstname' => $donation_data['_walkthecounty_donor_billing_first_name'],
						'lastname'  => $donation_data['_walkthecounty_donor_billing_last_name'],
					) );
	...
	private function get_donations( $atts = array() ) {
		...
		$where = " WHERE p1.post_status IN ('publish') AND p1.post_type = 'walkthecounty_payment'";

		// exclude donation with zero amount from result.
		$sql   .= " INNER JOIN {$wpdb->donationmeta} as m1 ON (p1.ID = m1.{$donation_id_col})";
		$where .= " AND m1.meta_key='_walkthecounty_payment_total' AND m1.meta_value>0";


		if ( $query_params['form_id'] ) {
			$sql   .= " INNER JOIN {$wpdb->donationmeta} as m2 ON (p1.ID = m2.{$donation_id_col})";
			$where .= " AND m2.meta_key='_walkthecounty_payment_form_id' AND m2.meta_value={$query_params['form_id']}";
		}

		// exclude donations which does not has donor comment.
		if ( $query_params['only_comments'] ) {
			$sql   .= " INNER JOIN {$wpdb->walkthecounty_comments} as gc1 ON (p1.ID = gc1.comment_parent)";
		...
		if (
			...
			$where .= " AND p1.ID NOT IN ( SELECT DISTINCT({$donation_id_col}) FROM {$wpdb->donationmeta} WHERE meta_key='_walkthecounty_anonymous_donation' AND meta_value='1')";
		}
	...
	private function get_donor_comments( $donations_data = array() ) {
		...
		// Backward compatibility.
		if (
			! walkthecounty_has_upgrade_completed( 'v230_move_donor_note' )
			|| ! walkthecounty_has_upgrade_completed( 'v230_move_donation_note' )
		) {
			foreach ( $donations_data as $id => $data ) {
				$comment         = walkthecounty_get_donor_donation_comment( $id, $data['_walkthecounty_payment_donor_id'] );
		...
		$sql   = "SELECT c1.comment_parent as donation_id, c1.comment_content as comment FROM {$wpdb->walkthecounty_comments} as c1";
		$sql   .= " INNER JOIN {$wpdb->walkthecounty_commentmeta} as cm1 ON (c1.comment_ID=cm1.walkthecounty_comment_id)";
		$where = array();

		foreach ( $donations_data as $id => $data ) {
			// Do not fetch comment for anonymous donation.
			if( ! empty( $data['_walkthecounty_anonymous_donation'] )  ) {
				continue;
			}

			$where[] = "(c1.comment_parent={$id} AND cm1.meta_key='_walkthecounty_donor_id' AND cm1.meta_value={$data['_walkthecounty_payment_donor_id']})";
		}
...
WalkTheCounty_Donor_Wall::get_instance();
