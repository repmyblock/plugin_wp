 ...
 * @return mixed
 */
function walkthecounty_get_currency_formatting_settings( $id_or_currency_code = null ) {
	$walkthecounty_options = walkthecounty_get_settings();
	$setting      = array();

	if ( ! empty( $id_or_currency_code ) ) {
		$currencies = walkthecounty_get_currencies( 'all' );

		// Set default formatting setting only if currency not set as global currency.
		if (
			is_string( $id_or_currency_code ) &&
			! empty( $walkthecounty_options['currency'] ) &&
			$id_or_currency_code !== $walkthecounty_options['currency'] &&
			array_key_exists( $id_or_currency_code, $currencies )
		) {
			$setting = $currencies[ $id_or_currency_code ]['setting'];
		} elseif ( is_numeric( $id_or_currency_code ) && 'walkthecounty_payment' === get_post_type( $id_or_currency_code ) ) {
			$currency = walkthecounty_get_meta( $id_or_currency_code, '_walkthecounty_payment_currency', true );

			if (
				! empty( $currency) &&
				$walkthecounty_options['currency'] !== $currency
			) {
	...
	if ( empty( $setting ) ) {
		...
		$thousand_separator = isset( $walkthecounty_options['thousands_separator'] ) ? $walkthecounty_options['thousands_separator'] : ',';
		...
		$decimal_separator = ! empty( $walkthecounty_options['decimal_separator'] ) ? $walkthecounty_options['decimal_separator'] : $default_decimal_separator;

		$setting = array(
			'currency_position'   => walkthecounty_get_option( 'currency_position', 'before' ),
			'thousands_separator' => $thousand_separator,
			'decimal_separator'   => $decimal_separator,
			'number_decimals'     => walkthecounty_get_option( 'number_decimals', 0 ),
		);
	}
	 ...
	 * @since 1.8.15
	 */
	return apply_filters( 'walkthecounty_get_currency_formatting_settings', $setting, $id_or_currency_code );
}
 ...
 * @return mixed
 */
function walkthecounty_get_price_decimals( $id_or_currency_code = null ) {
	// Set currency on basis of donation id.
	if ( empty( $id_or_currency_code ) ) {
		$id_or_currency_code = walkthecounty_get_currency();
	}

	$number_of_decimals = 0;

	if ( ! walkthecounty_is_zero_based_currency( $id_or_currency_code ) ) {
		$setting            = walkthecounty_get_currency_formatting_settings( $id_or_currency_code );
	 ...
	 * @since 1.6
	 */
	return apply_filters( 'walkthecounty_sanitize_amount_decimals', $number_of_decimals, $id_or_currency_code );
}
 ...
 * @return mixed
 */
function walkthecounty_get_price_thousand_separator( $id_or_currency_code = null ) {
	$setting = walkthecounty_get_currency_formatting_settings( $id_or_currency_code );
	 ...
	 * @since 1.6
	 */
	return apply_filters( 'walkthecounty_get_price_thousand_separator', $setting['thousands_separator'], $id_or_currency_code );
}
 ...
 * @return mixed
 */
function walkthecounty_get_price_decimal_separator( $id_or_currency_code = null ) {
	$setting = walkthecounty_get_currency_formatting_settings( $id_or_currency_code );
	 ...
	 * @since 1.6
	 */
	return apply_filters( 'walkthecounty_get_price_decimal_separator', $setting['decimal_separator'], $id_or_currency_code );
}
 ...
 * @since 2.4.5
 */
function walkthecounty_is_amount_sanitized( $amount ) {
	$is_sanitize = false;
 ...
 */
function walkthecounty_sanitize_amount_for_db( $number, $args = array() ) {
	$args['number_decimals'] = 6;

	if (
		( isset( $args['currency'] ) && 'BTC' === $args['currency'] )
		|| 'BTC' === walkthecounty_get_currency()
	) {
		$args['number_decimals'] = 10;
	}

	return walkthecounty_maybe_sanitize_amount( $number, $args );
}
 ...
 * @return string $amount Newly sanitized amount
 */
function walkthecounty_maybe_sanitize_amount( $number, $args = array() ) {
	// Bailout.
	...
	$thousand_separator = walkthecounty_get_price_thousand_separator( $args['currency'] );
	$decimal_separator  = walkthecounty_get_price_decimal_separator( $args['currency'] );
	$number_decimals    = is_bool( $args['number_decimals'] ) ?
		walkthecounty_get_price_decimals( $args['currency'] ) :
	...
	$number = trim( str_replace( walkthecounty_currency_symbols( true ), '', $number ) );
	...
	if ( walkthecounty_is_amount_sanitized( $number ) ) {
	...
	return walkthecounty_sanitize_amount( $number, $args );
}

/**
 * Sanitize Amount
 *
 * Note: Do not this function to sanitize amount instead use walkthecounty_maybe_sanitize_amount function.
 *
 ...
 * @return string $amount Newly sanitized amount
 */
function walkthecounty_sanitize_amount( $number, $args = array() ) {
	...
	$locale   = localeconv();
	$decimals = array(
		walkthecounty_get_price_decimal_separator( $args['currency'] ),
	...
	$default_dp = walkthecounty_get_price_decimals( $args['currency'] );
	...
	if ( false !== $args['number_decimals'] ) {
		...
		$dp     = apply_filters( 'walkthecounty_sanitize_amount_decimals', $dp, $number );
	 ...
	 * @since 1.0
	 */
	return apply_filters( 'walkthecounty_sanitize_amount', $number );
}
 ...
 */
function walkthecounty_format_amount( $amount, $args = array() ) {
	// Backward compatibility.
	if ( is_bool( $args ) ) {
		$args = array(
			'decimal' => $args,
	...
	if ( $args['donation_id'] && empty( $args['currency'] ) ) {
		$args['currency'] = walkthecounty_get_meta( $args['donation_id'], '_walkthecounty_payment_currency', true );
	}

	$formatted     = 0;
	$currency      = ! empty( $args['currency'] ) ? $args['currency'] : walkthecounty_get_currency( $args['donation_id'] );
	$thousands_sep = walkthecounty_get_price_thousand_separator( $currency );
	$decimal_sep   = walkthecounty_get_price_decimal_separator( $currency );
	$decimals      = ! empty( $args['decimal'] ) ? walkthecounty_get_price_decimals( $currency ) : 0;

	if ( ! empty( $amount ) ) {
		// Sanitize amount before formatting.
		$amount = ! empty( $args['sanitize'] ) ?
			walkthecounty_maybe_sanitize_amount( $amount, array( 'number_decimals' => $decimals, 'currency' => $currency ) ) :
	 ...
	 * @since 1.0
	 */
	return apply_filters( 'walkthecounty_format_amount', $formatted, $amount, $decimals, $decimal_sep, $thousands_sep, $currency, $args );
}
 ...
 * @since 1.6
 *
 * @use   walkthecounty_get_price_thousand_separator Get thousand separator.
 *
 ...
 */
function walkthecounty_human_format_large_amount( $amount, $args = array() ) {

	// Set default currency;
	if ( empty( $args['currency'] ) ) {
		$args['currency'] = walkthecounty_get_currency();
	}

	// Get thousand separator.
	$thousands_sep = walkthecounty_get_price_thousand_separator( $args['currency'] );

	// Sanitize amount for calculation purpose.
	$sanitize_amount = walkthecounty_maybe_sanitize_amount(
		$amount,
		array(
	...
	switch ( $args['currency'] ) {
		case 'INR':
			...
			if ( 4 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s arab', 'walkthecounty' ), round( ( $sanitize_amount / 1000000000 ), 2 ) );
			} elseif ( 3 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s crore', 'walkthecounty' ), round( ( $sanitize_amount / 10000000 ), 2 ) );
			} elseif ( 2 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s lakh', 'walkthecounty' ), round( ( $sanitize_amount / 100000 ), 2 ) );
			}
			break;
		default:
			// Calculate large number formatted amount.
			if ( 4 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s trillion', 'walkthecounty' ), round( ( $sanitize_amount / 1000000000000 ), 2 ) );
			} elseif ( 3 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s billion', 'walkthecounty' ), round( ( $sanitize_amount / 1000000000 ), 2 ) );
			} elseif ( 2 < $amount_count_parts ) {
				$human_format_amount = sprintf( esc_html__( '%s million', 'walkthecounty' ), round( ( $sanitize_amount / 1000000 ), 2 ) );
			}
	}

	return apply_filters( 'walkthecounty_human_format_large_amount', $human_format_amount, $amount, $sanitize_amount );
}
 ...
 * @return string $amount Newly formatted amount or Price Not Available
 */
function walkthecounty_format_decimal( $args ) {
	// Backward compatibility.
	if ( ! is_array( $args ) ) {
		$func_args = func_get_args();
		$args      = array(
	...
	if( ! empty( $args['donation_id'] ) ) {
		...
		if( empty( $args['currency'] ) ) {
			$args['currency'] = walkthecounty_get_payment_currency_code( $args['donation_id'] );
		}

		// Set amount if not already done.
		if( empty( $args['amount'] ) ) {
			$args['amount'] = walkthecounty_donation_amount( $args['donation_id'] );
		}
	}

	$decimal_separator = walkthecounty_get_price_decimal_separator();
	$formatted_amount  = $args['sanitize'] ?
		walkthecounty_maybe_sanitize_amount( $args['amount'], array( 'number_decimals' => $args['dp'], 'currency' => $args['currency'] ) ) :
		number_format( $args['amount'], ( is_bool( $args['dp'] ) ? walkthecounty_get_price_decimals( $args['currency'] ) : $args['dp'] ), '.', '' );

	if ( false !== strpos( $formatted_amount, '.' ) ) {
		$formatted_amount = str_replace( '.', $decimal_separator, $formatted_amount );
	}

	return apply_filters( 'walkthecounty_format_decimal', $formatted_amount, $args['amount'], $decimal_separator, $args );
}

/**
 * Get date format string on basis of walkthecountyn context.
 *
 * @since 1.7
 *
 ...
 */
function walkthecounty_date_format( $date_context = '' ) {
	/**
	 * Filter the date context
	 *
	 * You can add your own date context or use already exist context.
	 * For example:
	 *    add_filter( 'walkthecounty_date_format_contexts', 'add_new_date_contexts' );
	 ...
	 *    }
	 */
	$date_format_contexts = apply_filters( 'walkthecounty_date_format_contexts', array() );
	...
	return apply_filters( 'walkthecounty_date_format', $date_format );
}
 ...
 *
 * @since      1.7
 * @deprecated 1.8.7 You can access this function from WalkTheCounty_Cache.
 *
 ...
 * @return string
 */
function walkthecounty_get_cache_key( $action, $query_args ) {
	return WalkTheCounty_Cache::get_key( $action, $query_args );
}
 ...
 * @return string|array
 */
function walkthecounty_clean( $var ) {
	if ( is_array( $var ) ) {
		return array_map( 'walkthecounty_clean', $var );
	}
 ...
 * @return int
 */
function walkthecounty_let_to_num( $size ) {
	$l   = substr( $size, - 1 );
	$ret = substr( $size, 0, - 1 );
	switch ( strtoupper( $l ) ) {
		case 'P':
 ...
 * @return bool
 */
function walkthecounty_validate_nonce( $nonce, $action = - 1, $wp_die_args = array() ) {
 ...
 * @return bool
 */
function walkthecounty_verify_donation_form_nonce( $nonce = '', $form_id ) {

	// Form nonce action.
	$nonce_action = "walkthecounty_donation_form_nonce_{$form_id}";

	// Nonce validation.
	$verify_nonce = walkthecounty_validate_nonce( $nonce, $nonce_action );

	if ( ! $verify_nonce ) {
		walkthecounty_set_error( 'donation_form_nonce', __( 'We\'re unable to recognize your session. Please refresh the screen to try again; otherwise contact your website administrator for assistance.', 'walkthecounty' ) );
	}
 ...
 * @return mixed
 */
function walkthecounty_check_variable( $variable, $conditional = '', $default = false, $array_key_name = '' ) {
