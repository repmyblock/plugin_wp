...
/**
 * WalkTheCounty_Forms_Query Class
 ...
 * @since 2.5.0
 */
class WalkTheCounty_Forms_Query {
	/**
	 * Preserve args
	 *
	 * @since  2.5.0
	...
	/**
	 * The args to pass to the walkthecounty_get_forms() query
	 *
	 * @since  2.5.0
	 * @access public
	 *
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			'output'    => 'forms',
			'post_type' => array( 'walkthecounty_forms' ),
		);
	...
	public function get_forms() {
		...
		$results     = array();
		$this->forms = array();
		$cache_key   = WalkTheCounty_Cache::get_key( 'walkthecounty_form_query', $this->args, false );
		$this->forms = WalkTheCounty_Cache::get_db_query( $cache_key );
		...
		if ( $query->have_posts() ) {
			...
			if ( ! in_array( $this->args['output'], $custom_output ) ) {
				...
				while ( $query->have_posts() ) {
					...
					$this->forms[] = apply_filters( 'walkthecounty_form', $form, $form_id, $this );
				}
	...
	public static function update_meta_cache( $form_ids ) {
		...
		update_meta_cache( WalkTheCounty()->form_meta->get_meta_type(), $form_ids );
	}
}
