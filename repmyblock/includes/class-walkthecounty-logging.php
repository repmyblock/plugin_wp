 ...
 * @subpackage  Classes/WalkTheCounty_Logging
...
class WalkTheCounty_Logging {
	 ...
	 * Class Constructor
	 *
	 * Set up the WalkTheCounty Logging Class.
	 *
	 * @since  1.0
	 * @access public
	 */
	public function __construct() {
		 ...
		 * Setup properties
		 */
		$this->log_db     = WalkTheCounty()->log_db;
		$this->logmeta_db = WalkTheCounty()->logmeta_db;

		/**
		 * Setup hooks.
		 */

		add_action( 'save_post_walkthecounty_payment', array( $this, 'background_process_delete_cache' ) );
		add_action( 'save_post_walkthecounty_forms', array( $this, 'background_process_delete_cache' ) );
		add_action( 'save_post_walkthecounty_log', array( $this, 'background_process_delete_cache' ) );
		add_action( 'walkthecounty_delete_log_cache', array( $this, 'delete_cache' ) );
		add_action( 'update_log_metadata', array( $this, 'bc_200_set_payment_as_log_parent' ), 10, 4 );

		// Backward compatibility.
		if ( ! walkthecounty_has_upgrade_completed( 'v20_logs_upgrades' ) ) {
	 ...
	 * Log Post Type
	 *
	 * Registers the 'walkthecounty_log' Post Type.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function register_post_type() {
		...
		$log_args = array(
			'labels'              => array(
				'name' => esc_html__( 'Logs', 'walkthecounty' ),
			),
			'public'              => false,
		...
		register_post_type( 'walkthecounty_log', $log_args );
	}
	...
	public function register_taxonomy() {
		register_taxonomy( 'walkthecounty_log_type', 'walkthecounty_log', array(
			'public' => false,
		) );
	}
	...
	public function log_types() {
		...
		return apply_filters( 'walkthecounty_log_types', $terms );
	}
	...
	public function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		if ( $log_id && ! empty( $log_meta ) ) {
			foreach ( (array) $log_meta as $key => $meta ) {
				$this->logmeta_db->update_meta( $log_id, '_walkthecounty_log_' . sanitize_key( $key ), $meta );
			}
		}
	...
	public function update_log( $log_data = array(), $log_meta = array() ) {
		 ...
		 * @param array $log_meta Log entry meta.
		 */
		do_action( 'walkthecounty_pre_update_log', $log_data, $log_meta );
		...
		if ( ! walkthecounty_has_upgrade_completed( 'v20_logs_upgrades' ) ) {
			...
			if ( $log_id && ! empty( $log_meta ) ) {
				foreach ( (array) $log_meta as $key => $meta ) {
					if ( ! empty( $meta ) ) {
						walkthecounty_update_meta( $log_id, '_walkthecounty_log_' . sanitize_key( $key ), $meta );
					}
				}
			}
		} else {
		 ...
		 * @param array $log_meta Log entry meta.
		 */
		do_action( 'walkthecounty_post_update_log', $log_id, $log_data, $log_meta );
	}
	 ...
	 * For new table params check: WalkTheCounty_DB_Logs::get_column_defaults and WalkTheCounty_DB_Logs::get_sql#L262
	 *
	 * @since  1.0
	...
	public function background_process_delete_cache( $post_id ) {
		...
		wp_schedule_single_event( time() - 5, 'walkthecounty_delete_log_cache' );
	}
	...
	public function delete_cache() {
		...
		$cache_walkthecounty_logs      = WalkTheCounty_Cache::get_options_like( 'walkthecounty_logs' );
		$cache_walkthecounty_log_count = WalkTheCounty_Cache::get_options_like( 'log_count' );

		$cache_option_names = array_merge( $cache_walkthecounty_logs, $cache_walkthecounty_log_count );
		...
		WalkTheCounty_Cache::delete( $cache_option_names );
	}
				...
				switch ( $new_query_param ) {
					case 'log_type':
						$log_query['tax_query'] = array(
							array(
								'taxonomy' => 'walkthecounty_log_type',
								'field'    => 'slug',
						...
						if( ! empty( $log_query['meta_query'] ) && empty( $log_query['post_parent'] ) ) {
							foreach ( $log_query['meta_query'] as $index => $meta_query ){
								...
								switch ( $meta_query['key'] ) {
									case '_walkthecounty_log_form_id':
						...
						switch( $new_query_param ){
							case 'log_parent':
								$log_query['meta_query'][] = array(
									'key' => '_walkthecounty_log_payment_id',
	...
	private function bc_200_add_new_properties( &$logs ) {
		...
		$query_params = array(
			...
			'log_date'     => 'post_date',
			'log_date_gmt' => 'post_date_gmt',
			'log_type'     => 'walkthecounty_log_type',
		);
			...
			foreach ( $logs as $index => $log ) {
				foreach ( $query_params as $new_query_param => $old_query_param ) {
					if ( ! property_exists( $log, $old_query_param ) ) {
						...
						// 1. log_type
						$term = get_the_terms( $log->ID, 'walkthecounty_log_type' );
					...
					switch ( $old_query_param ) {
						case 'post_parent':
							$logs[ $index ]->{$new_query_param} = walkthecounty_get_meta( $log->ID, '_walkthecounty_log_payment_id', true );
							break;
	...
	public function bc_200_set_payment_as_log_parent( $check, $log_id, $meta_key, $meta_value ) {
		...
		// Bailout.
		if (
			'walkthecounty_payment' === $post_type ||
			'_walkthecounty_log_payment_id' !== $meta_key
		) {
			return $check;
		}
		...
		if ( $form_id ) {
			$this->logmeta_db->delete_meta( $log_id, '_walkthecounty_log_payment_id' );
			$this->logmeta_db->update_meta( $log_id, '_walkthecounty_log_form_id', $form_id );
