...
/**
 * WalkTheCounty_Donor_List_Table Class.
 *
 * @since 1.0
 */
class WalkTheCounty_Donor_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( array(
			'singular' => __( 'Donor', 'walkthecounty' ), // Singular name of the listed records.
			'plural'   => __( 'Donors', 'walkthecounty' ), // Plural name of the listed records.
	...
	public function advanced_filters() {
		...
		?>
		<div id="walkthecounty-donor-filters" class="walkthecounty-filters">
			<div class="walkthecounty-donor-search-box">
				<input type="text" id="walkthecounty-donors-search-input" placeholder="<?php _e( 'Name, Email, or Donor ID', 'walkthecounty' ); ?>" name="s" value="<?php echo $search; ?>">
				<?php submit_button( __( 'Search', 'walkthecounty' ), 'button', false, false, array(
			 ...
			 * @since 2.4.0
			 */
			do_action( 'walkthecounty_donor_table_advanced_filters' );
			...
			<div class="walkthecounty-filter">
				...
				if ( ! empty( $start_date ) || ! empty( $end_date ) || ! empty( $donor ) || ! empty( $search ) || ! empty( $status ) || ! empty( $form_id ) ) :
					?>
					<a href="<?php echo admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-donors' ); ?>"
	...
	public function column_default( $donor, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'num_donations' :
				$value = sprintf(
					'<a href="%s">%s</a>',
					admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-payment-history&donor=' . absint( $donor['id'] ) ),
					esc_html( $donor['num_donations'] )
				);
				break;

			case 'amount_spent' :
				$value = walkthecounty_currency_filter( walkthecounty_format_amount( $donor[ $column_name ], array( 'sanitize' => false ) ) );
				break;

			case 'date_created' :
				$value = date_i18n( walkthecounty_date_format(), strtotime( $donor['date_created'] ) );
				break;
		...
		return apply_filters( "walkthecounty_donors_column_{$column_name}", $value, $donor['id'] );
	...
	public function column_name( $donor ) {
		...
		$donation_gravatar_image = sprintf(
			'<span class="walkthecounty-donor__image walkthecounty-donor-admin-avatar" data-donor_email="%1$s" data-has-valid-gravatar="%2$s">%3$s</span>',
			md5( strtolower( trim( $donor['email'] ) ) ),
			absint( walkthecounty_validate_gravatar( $donor['email'] ) ),
			esc_attr( $donor_name_initial )
		);

		$name = ! empty( $donor['name'] )
			? sprintf(
				'%1$s<span class="walkthecounty-donor-name-text">%2$s</span>',
				$donation_gravatar_image,
				esc_attr( $donor['name'] )
			)
			: sprintf(
				'<em>%1$s</em>',
				__( 'Unnamed Donor', 'walkthecounty' )
			);

		$view_url = admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-donors&view=overview&id=' . $donor['id'] );
		$actions  = $this->get_row_actions( $donor );

		return sprintf(
			'<a href="%1$s" class="walkthecounty-donor-name">%2$s</a>%3$s',
			esc_url( $view_url ),
			$name,
	...
	public function get_columns() {
		$columns = array(
			...
			'name'          => __( 'Name', 'walkthecounty' ),
			'email'         => __( 'Email', 'walkthecounty' ),
			'num_donations' => __( 'Donations', 'walkthecounty' ),
			'amount_spent'  => __( 'Total Donated', 'walkthecounty' ),
			'date_created'  => __( 'Date Created', 'walkthecounty' ),
		);

		return apply_filters( 'walkthecounty_list_donors_columns', $columns );
	...
	public function get_sortable_columns() {
		...
		return apply_filters( 'walkthecounty_list_donors_sortable_columns', $columns );
	}
	...
	public function get_row_actions( $donor ) {
		...
		$actions = array(
			'id'     => '<span class="walkthecounty-donor-id">ID: ' . $donor['id'] . '  </span>',
			'view'   => sprintf( '<a href="%1$s" aria-label="%2$s">%3$s</a>', admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-donors&view=overview&id=' . $donor['id'] ), sprintf( esc_attr__( 'View "%s"', 'walkthecounty' ), esc_attr( $donor['name'] ) ), __( 'View Donor', 'walkthecounty' ) ),
			'delete' => sprintf( '<a class="%1$s" data-id="%2$s" href="#" aria-label="%3$s">%4$s</a>', 'walkthecounty-single-donor-delete', $donor['id'], sprintf( esc_attr__( 'Delete "%s"', 'walkthecounty' ), esc_attr( $donor['name'] ) ), __( 'Delete', 'walkthecounty' ) ),
		);

		return apply_filters( 'walkthecounty_donor_row_actions', $actions, $donor );
	...
	public function donor_data() {
		...
		return apply_filters( 'walkthecounty_donors_column_query_data', $data );
	}
	...
	private function get_donor_count() {
		...
		$donors                 = WalkTheCounty()->donors->get_donors( $_donor_query );
	...
	public function get_donor_query() {
		...
		$start_date = ! empty ( $_GET['start-date'] ) ? strtotime( walkthecounty_clean( $_GET['start-date'] ) ) : false;
		$end_date   = ! empty( $_GET['end-date'] ) ? strtotime( walkthecounty_clean( $_GET['end-date'] ) ) : false;
		 ...
		 * @since 2.4.0
		 */
		$args = (array) apply_filters( 'walkthecounty_donor_table_query', $args );
	...
	public function display() {
		...
		<table class="wp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
			...
			<tr class="hidden"></tr>
			<tr id="walkthecounty-bulk-delete"
				...
				<td colspan="6" class="colspanchange">
					...
					<fieldset class="inline-edit-col-left">
						<legend class="inline-edit-legend"><?php esc_attr_e( 'BULK DELETE', 'walkthecounty' ); ?></legend>
						<div class="inline-edit-col">
							<div id="bulk-titles">
								<div id="walkthecounty-bulk-donors" class="walkthecounty-bulk-donors">
					...
					<fieldset class="inline-edit-col-right">
						<div class="inline-edit-col">
							<label>
								<input class="walkthecounty-donor-delete-confirm" type="checkbox"
								       name="walkthecounty-donor-delete-confirm"/>
								<?php esc_attr_e( 'Are you sure you want to delete the selected donor(s)?', 'walkthecounty' ); ?>
							</label>
							<label>
								<input class="walkthecounty-donor-delete-records" type="checkbox"
								       name="walkthecounty-donor-delete-records"/>
								<?php esc_attr_e( 'Delete all associated donations and records?', 'walkthecounty' ); ?>
							</label>
						</div>
					</fieldset>

					<p class="submit inline-edit-save">
						<input type="hidden" name="walkthecounty_action" value="delete_bulk_donor"/>
						...
						<button type="button" id="walkthecounty-bulk-delete-cancel"
						        class="button cancel alignleft"><?php esc_attr_e( 'Cancel', 'walkthecounty' ); ?></button>
						<input type="submit" id="walkthecounty-bulk-delete-button" disabled
						       class="button button-primary alignright"
						       value="<?php esc_attr_e( 'Delete', 'walkthecounty' ); ?>">
						<br class="clear">
					</p>
				</td>
			</tr>
