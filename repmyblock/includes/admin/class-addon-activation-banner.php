<?php
/**
 * WalkTheCounty Activation Banner
 *
...
global $walkthecounty_addons;

/**
 * Class WalkTheCounty_Addon_Activation_Banner
 *
...
class WalkTheCounty_Addon_Activation_Banner {
	 ...
	 *                               'name'              => __( 'Authorize.net Gateway', 'walkthecounty-authorize' ), // (required) Name of the Add-on
	 *                               'version'           => WALKTHECOUNTY_AUTHORIZE_VERSION, // (required)The most current version
	 *                               'documentation_url' => 'http://docs.walkthecountywp.com/addon-authorize',// (required)
	 *                               'support_url'       => 'https://walkthecountywp.com/support/', // (required)Location of Add-on settings page, leave blank to hide
	 *                               'testing'           => false, // (required) Never leave as "true" in production!!!
	 *                               }
	 */
	function __construct( $_banner_details ) {
		global $walkthecounty_addons, $pagenow;

		// Append add-on information to the global variable.
		$walkthecounty_addons[] = $_banner_details;
	...
	public static function get_banner_user_meta_key( $addon_banner_key ) {
		...
		return "walkthecounty_addon_{$addon_slug}_active_by_user";
	}
	...
	public function init() {
		...
		add_action( 'current_screen', array( $this, 'walkthecounty_addon_notice_ignore' ) );

		// Get the WalkTheCounty add-ons.
		$walkthecounty_addons = $this->get_plugin_file_names();

		if ( ! empty( $walkthecounty_addons ) ) {

			// Go through each of the add-on and hook deactivate action.
			foreach ( $walkthecounty_addons as $addon_name => $walkthecounty_addon ) {

				// Testing?
				if ( true === $walkthecounty_addon['testing'] ) {
					$nag_meta_key = 'walkthecounty_addon_activation_ignore_' . $addon_name;
					delete_user_meta( $this->user_id, $nag_meta_key );
				}

				// Add deactivate hook.
				add_action( 'deactivate_' . $walkthecounty_addon['plugin_main_file'], array( $this, 'remove_addon_activate_meta' ) );
			}
		}
	}
	...
	private function get_plugin_file_names() {
		...
		foreach ( $walkthecounty_addons as $walkthecounty_addon ) {
			$file_name = '';
			$file_path = explode( '/plugins/', $walkthecounty_addon['file'] );
			if ( $file_path ) {
			...
			if ( ! empty( $file_name ) ) {
				foreach ( $active_plugins as $plugin ) {
					if ( false !== strpos( $plugin, $file_name ) ) {
						$add_on_key                     = sanitize_title( $walkthecounty_addon['name'] );
						$walkthecounty_addon['plugin_main_file'] = $plugin; // Include plugin file.
						$file_names[ $add_on_key ]      = $walkthecounty_addon;
						break;
					}
				}
			}
	...
	private function add_addon_activate_meta() {
		...
		if ( ! empty( $walkthecounty_addons ) ) {
			...
			foreach ( $walkthecounty_addons as $banner_addon_name => $addon ) {

				// User meta key.
				$user_id = __walkthecounty_get_active_by_user_meta( $banner_addon_name );
				...
				if ( ! $user_id ) {
					...
					// Update global cache.
					$GLOBALS['walkthecounty_addon_activated_by_user'][$option_key] = $this->user_id;
				}
			}
		}
	}
	...
	public function addon_activation_banner_notices() {
		global $pagenow, $walkthecounty_addons;
		...
		// Get the plugin folder name, because many walkthecounty-addon not sending proper plugin_file.
		...
		// Go through each of the walkthecounty add-on.
		foreach ( $walkthecounty_addons as $addon ) {
			$addon_sanitized_name = sanitize_title( $addon['name'] );

			// Get the add-on dismiss status.
			$add_on_state = get_user_meta( $this->user_id, "walkthecounty_addon_activation_ignore_{$addon_sanitized_name}", true );

			// Get the option key.
			$activate_by_user = (int) __walkthecounty_get_active_by_user_meta( $addon_sanitized_name );
		...
		if ( ! empty( $addon_to_display ) ) {
			...
			$this->print_css_js();
			?>
			<div class="<?php echo ( 1 !== count( $addon_to_display ) ) ? 'walkthecounty-alert-tab-wrapper' : ''; ?> updated walkthecounty-addon-alert walkthecounty-notice">
				<?php
				// If multiple add-on are activated.
				if ( 1 !== count( $addon_to_display ) ) {
					?>
					<div class="walkthecounty-vertical-tab">
						<div class="walkthecounty-addon-tab-list">
							<ul class="walkthecounty-alert-addon-list">
								<?php
								$is_first = true;
								foreach ( $addon_to_display as $banner ) {
									?>
									<li class="walkthecounty-tab-list<?php echo ( true === $is_first ) ? ' active' : ''; ?>" id="walkthecounty-addon-<?php echo esc_attr( basename( $banner['file'], '.php' ) ); ?>">
						...
						<div class="walkthecounty-right-side-block">
							<?php foreach ( $addon_to_display as $banner ) : ?>
								<div class="walkthecounty-tab-details <?php echo ( true === $is_first ) ? ' active' : ''; ?> " id="walkthecounty-addon-<?php echo esc_attr( basename( $banner['file'], '.php' ) ); ?>">
									<?php
			...
			WalkTheCounty()->notices->register_notice( array(
				'id'               => 'walkthecounty_add_on_activation_notice',
				'type'             => 'updated',
	...
	private function render_single_addon_banner( $banner_arr, $is_single ) {
		// Get all walkthecounty add-on.
		$walkthecounty_addons = walkthecounty_get_plugins();

		// Plugin main file.
		$plugin_file = $banner_arr['file'];

		// Get the plugin main file.
		foreach ( $walkthecounty_addons as $main_file => $addon ) {
		...
		$dismiss_url = $is_single
			? admin_url( 'plugins.php?walkthecounty_addon_activation_ignore=1&walkthecounty_addon=' . sanitize_title( $banner_arr['name'] ) )
			: admin_url( 'plugins.php?walkthecounty_addon_activation_ignore=1&walkthecounty_addon=all' );

		// Get the add-on details.
		$plugin_data = get_plugin_data( $plugin_file );
		?>
		<img src="<?php echo esc_url( WALKTHECOUNTY_PLUGIN_URL . 'assets/dist/images/walkthecounty-icon-full-circle.svg' ); ?>" class="walkthecounty-logo" />
		<div class="walkthecounty-alert-message">
			<h3>
				<?php
				printf(
					/* translators: %s: Add-on name */
					'%s<span>%s</span>',
					__( 'New WalkTheCountyWP Add-on Activated: ', 'walkthecounty' ),
			...
			<div class="alert-actions">
				...
				if ( ! empty( $plugin_data['Description'] ) ) {
					?>
					<span class="walkthecounty-addon-description">
				...
				if ( isset( $banner_arr['settings_url'] ) ) {
					printf(
						...
						esc_html__( 'Go to Settings', 'walkthecounty' )
					);
				}
				...
				if ( isset( $banner_arr['documentation_url'] ) ) {
					printf(
						...
						sprintf(
							/* translators: %s: Add-on name */
							esc_html__( 'Documentation: %s Add-on', 'walkthecounty' ),
							esc_html( $banner_arr['name'] )
						)
					);
				}
				...
				if ( isset( $banner_arr['support_url'] ) ) {
					printf(
						...
						esc_html__( 'Get Support', 'walkthecounty' )
					);
				}
				?>
			</div>
	 ...
	 * @access public
	 */
	public function walkthecounty_addon_notice_ignore() {
		/**
		 ...
		 */
		if (
			isset( $_GET['walkthecounty_addon'], $_GET['walkthecounty_addon_activation_ignore'] )
			&& '1' === $_GET['walkthecounty_addon_activation_ignore']
		) {
			// Get the value of the 'walkthecounty_addon' query string.
			$addon_query_arg    = sanitize_text_field( wp_unslash( $_GET['walkthecounty_addon'] ) );
			...
			if ( 'all' === $addon_query_arg ) {
				...
				if ( ! empty( $walkthecounty_addons ) ) {
					$deactivated_addons = array_keys( $walkthecounty_addons );
				}
			} else {
			...
			if ( ! empty( $deactivated_addons ) ) {
				foreach ( $deactivated_addons as $addon ) {
					// Record it user meta.
					add_user_meta( $this->user_id, "walkthecounty_addon_activation_ignore_{$addon}", 'true', true );
				}
			}
		}
	}
		...
		if ( ! empty( $walkthecounty_addons ) ) {
			foreach ( $walkthecounty_addons as $banner_addon_name => $addon ) {
				if ( $plugin_file === $addon['plugin_main_file'] ) {
					...
					$user_id = (int) __walkthecounty_get_active_by_user_meta( $banner_addon_name );

					if ( $user_id ) {
						// Get user meta for this add-on.
						$nag_meta_key = "walkthecounty_addon_activation_ignore_{$banner_addon_name}";
	...
	public function get_recently_activated_addons() {
		return walkthecounty_get_recently_activated_addons();
	}
	...
	private function print_css_js() {
		?>
		<style>
			div.walkthecounty-addon-alert.updated {
				padding: 20px;
				position: relative;
				border-color: #66BB6A;
				min-height: 85px;
			...
			div.walkthecounty-alert-message {
				margin-left: 108px;
			}

			div.walkthecounty-addon-alert img.walkthecounty-logo {
				max-width: 85px;
				float: left;
			}

			div.walkthecounty-addon-alert h3 {
				margin: -5px 0 10px;
				font-size: 22px;
				font-weight: 400;
				line-height: 30px;
			...
			div.walkthecounty-addon-alert h3 span {
				font-weight: 700;
				color: #66BB6A;
			}

			div.walkthecounty-addon-alert a {
				color: #66BB6A;
			}

			div.walkthecounty-addon-alert .alert-actions a {
				margin-right: 2em;
			}
			...
			div.walkthecounty-addon-alert .alert-actions a:hover {
				color: #555555;
			}

			div.walkthecounty-addon-alert .alert-actions a span {
				text-decoration: none;
				margin-right: 5px;
			}

			div.walkthecounty-addon-alert .dismiss {
				position: absolute;
				right: 0px;
				height: 99%;
				top: 23%;
			...
			div.walkthecounty-alert-tab-wrapper .dismiss {
				right: 0px !important;
				height: 99% !important;
				top: 23% !important;
			}

			div.walkthecounty-addon-alert .dismiss:hover {
				color: #333;
			}

			ul.walkthecounty-alert-addon-list {
				min-width: 220px;
			...
			div.walkthecounty-addon-alert .walkthecounty-addon-description {
				padding: 1px;
				display: inline-block;
				color: #777;
				margin-bottom: 12px;
			...
			div.walkthecounty-alert-tab-wrapper .walkthecounty-right-side-block {
			...
			div.walkthecounty-vertical-tab {
				width: 100%;
			}

			ul.walkthecounty-alert-addon-list li {
				display: block;
			...
			ul.walkthecounty-alert-addon-list li a.inactivate {
				cursor: default;
			}

			ul.walkthecounty-alert-addon-list li a {
				display: block;
				font-weight: bold;
				color: #a3a3a3;
				text-decoration: none;
			...
			ul.walkthecounty-alert-addon-list li.walkthecounty-tab-list.active a {
				color: #5f6c74;
				box-shadow: none;
			}

			div.updated.walkthecounty-addon-alert.walkthecounty-notice.walkthecounty-alert-tab-wrapper {
				display: inline-block;
				width: 100%;
			}

			.walkthecounty-alert-tab-wrapper .walkthecounty-tab-details {
				display: none;
				min-height: 100px;
				position: absolute;
				top: 0;
			...
			.walkthecounty-alert-tab-wrapper .walkthecounty-tab-details.active {
				display: block;
				z-index: 1;
				position: relative;
			}

			.walkthecounty-alert-tab-wrapper.walkthecounty-addon-alert img.walkthecounty-logo {
				max-width: 80px;
			}

			.walkthecounty-alert-tab-wrapper .walkthecounty-alert-message {
				margin-left: 100px;
				padding-top: 10px;
			}

			ul.walkthecounty-alert-addon-list li.walkthecounty-tab-list.active {
				background: #fff;
			}

			ul.walkthecounty-alert-addon-list li.walkthecounty-tab-list:last-child {
				border-bottom: 0px solid #ccc;
			}

			ul.walkthecounty-alert-addon-list li.walkthecounty-tab-list:first-child {
				border-top: 0 none;
			}

			.walkthecounty-alert-tab-wrapper {
				padding: 0 !important;
			}

			ul.walkthecounty-alert-addon-list::-webkit-scrollbar {
				height: 10px;
				width: 10px;
				border-radius: 4px;
			...
			ul.walkthecounty-alert-addon-list::-webkit-scrollbar-thumb {
				background: #939395;
				border-radius: 4px;
			}
			...
			@media screen and (max-width: 767px) {
				...
				.walkthecounty-alert-tab-wrapper .walkthecounty-right-side-block {
					width: 100%;
				}

				.walkthecounty-alert-tab-wrapper ul.walkthecounty-alert-addon-list {
					min-width: 100%;
				}
			}
		</style>

		<!-- Start of the WalkTheCounty Add-on tab JS -->
		<script type="text/javascript">
					jQuery( document ).ready( function( $ ) {
						$( '.walkthecounty-alert-tab-wrapper' ).on( 'click', '.walkthecounty-tab-list', function() {
							...
							var
								...
								addon_tab_wrapper = $( this ).closest( '.walkthecounty-alert-tab-wrapper' );

							// Remove 'active' class from all tab list.
							$( '.walkthecounty-alert-addon-list li' ).removeClass( 'active' );
							// Add active class to the selected tab.
							$( this ).addClass( 'active' );
							// Remove 'active' class from the details.
							addon_tab_wrapper.find( '.walkthecounty-tab-details' ).removeClass( 'active' );
							addon_tab_wrapper.find( '.walkthecounty-right-side-block .walkthecounty-tab-details#' + clicked_tab ).addClass( 'active' );

							return false;
						} );

						var add_on_tabs = $( '.walkthecounty-alert-addon-list' );
						...
						if ( 2 === add_on_tabs.find( 'li' ).length ) {
							var li = $( 'li.walkthecounty-tab-list' );
							li.last().clone().prependTo( 'ul.walkthecounty-alert-addon-list' );
							li.last().removeAttr( 'id' ).find( 'a' ).addClass( 'inactivate' ).html( '&nbsp;' );
							$( '.walkthecounty-tab-list:first' ).trigger( 'click' );
						}
					} );
		</script>
		<!-- End of the WalkTheCounty Add-on tab JS -->
		<?php
	}
}
