...
if ( ! class_exists( 'WalkTheCounty_Email_Notification' ) ) :

	/**
	 * WalkTheCounty_Email_Notification
	 *
	 * @abstract
	 * @since       2.0
	 */
	abstract class WalkTheCounty_Email_Notification {
		/**
		 * Array of instances
		 *
		 * @since  2.0
		...
		public $config = array(
			...
			'form_metabox_id'              => 'walkthecounty_email_notification_options_metabox_fields',
		);
		...
		public function load( $config ) {
			...
			$this->config['content_type'] = empty( $this->config['content_type'] ) || ! in_array(
				...
				? WalkTheCounty()->emails->get_content_type()
				: $this->config['content_type'];
			$this->config['content_type'] = walkthecounty_get_option( WalkTheCounty_Email_Setting_Field::get_prefix( $this ) . 'email_content_type', $this->config['content_type'] );

			// Set email template type.
			$this->config['email_template'] = empty( $this->config['email_template'] )
				? walkthecounty_get_option( 'email_template' )
			...
			$this->config['recipient_group_name'] = empty( $this->config['recipient_group_name'] )
				? ( ! WalkTheCounty_Email_Notification_Util::has_recipient_field( $this ) ? __( 'Donor', 'walkthecounty' ) : '' )
			...
			$this->config['notices']['non-notification-status-editable'] = empty( $this->config['notices']['non-notification-status-editable'] )
				? __( 'You can not edit notification status from here.', 'walkthecounty' )
			 ...
			 * @since 2.0
			 *
			 * @param                         array                   WalkTheCounty_Email_Notification::config
			 * @param WalkTheCounty_Email_Notification $this
			 */
			$this->config = apply_filters( 'walkthecounty_email_api_notification_config', $this->config, $this );
		...
		private function setup_filters() {
			...
			if ( isset( $_GET['section'] ) && walkthecounty_get_current_setting_section() === $this->config['id'] ) {
				// Initialize email context for email notification.
				$this->config['email_tag_context'] = apply_filters(
					"walkthecounty_{$this->config['id']}_email_tag_context",
			...
			if ( $this->config['show_on_emails_setting_page'] ) {
				add_filter( 'walkthecounty_get_settings_emails', array( $this, 'add_setting_fields' ), 10, 2 );
			}
			...
			if ( $this->config['has_recipient_field'] ) {
				add_action(
					"walkthecounty_save__walkthecounty_{$this->config['id']}_recipient",
			...
			$this->config['default_email_header'] = apply_filters(
				"walkthecounty_{$this->config['id']}_get_default_email_header",
		...
		public function add_setting_fields( $settings ) {
			if ( $this->config['id'] === walkthecounty_get_current_setting_section() ) {
		...
		public function get_setting_fields( $form_id = null ) {
			return WalkTheCounty_Email_Setting_Field::get_setting_fields( $this, $form_id );
		}
		...
		public function add_metabox_setting_field( $settings, $form_id ) {
			...
			if ( WalkTheCounty_Email_Notification_Util::is_email_notification_active( $this ) ) {
				$settings[] = array(
					'id'     => $this->config['id'],
		...
		public function get_recipient( $form_id = null ) {
			if ( empty( $this->recipient_email ) && $this->config['has_recipient_field'] ) {
				$this->recipient_email = WalkTheCounty_Email_Notification_Util::get_value(
					$this,
					WalkTheCounty_Email_Setting_Field::get_prefix( $this, $form_id ) . 'recipient',
					$form_id
				);
				 ...
				 * @deprecated 2.0
				 */
				$this->recipient_email = apply_filters( 'walkthecounty_admin_notice_emails', $this->recipient_email, $this, $form_id );
			}
			...
			return apply_filters(
				"walkthecounty_{$this->config['id']}_get_recipients",
		...
		public function get_notification_status( $form_id = null ) {
			$notification_status = empty( $form_id )
				? walkthecounty_get_option( "{$this->config['id']}_notification", $this->config['notification_status'] )
				: walkthecounty_get_meta( $form_id, WalkTheCounty_Email_Setting_Field::get_prefix( $this, $form_id ) . 'notification', true, 'global' );
			 ...
			 */
			return apply_filters(
				"walkthecounty_{$this->config['id']}_get_notification_status",
				$notification_status,
				$this,
				$form_id
			);
		...
		public function get_email_header( $form_id = null ) {
			$header = WalkTheCounty_Email_Notification_Util::get_value(
				$this,
				WalkTheCounty_Email_Setting_Field::get_prefix( $this, $form_id ) . 'email_header',
				$form_id,
				$this->config['default_email_header']
			);
			 ...
			 */
			return apply_filters(
				"walkthecounty_{$this->config['id']}_get_email_header",
				$header,
				$this,
				$form_id
			);
		...
		public function get_email_content_type( $form_id ) {
			$content_type = WalkTheCounty_Email_Notification_Util::get_value(
				$this,
				WalkTheCounty_Email_Setting_Field::get_prefix( $this, $form_id ) . 'email_content_type',
				$form_id,
				$this->config['content_type']
			);
			 ...
			 */
			return apply_filters(
				"walkthecounty_{$this->config['id']}_get_email_content_type",
				$content_type,
				$this,
				$form_id
			);
		...
		public function get_email_template( $form_id ) {
			$email_template = walkthecounty_get_meta( $form_id, '_walkthecounty_email_template', true );
			$email_template = WalkTheCounty_Email_Notification_Util::get_value(
				$this,
				WalkTheCounty_Email_Setting_Field::get_prefix( $this, $form_id ) . 'email_template',
				$form_id,
				! empty( $email_template ) && WalkTheCounty_Email_Notification_Util::can_use_form_email_options( $this, $form_id ) ?
					$email_template :
					$this->config['email_template']
			);
			 ...
			 */
			return apply_filters(
				"walkthecounty_{$this->config['id']}_get_email_template",
				$email_template,
				$this,
				$form_id
			);
		...
		public function get_allowed_email_tags( $formatted = false ) {
			// Get all email tags.
			$email_tags = WalkTheCounty()->email_tags->get_tags();
			...
			if ( count( $email_tags ) && $formatted ) :
				ob_start() ?>
				<ul class="walkthecounty-email-tags-wrap">
					<?php foreach ( $email_tags as $email_tag ) : ?>
						<li class="walkthecounty_<?php echo $email_tag['tag']; ?>_tag">
		...
		public function get_preview_email_recipient( $form_id = null ) {
			 ...
			 * @param WalkTheCounty_Email_Notification $this
			 */
			$recipients = apply_filters( 'walkthecounty_get_preview_email_recipient', $recipients, $this, $form_id );
		...
		public function get_email_attachments( $form_id = null ) {
			 ...
			 * @since 2.0
			 */
			return apply_filters( "walkthecounty_{$this->config['id']}_get_email_attachments", array(), $this, $form_id );
		}
		...
		public function send_preview_email( $send = true ) {
			...
			WalkTheCounty()->emails->__set( 'content_type', $content_type );
			WalkTheCounty()->emails->__set( 'html', true );

			// Setup email template
			WalkTheCounty()->emails->__set( 'template', $this->get_email_template( $form_id ) );

			// Set email header.
			WalkTheCounty()->emails->__set( 'heading', $this->preview_email_template_tags( $this->get_email_header( $form_id ) ) );
			...
			if ( WalkTheCounty_Email_Notification_Util::can_use_form_email_options( $this, $form_id ) ) {
				WalkTheCounty()->emails->form_id      = $form_id;
				WalkTheCounty()->emails->from_name    = walkthecounty_get_meta( $form_id, '_walkthecounty_from_name', true );
				WalkTheCounty()->emails->from_address = walkthecounty_get_meta( $form_id, '_walkthecounty_from_email', true );
			}

			return $send
				? WalkTheCounty()->emails->send( $this->get_preview_email_recipient( $form_id ), $subject, $message, $attachments )
				: false;
		}
		 ...
		 *
		 * Note: To render email tags in all context certain parameters are necessary for core (includes/emails/class-walkthecounty-emails):
		 *    1. payment_id
		 *    2. user_id
		 *    3. form_id
		 *    4. donor_id
		...
		public function send_email_notification( $email_tag_args = array() ) {
			 ...
			 * @since 2.2.3
			 */
			if ( apply_filters( 'walkthecounty_is_stop_email_notification', false, $this ) ) {
				return false;
			}
			 ...
			 * @since 2.0
			 */
			$email_tag_args = apply_filters( "walkthecounty_{$this->config['id']}_email_tag_args", $email_tag_args, $this );

			// Get form id.
			$form_id = ! empty( $email_tag_args['form_id'] )
				? absint( $email_tag_args['form_id'] )
				: ( ! empty( $email_tag_args['payment_id'] ) ? walkthecounty_get_payment_form_id( $email_tag_args['payment_id'] ) : null );

			// Do not send email if notification is disabled.
			if ( ! WalkTheCounty_Email_Notification_Util::is_email_notification_active( $this, $form_id ) ) {
				return false;
			}
			 ...
			 * @since 2.0
			 */
			do_action( "walkthecounty_{$this->config['id']}_email_send_before", $this, $form_id );

			$attachments  = $this->get_email_attachments();
			$message      = walkthecounty_do_email_tags( $this->get_email_message( $form_id ), $email_tag_args );
			$subject      = walkthecounty_do_email_tags( $this->get_email_subject( $form_id ), $email_tag_args );
			$content_type = $this->get_email_content_type( $form_id );
			...
			// Set email header.
			WalkTheCounty()->emails->__set( 'heading', walkthecounty_do_email_tags( $this->get_email_header( $form_id ), $email_tag_args ) );
			...
			// Send email.
			$email_status = WalkTheCounty()->emails->send( $this->get_recipient( $form_id ), $subject, $message, $attachments );
			 ...
			 * @since 2.0
			 */
			do_action( "walkthecounty_{$this->config['id']}_email_send_after", $email_status, $this, $form_id );
		...
		public function preview_email_template_tags( $message ) {
			...
			// Set Payment.
			$payment_id = walkthecounty_check_variable( $get_data, 'isset_empty', 0, 'preview_id' );
			$payment    = $payment_id ? new WalkTheCounty_Payment( $payment_id ) : new stdClass();

			// Set donor.
			$user_id = $payment_id
				? $payment->user_id
				: walkthecounty_check_variable( $get_data, 'isset_empty', 0, 'user_id' );
			$user_id = $user_id ? $user_id : wp_get_current_user()->ID;

			$receipt_link_url = walkthecounty_get_receipt_url( $payment_id );
			$receipt_link     = walkthecounty_get_receipt_link( $payment_id );

			// Set default values for tags.
			$this->config['preview_email_tags_values'] = wp_parse_args(
				$this->config['preview_email_tags_values'],
				array(
					'name'                    => walkthecounty_email_tag_first_name(
						array(
							'payment_id' => $payment_id,
							'user_id'    => $user_id,
						)
					),
					'fullname'                => walkthecounty_email_tag_fullname(
						array(
							'payment_id' => $payment_id,
							'user_id'    => $user_id,
						)
					),
					'username'                => walkthecounty_email_tag_username(
						array(
							'payment_id' => $payment_id,
							'user_id'    => $user_id,
						)
					),
					'user_email'              => walkthecounty_email_tag_user_email(
						array(
							'payment_id' => $payment_id,
							'user_id'    => $user_id,
						)
					),
					'payment_total'           => $payment_id ? walkthecounty_email_tag_payment_total( array( 'payment_id' => $payment_id ) ) : walkthecounty_currency_filter( '10.50' ),
					'amount'                  => $payment_id ? walkthecounty_email_tag_amount( array( 'payment_id' => $payment_id ) ) : walkthecounty_currency_filter( '10.50' ),
					'price'                   => $payment_id ? walkthecounty_email_tag_price( array( 'payment_id' => $payment_id ) ) : walkthecounty_currency_filter( '10.50' ),
					'payment_method'          => $payment_id ? walkthecounty_email_tag_payment_method( array( 'payment_id' => $payment_id ) ) : __( 'PayPal', 'walkthecounty' ),
					'payment_id'              => $payment_id ? $payment_id : rand( 2000, 2050 ),
					'receipt_link_url'        => $receipt_link_url,
					'receipt_link'            => $receipt_link,
					'date'                    => $payment_id ? date( walkthecounty_date_format(), strtotime( $payment->date ) ) : date( walkthecounty_date_format(), current_time( 'timestamp' ) ),
					'donation'                => $payment_id ? walkthecounty_email_tag_donation( array( 'payment_id' => $payment_id ) ) : esc_html__( 'Sample Donation Form Title', 'walkthecounty' ),
					'form_title'              => $payment_id ? walkthecounty_email_tag_form_title( array( 'payment_id' => $payment_id ) ) : esc_html__( 'Sample Donation Form Title - Sample Donation Level', 'walkthecounty' ),
					'sitename'                => $payment_id ? walkthecounty_email_tag_sitename( array( 'payment_id' => $payment_id ) ) : get_bloginfo( 'name' ),
					'billing_address'         => $payment_id ? walkthecounty_email_tag_billing_address( array( 'payment_id' => $payment_id ) ) : '',
					'email_access_link'       => sprintf(
						'<a href="%1$s">%2$s</a>',
						add_query_arg(
							array(
								'walkthecounty_nl' => uniqid(),
							),
					...
					'reset_password_link'     => $user_id ? walkthecounty_email_tag_reset_password_link( array( 'user_id' => $user_id ), $payment_id ) : '',
					'site_url'                => sprintf(
						'<a href="%1$s">%2$s</a>',
						get_bloginfo( 'url' ),
						get_bloginfo( 'url' )
					),
					'admin_email'             => walkthecounty_email_admin_email(),
					'offline_mailing_address' => walkthecounty_email_offline_mailing_address(),
					'donor_comment'           => $payment_id ? walkthecounty_email_donor_comment( array( 'payment_id' => $payment_id ) ) : esc_html__( 'Sample Donor Comment', 'walkthecounty' ),
				)
			);
			...
			return apply_filters( 'walkthecounty_email_preview_template_tags', $message );
		}
