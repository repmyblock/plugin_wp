...
/**
 * WalkTheCounty_Tools_Delete_Donors Class
 *
 * @since 1.8.12
 */
class WalkTheCounty_Tools_Delete_Donors extends WalkTheCounty_Batch_Export {
	...
	public function process_step() {
		...
		if ( ! $this->can_export() ) {
			wp_die(
				esc_html__( 'You do not have permission to delete test transactions.', 'walkthecounty' ),
		...
		if ( $had_data ) {
			...
			$this->done    = true;
			$this->message = __( 'Test donor and transactions successfully deleted.', 'walkthecounty' );
	...
	public function get_data() {
		...
		if ( 2 === $step ) {
			...
			do_action( 'walkthecounty_delete_log_cache' );
		}
		...
		if ( 3 === $step ) {
			...
			$args = apply_filters( 'walkthecounty_tools_reset_stats_total_args', array(
				...
				'post_status'    => 'any',
				'posts_per_page' => 1,
				'meta_key'       => '_walkthecounty_payment_mode',
				'meta_value'     => 'live',
