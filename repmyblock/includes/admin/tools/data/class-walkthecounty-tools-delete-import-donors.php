 ...
 *
 * @subpackage  Admin/Tools/WalkTheCounty_Tools_Delete_Donors
...
/**
 * WalkTheCounty_Tools_Import_Donors Class
 *
 * @since 1.8.13
 */
class WalkTheCounty_Tools_Import_Donors extends WalkTheCounty_Batch_Export {
	 ...
	 * @since 1.8.13
	 */
	var $form_key = 'walkthecounty_temp_delete_form_ids';
	 ...
	 * @since 1.8.12
	 */
	var $donation_key = 'walkthecounty_temp_delete_donation_ids';
	 ...
	 * @since 1.8.12
	 */
	var $step_key = 'walkthecounty_temp_delete_step';
	 ...
	 * @since 1.8.12
	 */
	var $donor_key = 'walkthecounty_temp_delete_donor_ids';
	 ...
	 * @since 1.8.12
	 */
	var $step_on_key = 'walkthecounty_temp_delete_step_on';
	...
	/**
	 * WalkTheCounty_Tools_Import_Donors constructor.
	 *
	...
	private function count( $step, $donation_ids = array(), $donor_ids = array() ) {
		...
		$args = apply_filters( 'walkthecounty_tools_reset_stats_total_args', array(
			...
			'paged'          => $paged,
			'meta_key'       => '_walkthecounty_payment_import',
			'meta_value_num' => 1,
			'meta_compare'   => '=',
		) );
		...
		if ( $donation_posts->have_posts() ) {
			while ( $donation_posts->have_posts() ) {
				...
				$donor_id = (int) walkthecounty_get_meta( $post->ID, '_walkthecounty_payment_customer_id', true );
				if ( ! empty( $donor_id ) ) {
					...
					if ( ! empty( $donor->id ) ) {
						if ( empty( $donor->user_id ) && ! empty( $donor->payment_ids ) ) {
							...
							if ( 1 === $count ) {
								walkthecounty_delete_donor_and_related_donation( $donor );
							} else {
	...
	public function process_step() {
		...
		if ( ! $this->can_export() ) {
			wp_die(
				esc_html__( 'You do not have permission to delete Import transactions.', 'walkthecounty' ),
		...
		if ( $had_data ) {
			...
			$this->done    = true;
			$this->message = __( 'Imported donor and transactions successfully deleted.', 'walkthecounty' );
	...
	public function get_data() {
		...
		if ( 2 === $step ) {
			...
			foreach ( $donation_ids as $item ) {
				$form_ids[] = walkthecounty_get_meta( $item, '_walkthecounty_payment_form_id', true );

				// Delete the main payment.
				walkthecounty_delete_donation( absint( $item ) );
			}
		...
		if ( 3 === $step ) {
			...
			if ( ! empty( $form_ids ) ) {
				...
				foreach ( $form_ids as $form_id ) {
					walkthecounty_recount_form_income_donation( (int) $form_id );
				}
			}
			// update the new form list.
			...
			if ( ! empty( $donor_ids[ $page ] ) ) {
				...
				if ( empty( $payments ) ) {
					WalkTheCounty()->donors->delete_by_user_id( $donor_ids[ $page ] );
					...
					$donor          = new WalkTheCounty_Donor( $donor_ids[ $page ], true );
