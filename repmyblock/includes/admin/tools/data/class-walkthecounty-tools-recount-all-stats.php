 ...
 *
 * @subpackage Admin/Tools/WalkTheCounty_Tools_Recount_All_Stats
...
/**
 * WalkTheCounty_Tools_Recount_All_Stats Class
 *
 * @since 1.5
 */
class WalkTheCounty_Tools_Recount_All_Stats extends WalkTheCounty_Batch_Export {
	...
	public function get_data() {
		...
		$totals             = $this->get_stored_data( 'walkthecounty_temp_recount_all_stats' );
		$payment_items      = $this->get_stored_data( 'walkthecounty_temp_payment_items' );
		$processed_payments = $this->get_stored_data( 'walkthecounty_temp_processed_payments' );
		...
		$all_forms = $this->get_stored_data( 'walkthecounty_temp_form_ids' );

		$payments = $this->get_stored_data( 'walkthecounty_temp_all_payments_data' );

		if ( empty( $payments ) ) {
			$args = apply_filters( 'walkthecounty_recount_form_stats_args', array(
				'walkthecounty_forms' => $all_forms,
		...
		if ( ! empty( $payments ) ) {
			...
			$this->store_data( 'walkthecounty_temp_processed_payments', $processed_payments );
			$this->store_data( 'walkthecounty_temp_recount_all_stats', $totals );
		...
		foreach ( $totals as $key => $stats ) {
			walkthecounty_update_meta( $key, '_walkthecounty_form_sales', $stats['sales'] );
			walkthecounty_update_meta( $key, '_walkthecounty_form_earnings', walkthecounty_sanitize_amount_for_db( $stats['earnings'] ) );
		}
	...
	public function get_percentage_complete() {
		...
		$total = $this->get_stored_data( 'walkthecounty_recount_all_total' );
	...
	public function process_step() {
		...
		if ( $had_data ) {
			...
			$this->delete_data( 'walkthecounty_recount_all_total' );
			$this->delete_data( 'walkthecounty_temp_recount_all_stats' );
			$this->delete_data( 'walkthecounty_temp_payment_items' );
			$this->delete_data( 'walkthecounty_temp_form_ids' );
			$this->delete_data( 'walkthecounty_temp_processed_payments' );
			$this->done    = true;
			$this->message = esc_html__( 'Donation form income amounts and donation counts stats successfully recounted.', 'walkthecounty' );
	...
	public function pre_fetch() {
		...
		if ( 1 == $this->step ) {
			$this->delete_data( 'walkthecounty_temp_recount_all_total' );
			$this->delete_data( 'walkthecounty_temp_recount_all_stats' );
			$this->delete_data( 'walkthecounty_temp_payment_items' );
			$this->delete_data( 'walkthecounty_temp_processed_payments' );
			$this->delete_data( 'walkthecounty_temp_all_payments_data' );
		}

		$accepted_statuses = apply_filters( 'walkthecounty_recount_accepted_statuses', array( 'publish' ) );
		$total             = $this->get_stored_data( 'walkthecounty_temp_recount_all_total' );
		...
		if ( false === $total ) {
			...
			if ( false === $payment_items ) {
				$payment_items = array();
				$this->store_data( 'walkthecounty_temp_payment_items', $payment_items );
			}
			...
			$this->store_data( 'walkthecounty_temp_form_ids', $all_forms );
			...
			$total = wp_count_posts( 'walkthecounty_payment' )->publish;

			$this->store_data( 'walkthecounty_temp_all_payments_data', $payments );
				...
				foreach ( $payments as $payment ) {
					...
					if ( ! array_key_exists( $payment->ID, $payment_items ) ) {
						...
						$payment_items[ $payment->ID ] = array(
							...
							'price'      => (float) walkthecounty_maybe_sanitize_amount( $payment_total ),
						);
					}
				}
			}
			$this->store_data( 'walkthecounty_temp_payment_items', $payment_items );
			$this->store_data( 'walkthecounty_recount_all_total', $total );
		}
