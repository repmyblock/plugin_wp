...
/**
 * WalkTheCounty_API_Request_Log_Table List Table Class
 *
 * Renders the gateway errors list table
 *
 * @since 1.0
 */
class WalkTheCounty_API_Request_Log_Table extends WP_List_Table {
	/**
	...
	public function get_columns() {
		$columns = array(
			...
			'ip'      => __( 'Request IP', 'walkthecounty' ),
			'date'    => __( 'Date', 'walkthecounty' ),
			'details' => __( 'Request Details', 'walkthecounty' ),
		);
	...
	public function column_details( $item ) {
		echo WalkTheCounty()->tooltips->render_link( array(
			'label'       => __( 'View Request', 'walkthecounty' ),
		...
		<div id="log-details-<?php echo $item['ID']; ?>" style="display:none;">
			...
			echo sprintf(
				...
				__( 'API Request:', 'walkthecounty' ),
				WalkTheCounty()->logs->logmeta_db->get_meta( $item['ID'], '_walkthecounty_log_api_query', true )
			);
			...
			echo sprintf(
				'<p><strong>%1$s</strong></p><div>%2$s</div>',
				__( 'API User:', 'walkthecounty' ),
				WalkTheCounty()->logs->logmeta_db->get_meta( $item['ID'], '_walkthecounty_log_user', true )
			);

			// Print the logged key used by API.
			echo sprintf(
				'<p><strong>%1$s</strong></p><div>%2$s</div>',
				__( 'API Key:', 'walkthecounty' ),
				WalkTheCounty()->logs->logmeta_db->get_meta( $item['ID'], '_walkthecounty_log_key', true )
			);

			// Print the API Request Date.
			echo sprintf(
				'<p><strong>%1$s</strong></p><div>%2$s</div>',
				__( 'Request Date:', 'walkthecounty' ),
				$item['log_date']
			);
			?>
		</div>
	...
	function get_meta_query() {
		...
		if ( $search ) {
			if ( filter_var( $search, FILTER_VALIDATE_IP ) ) {
				...
				$key = '_walkthecounty_log_request_ip';
	...
	public function get_logs() {
		...
		if ( $logs ) {
			foreach ( $logs as $log ) {
				...
				$logs_data[] = array(
					'ID'          => $log->ID,
					'ip'          => WalkTheCounty()->logs->logmeta_db->get_meta( $log->ID, '_walkthecounty_log_request_ip', true ),
					'date'        => $log->log_date,
	 ...
	 * @access public
	 * @since  1.0
	 * @uses   WalkTheCounty_API_Request_Log_Table::get_columns()
	 * @uses   WP_List_Table::get_sortable_columns()
	 * @uses   WalkTheCounty_API_Request_Log_Table::get_pagenum()
	 * @uses   WalkTheCounty_API_Request_Log_Table::get_logs()
	 * @uses   WalkTheCounty_API_Request_Log_Table::get_log_count()
	 *
	 * @return void
	 */
	public function prepare_items() {
		...
		$total_items           = WalkTheCounty()->logs->get_log_count( 0, 'api_request', $this->get_meta_query() );
