 ...
 * @subpackage  Classes/WalkTheCounty_Import_Core_Settings
...
if ( ! class_exists( 'WalkTheCounty_Import_Core_Settings' ) ) {

	/**
	 * WalkTheCounty_Import_Core_Settings.
	 *
	 * @since 1.8.17
	 */
	final class WalkTheCounty_Import_Core_Settings {
		...
		private function setup_hooks() {
			...
			// Print the HTML.
			add_action( 'walkthecounty_tools_import_core_settings_form_start', array( $this, 'html' ), 10 );
			...
			add_action( 'walkthecounty_tools_import_core_settings_form_end', array( $this, 'submit' ), 10 );
		}
		...
		public function html() {
			...
			<section>
				<table
					class="widefat export-options-table walkthecounty-table <?php echo "step-{$step}"; ?> <?php echo( 1 === $step && ! empty( $this->is_json_valid ) ? 'walkthecounty-hidden' : '' ); ?> "
		...
		public function import_success() {
			...
			$success = (bool) ( isset( $_GET['success'] ) ? walkthecounty_clean( $_GET['success'] ) : false );
			$undo = (bool) ( isset( $_GET['undo'] ) ? walkthecounty_clean( $_GET['undo'] ) : false );
			...
			$title = __( 'Settings Import Complete!', 'walkthecounty' );
			if ( $success ) {
				$query_arg_success['undo'] = '1';
				$query_arg_success['step'] = '3';
				$query_arg_success['success'] = '1';
				$text = __( 'Undo Importing', 'walkthecounty' );
			} else {
				if ( $undo ) {
					$host_walkthecounty_options = get_option( 'walkthecounty_settings_old', array() );
					update_option( 'walkthecounty_settings', $host_walkthecounty_options, false );
					$title = __( 'Successfully Reverted Settings Import', 'walkthecounty' );
				} else {
					$title = __( 'Failed to Import', 'walkthecounty' );
				}

				$text = __( 'Import Again', 'walkthecounty' );
			}
			?>
			<tr valign="top" class="walkthecounty-import-dropdown">
				<th colspan="2">
					...
					<p>
						...
						<a class="button button-large button-secondary" href="<?php echo add_query_arg( $query_arg_setting, admin_url( 'edit.php' ) ); ?>"><?php echo __( 'View Settings', 'walkthecounty' ); ?></a>
					</p>
				</th>
			</tr>
			<?php
		...
		public function start_import() {
			$type      = ( ! empty( $_GET['type'] ) ? walkthecounty_clean( $_GET['type'] ) : 'replace' );
			$file_name = ( ! empty( $_GET['file_name'] ) ? walkthecounty_clean( $_GET['file_name'] ) : '' );

			?>
			<tr valign="top" class="walkthecounty-import-dropdown">
				<th colspan="2">
					<h2 id="walkthecounty-import-title"><?php esc_html_e( 'Importing', 'walkthecounty' ) ?></h2>
					<p class="walkthecounty-field-description"><?php esc_html_e( 'Your settings are now being imported...', 'walkthecounty' ) ?></p>
				</th>
			</tr>
		...
		public function render_progress() {
			...
			<ol class="walkthecounty-progress-steps">
				<li class="<?php echo( 1 === $step ? 'active' : '' ); ?>">
					<?php esc_html_e( 'Upload JSON file', 'walkthecounty' ); ?>
				</li>
				<li class="<?php echo( 2 === $step ? 'active' : '' ); ?>">
					<?php esc_html_e( 'Import', 'walkthecounty' ); ?>
				</li>
				<li class="<?php echo( 3 === $step ? 'active' : '' ); ?>">
					<?php esc_html_e( 'Done!', 'walkthecounty' ); ?>
				</li>
			</ol>
			<?php
		}
		...
		public function render_page() {
			include_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/admin/tools/views/html-admin-page-import-core-settings.php';
		}
		...
		public function render_upload_html() {
			$json = ( isset( $_POST['json'] ) ? walkthecounty_clean( $_POST['json'] ) : '' );
			$type = ( isset( $_POST['type'] ) ? walkthecounty_clean( $_POST['type'] ) : 'merge' );
			$step = $this->get_step();

			?>
			<tr valign="top">
				<th colspan="2">
					<h2 id="walkthecounty-import-title"><?php esc_html_e( 'Import Core Settings from a JSON file', 'walkthecounty' ) ?></h2>
					<p class="walkthecounty-field-description"><?php esc_html_e( 'This tool allows you to import WalkTheCountyWP settings from another WalkTheCountyWP installation. Settings imported contain data from WalkTheCountyWP core as well as any of our Premium Add-ons.', 'walkthecounty' ) ?></p>
				</th>
			</tr>

			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="json"><?php esc_html_e( 'Choose a JSON file:', 'walkthecounty' ) ?></label>
				</th>
				<td class="walkthecounty-forminp">
					<div class="walkthecounty-field-wrap">
						<label for="json">
							<input type="file" name="json" class="walkthecounty-upload-json-file" value="<?php echo $json; ?>"
							       accept=".json">
							<p class="walkthecounty-field-description"><?php esc_html_e( 'The file type must be JSON.', 'walkthecounty' )?></p>
						</label>
					</div>
				</td>
			</tr>
			...
			$settings = array(
				array(
					'id'          => 'type',
					'name'        => __( 'Merge Type:', 'walkthecounty' ),
					'description' => __( 'Select "Merge" to retain existing settings, or "Replace" to overwrite with the settings from the JSON file', 'walkthecounty' ),
					'default'     => $type,
					'type'        => 'radio_inline',
					'options'     => array(
						'merge'   => __( 'Merge', 'walkthecounty' ),
						'replace' => __( 'Replace', 'walkthecounty' ),
					),
				),
			);

			$settings = apply_filters( 'walkthecounty_import_core_setting_html', $settings );
				...
				<tr valign="top">
					...
					<th>
						<input type="submit"
						       ...
						       value="<?php esc_attr_e( 'Submit', 'walkthecounty' ); ?>"/>
					</th>
				</tr>
				<?php
			} else {
		...
		public function save() {
			...
			if ( 1 === $step ) {
				$type          = ( ! empty( $_REQUEST['type'] ) ? walkthecounty_clean( $_REQUEST['type'] ) : 'replace' );
				$core_settings = self::upload_widget_settings_file();
				if ( ! empty( $core_settings['error'] ) ) {
					WalkTheCounty_Admin_Settings::add_error( 'walkthecounty-import-csv', __( 'Please upload a valid JSON settings file.', 'walkthecounty' ) );
				} else {
					$file_path = explode( '/', $core_settings['file'] );
					$count     = ( count( $file_path ) - 1 );
					$url       = walkthecounty_import_page_url( (array) apply_filters( 'walkthecounty_import_core_settings_importing_url', array(
						'step'          => '2',
		...
		private function is_donations_import_page() {
			return 'import' === walkthecounty_get_current_setting_tab() && isset( $_GET['importer-type'] ) && $this->importer_type === walkthecounty_clean( $_GET['importer-type'] );
		}
		 ...
		 *
		 * https://github.com/impress-org/walkthecounty/issues/3907
		 *
		 * @param $check
		 * @param $file
		 * @param $filename
	...
	WalkTheCounty_Import_Core_Settings::get_instance()->setup();
}
