...
class WalkTheCounty_Updates {
	 ...
	 * @since
	 * @access public
	 * @var WalkTheCounty_Background_Updater
	 */
	...
	public function setup() {
		 ...
		 * Load file
		 */
		require_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/class-walkthecounty-background-updater.php';
		require_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/admin/upgrades/upgrade-functions.php';

		self::$background_updater = new WalkTheCounty_Background_Updater();

		/**
		 * Setup hooks.
		 */
		add_action( 'init', array( $this, '__register_upgrade' ), 9999 );
		add_action( 'walkthecounty_set_upgrade_completed', array( $this, '__flush_resume_updates' ), 9999 );
		add_action( 'wp_ajax_walkthecounty_db_updates_info', array( $this, '__walkthecounty_db_updates_info' ) );
		add_action( 'wp_ajax_walkthecounty_run_db_updates', array( $this, '__walkthecounty_start_updating' ) );
		...
		add_action( 'walkthecounty_restart_db_upgrade', array( $this, '__health_background_update' ) );
	...
	public function __register_plugin_addon_updates() {
		$addons         = walkthecounty_get_plugins( array( 'only_premium_add_ons' => true ) );
	...
	public function __register_upgrade() {
		 ...
		 * @since 1.8.12
		 */
		do_action( 'walkthecounty_register_updates', $this );
	}
	...
	function __change_donations_label() {
		...
		foreach ( $menu as $index => $menu_item ) {
			if ( 'edit.php?post_type=walkthecounty_forms' !== $menu_item[2] ) {
				continue;
			}

			$menu[ $index ][0] = sprintf(
				'%1$s <span class="update-plugins"><span class="plugin-count walkthecounty-update-progress-count">%2$s%3$s</span></span>',
	...
	public function __register_menu() {
		...
		if ( ! $this->get_total_update_count() ) {
			...
			if ( isset( $_GET['page'] ) && 'walkthecounty-updates' === $_GET['page'] ) {
				// Upgrades
				add_submenu_page(
					'edit.php?post_type=walkthecounty_forms',
					esc_html__( 'WalkTheCountyWP Updates Complete', 'walkthecounty' ),
					__( 'Updates', 'walkthecounty' ),
					'manage_walkthecounty_settings',
					'walkthecounty-updates',
		...
		add_submenu_page(
			'edit.php?post_type=walkthecounty_forms',
			esc_html__( 'WalkTheCountyWP Updates', 'walkthecounty' ),
			sprintf(
				'%1$s <span class="update-plugins"%2$s><span class="plugin-count walkthecounty-update-progress-count">%3$s%4$s</span></span>',
				__( 'Updates', 'walkthecounty' ),
				isset( $_GET['walkthecounty-pause-db-upgrades'] ) ? ' style="display:none;"' : '',
				$is_update ?
					$this->get_db_update_processing_percentage() :
					$this->get_total_update_count(),
				$is_update ? '%' : ''
	...
	public function __redirect_admin() {
		...
		if (
			...
			WalkTheCounty_Cache_Setting::get_option( 'walkthecounty_show_db_upgrade_complete_notice' ) &&
			! isset( $_GET['walkthecounty-db-update-completed'] )
		) {
			delete_option( 'walkthecounty_show_db_upgrade_complete_notice' );

			wp_redirect( admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-updates&walkthecounty-db-update-completed=walkthecounty_db_upgrade_completed' ) );
			exit();
		}
	}
	...
	public function __pause_db_update( $force = false ) {
		...
		if (
			...
			(
				wp_doing_ajax() ||
				! isset( $_GET['page'] ) ||
				'walkthecounty-updates' !== $_GET['page'] ||
				! isset( $_GET['walkthecounty-pause-db-upgrades'] ) ||
		...
		delete_option( 'walkthecounty_upgrade_error' );
		...
		// Remove cache.
		WalkTheCounty_Background_Updater::flush_cache();

		// Do not stop background process immediately if task running.
		// @see WalkTheCounty_Background_Updater::lock_process
		if ( ! $force && self::$background_updater->is_process_running() ) {
			update_option( 'walkthecounty_pause_upgrade', 1, false );

			return true;
		}

		update_option( 'walkthecounty_paused_batches', $batch, false );
		...
		WalkTheCounty()->logs->add( 'Update Pause', print_r( $batch, true ), 0, 'update' );
		 ...
		 * @since 2.0.1
		 */
		do_action( 'walkthecounty_pause_db_upgrade', $this );
	...
	public function __restart_db_update() {
		...
		if (
			...
			! isset( $_GET['page'] ) ||
			'walkthecounty-updates' !== $_GET['page'] ||
			! isset( $_GET['walkthecounty-restart-db-upgrades'] ) ||
			! self::$background_updater->is_paused_process()
		) {
			return false;
		}
		...
		WalkTheCounty_Background_Updater::flush_cache();
		$batch = get_option( 'walkthecounty_paused_batches' );

		if ( ! empty( $batch ) ) {
			wp_cache_delete( $batch->key, 'options' );
			update_option( $batch->key, $batch->data, false );

			delete_option( 'walkthecounty_paused_batches' );

			WalkTheCounty()->logs->add( 'Update Restart', print_r( $batch, true ), 0, 'update' );
			 ...
			 * @since 2.0.1
			 */
			do_action( 'walkthecounty_restart_db_upgrade', $this );
	 ...
	 */
	public function __health_background_update( $walkthecounty_updates ) {
		...
		if ( ! empty( $batch->data ) ) {
			...
			foreach ( $batch->data as $index => $update ) {
				...
				} elseif ( walkthecounty_has_upgrade_completed( $update['id'] ) ) {
				...
				if ( ! empty( $update['depend'] ) ) {
					...
					foreach ( $update['depend'] as $depend ) {
						if ( walkthecounty_has_upgrade_completed( $depend ) ) {
		...
		if ( $new_updates = $this->get_updates( 'database', 'new' ) ) {
			...
			foreach ( $new_updates as $index => $new_update ) {
				if ( walkthecounty_has_upgrade_completed( $new_update['id'] ) || in_array( $new_update['id'], $all_batch_update_ids ) ) {
			...
			if ( ! empty( $new_updates ) ) {
				...
				update_option( 'walkthecounty_db_update_count', ( absint( get_option( 'walkthecounty_db_update_count' ) ) + count( $new_updates ) ), false );
			}
		}
		...
		if ( empty( $batch->data ) ) {
			...
			if ( ! empty( $batch->key ) ) {
				...
				foreach ( $batch->data as $data ) {
					WalkTheCounty_Updates::$background_updater->push_to_queue( $data );
				}

				WalkTheCounty_Updates::$background_updater->save();
			}
		}


		/**
		 * Fix walkthecounty_doing_upgrade option
		 */
		if( $fresh_new_db_count = $this->get_total_new_db_update_count( true ) ) {
			update_option( 'walkthecounty_db_update_count', $fresh_new_db_count, false );
		}
		...
		if (
			...
			|| walkthecounty_has_upgrade_completed( $doing_upgrade_args['update_info']['id'] )
		) {
		...
		if( ! empty( $doing_upgrade_args['update_info'] ) ) {
			update_option( 'walkthecounty_doing_upgrade', $doing_upgrade_args, false );

			$log_data .= 'Updated doing update:' . "\n";
			$log_data .= print_r( $doing_upgrade_args, true ) . "\n";
		}

		WalkTheCounty()->logs->add( 'Update Health Check', $log_data, 0, 'update' );
	}
	...
	public function __show_notice() {
		...
		$hide_on_pages = array(
			'walkthecounty_forms_page_walkthecounty-updates',
			'update-core',
			'walkthecounty_forms_page_walkthecounty-addons'
		);
		...
		// Run DB updates.
		if ( ! empty( $_GET['walkthecounty-run-db-update'] ) ) {
			$this->run_db_update();
		}
		...
		if ( self::$background_updater->is_paused_process() ) {
			...
			$upgrade_error = get_option( 'walkthecounty_upgrade_error' );
			if ( ! $upgrade_error ) : ?>
				<strong><?php _e( 'Database Update', 'walkthecounty' ); ?></strong>
				&nbsp;&#8211;&nbsp;<?php _e( 'WalkTheCountyWP needs to update your database to the latest version. The following process will make updates to your site\'s database. Please create a backup before proceeding.', 'walkthecounty' ); ?>
				<br>
				<br>
				<a href="<?php echo esc_url( add_query_arg( array( 'walkthecounty-restart-db-upgrades' => 1 ), admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-updates' ) ) ); ?>" class="button button-primary walkthecounty-restart-updater-btn">
					<?php _e( 'Restart the updater', 'walkthecounty' ); ?>
				</a>
			<?php else: ?>
				<strong><?php _e( 'Database Update', 'walkthecounty' ); ?></strong>
				&nbsp;&#8211;&nbsp;<?php _e( 'An unexpected issue occurred during the database update which caused it to stop automatically. Please contact support for assistance.', 'walkthecounty' ); ?>
				<a href="<?php echo esc_url('http://docs.walkthecountywp.com/troubleshooting-db-updates')?>" target="_blank"><?php _e( 'Read More', 'walkthecounty' ); ?> &raquo;</a>
			<?php
			endif;
			$desc_html = ob_get_clean();

			WalkTheCounty()->notices->register_notice( array(
				'id'          => 'walkthecounty_upgrade_db',
				'type'        => 'error',
				'dismissible' => false,
		...
		// Show db upgrade completed notice.
		if ( ! empty( $_GET['walkthecounty-db-update-completed'] ) ) {
			WalkTheCounty()->notices->register_notice( array(
				'id'          => 'walkthecounty_db_upgrade_completed',
				'type'        => 'updated',
				'description' => __( 'WalkTheCountyWP database updates completed successfully. Thank you for updating to the latest version!', 'walkthecounty' ),
				'show'        => true,
			) );

			// Start update.
		} elseif ( ! empty( $_GET['walkthecounty-run-db-update'] ) ) {
			$this->run_db_update();
			...
			<p>
				<strong><?php _e( 'Database Update', 'walkthecounty' ); ?></strong>
				&nbsp;&#8211;&nbsp;<?php _e( 'WalkTheCountyWP needs to update your database to the latest version. The following process will make updates to your site\'s database. Please create a complete backup before proceeding.', 'walkthecounty' ); ?>
			</p>
			<p class="submit">
				<a href="<?php echo esc_url( add_query_arg( array( 'walkthecounty-run-db-update' => 1 ), admin_url( 'edit.php?post_type=walkthecounty_forms&page=walkthecounty-updates' ) ) ); ?>" class="button button-primary walkthecounty-run-update-now">
					<?php _e( 'Run the updater', 'walkthecounty' ); ?>
				</a>
			</p>
			<?php
			$desc_html = ob_get_clean();
	...
	/**
	 * Render WalkTheCounty Updates Completed page
	 *
	 * @since  1.8.12
	 * @access public
	 */
	public function render_complete_page() {
		include_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/admin/upgrades/views/upgrades-complete.php';
	}

	/**
	 * Render WalkTheCounty Updates page
	 *
	 * @since  1.8.12
	 * @access public
	 */
	public function render_page() {
		include_once WALKTHECOUNTY_PLUGIN_DIR . 'includes/admin/upgrades/views/upgrades.php';
	}
	...
	private function run_db_update() {
		...
		add_option( 'walkthecounty_db_update_count', count( $updates ), '', false );

		add_option( 'walkthecounty_doing_upgrade', array(
	 ...
	 * @return void
	 */
	public function __walkthecounty_start_updating() {
		// Check permission.
		if (
			! current_user_can( 'manage_walkthecounty_settings' ) ||
	 ...
	 * @return string
	 */
	public function __walkthecounty_db_updates_info() {
		// Check permission.
		if ( ! current_user_can( 'manage_walkthecounty_settings' ) ) {
			walkthecounty_die();
		}

		$update_info   = get_option( 'walkthecounty_doing_upgrade' );
		$response_type = '';

		if ( self::$background_updater->is_paused_process() ) {
			$update_info = array(
				'message'    => __( 'The updates have been paused.', 'walkthecounty' ),
				'heading'    => '',
				'percentage' => 0,
			);

			if ( get_option( 'walkthecounty_upgrade_error' ) ) {
				$update_info['message'] = __( 'An unexpected issue occurred during the database update which caused it to stop automatically. Please contact support for assistance.', 'walkthecounty' );
			}
			...
			$update_info   = array(
				'message'    => __( 'WalkTheCountyWP database updates completed successfully. Thank you for updating to the latest version!', 'walkthecounty' ),
				'heading'    => __( 'Updates Completed.', 'walkthecounty' ),
				'percentage' => 0,
			);
	...
	public function is_doing_updates() {
		return (bool) WalkTheCounty_Cache_Setting::get_option( 'walkthecounty_doing_upgrade' );
	}
	...
	public function get_updates( $update_type = '', $status = 'all' ) {
		...
		switch ( $status ) {
			case 'new':
				...
				wp_cache_delete( 'walkthecounty_completed_upgrades', 'options' );
				$completed_updates = walkthecounty_get_completed_upgrades();
	...
	public function get_total_new_db_update_count( $refresh = false ) {
		$update_count = $this->is_doing_updates() && ! $refresh ?
			get_option( 'walkthecounty_db_update_count' ) :
	...
	public function get_running_db_update( $refresh = false ) {
		...
		if ( $this->is_doing_updates() && ! $refresh ) {
			$current_update = get_option( 'walkthecounty_doing_upgrade' );
	...
	public function get_db_update_processing_percentage( $refresh = false ) {
		...
		$resume_update            = get_option( 'walkthecounty_doing_upgrade' );
...
WalkTheCounty_Updates::get_instance()->setup();
