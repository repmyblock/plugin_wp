...
/**
 * Class WalkTheCounty_Shortcode_Generator
 */
abstract class WalkTheCounty_Shortcode_Generator {
	...
	public function init() {
		...
		if ( $this->shortcode_tag ) {
			...
			$defaults = array(
				'btn_close' => esc_html__( 'Close', 'walkthecounty' ),
				'btn_okay'  => esc_html__( 'Insert Shortcode', 'walkthecounty' ),
				'errors'    => $this->errors,
				'fields'    => $fields,
				'label'     => '[' . $this->shortcode_tag . ']',
				'required'  => $this->required,
				'title'     => esc_html__( 'Insert Shortcode', 'walkthecounty' ),
			);

			if ( user_can_richedit() ) {

				WalkTheCounty_Shortcode_Button::$shortcodes[ $this->shortcode_tag ] = wp_parse_args( $this->shortcode, $defaults );
	...
	protected function get_fields() {
		...
		if ( ! empty( $this->errors ) ) {
			foreach ( $this->required as $name => $alert ) {
				...
				if ( false === array_search( $name, walkthecounty_list_pluck( $generated_fields, 'name' ) ) ) {
	...
	protected function generate_docs_link( $field ){
		...
		return $this->generate_container(
			array(
				'type' => 'container',
				'html' => sprintf(
					'<p class="walkthecounty-docs-link" style="%5$s"><a href="%4$s" style="%3$s" target="_blank">%1$s<span class="dashicons dashicons-editor-help" style="%2$s"></a></span></p>',
					$field['text'],
					$dashicon_style,
					$a_style,
					esc_url( $field['link'] ),
	...
	protected function generate_listbox( $field ) {
		...
		if ( $this->validate( $field ) ) {
			...
			$field['options'] = array(
				                    '' => ( $field['placeholder'] ? $field['placeholder'] : esc_attr__( '- Select -', 'walkthecounty' ) ),
			                    ) + $field['options'];
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$options[ absint( $post->ID ) ] = empty( $post->post_title )
					? sprintf( __( 'Untitled (#%s)', 'walkthecounty' ), $post->ID )
	...
	protected function validate( $field ) {
		...
		if ( $name ) {
			...
			if ( ! ! $required || is_array( $required ) ) {
				...
				$alert = esc_html__( 'Some of the shortcode options are required.', 'walkthecounty' );
				...
				if ( isset( $required['alert'] ) ) {
					...
					/* translators: %s: option label */
						esc_html__( 'The "%s" option is required.', 'walkthecounty' ),
						str_replace( ':', '', $label )
					);
				}
