...
/**
 * Class WalkTheCounty_Shortcode_Button
 */
final class WalkTheCounty_Shortcode_Button {
	...
	public function ajax_handler(){
		add_action( 'wp_ajax_walkthecounty_shortcode', array( $this, 'shortcode_ajax' ) );
	}
	...
	public function mce_external_plugins( $plugin_array ) {
		...
		$plugin_array['walkthecounty_shortcode'] = WALKTHECOUNTY_PLUGIN_URL . 'includes/admin/shortcodes/mce-plugin.js';
	...
	public function admin_enqueue_assets() {
		...
		wp_enqueue_script(
			'walkthecounty_shortcode',
			WALKTHECOUNTY_PLUGIN_URL . 'assets/dist/js/admin-shortcodes.js',
			array( 'jquery' ),
		...
		wp_enqueue_style(
			'walkthecounty-admin-shortcode-button-style',
			WALKTHECOUNTY_PLUGIN_URL . 'assets/dist/css/admin-shortcode-button' . $direction . '.css',
			array(),
			WALKTHECOUNTY_VERSION
		);
	}
	...
	public function admin_localize_scripts() {
		...
		if ( ! empty( self::$shortcodes ) ) {
			...
			wp_localize_script( 'walkthecounty_shortcode', 'scShortcodes', $variables );
		}
	}
	...
	public function shortcode_button() {
		...
		foreach ( self::$shortcodes as $shortcode => $values ) {
			...
			if ( apply_filters( sanitize_title( $shortcode ) . '_condition', true ) ) {
				...
				$shortcodes[ $shortcode ] = sprintf(
					'<div class="sc-shortcode mce-menu-item walkthecounty-shortcode-item-%1$s" data-shortcode="%2$s">%3$s</div>',
					$shortcode,
					$shortcode,
					$values['label']
				);
		...
		if ( ! empty( $shortcodes ) ) {
			...
			$img = ( version_compare( get_bloginfo( 'version' ), '3.5', '<' ) )
				? '<img src="' . WALKTHECOUNTY_PLUGIN_URL . 'assets/dist/images/walkthecounty-media.png" />'
				: '<span class="wp-media-buttons-icon" id="walkthecounty-media-button" style="background-image: url(' . walkthecounty_svg_icons( 'walkthecounty_grey' ) . ');"></span>';
			...
			if ( 1 === count( $shortcodes ) ) {
				...
				printf(
					...
					'</div>',
					$img,
					__( 'WalkTheCountyWP Shortcodes', 'walkthecounty' ),
	...
	public function shortcode_ajax() {
		if( ! current_user_can( 'edit_walkthecounty_forms' ) ) {
			wp_die();
		}
		...
		if ( $shortcode && array_key_exists( $shortcode, self::$shortcodes ) ) {
			...
			if ( ! empty( $data['errors'] ) ) {
				$data['btn_okay'] = array( esc_html__( 'Okay', 'walkthecounty' ) );
			}
	...
	private function is_add_button() {
		...
		$shortcode_button_pages = apply_filters( 'walkthecounty_shortcode_button_pages', array(
			'post.php',
			'page.php',
			'post-new.php',
			'post-edit.php',
		...
		$exclude_post_types = array( 'walkthecounty_forms' );
		...
		if (
			...
			/**
			 * Fire the filter
			 * Use this filter to show WalkTheCounty Shortcode button on custom pages
			 *
			 * @since 1.0
			 *
			 */
			|| ! apply_filters( 'walkthecounty_shortcode_button_condition', true )
...
new WalkTheCounty_Shortcode_Button;
