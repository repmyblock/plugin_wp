/*!
 * WalkTheCounty Admin Shortcodes JS
 *
 * @description: The WalkTheCounty Admin Shortcode scripts. Only enqueued on the admin widgets screen; used to show shortcode dialogs, show/hide, and other functions
...
/* global WalkTheCounty, ajaxurl, jQuery, scShortcodes, tinymce */
...
jQuery( function( $ ) {
	...
	window.render_continue_button_title_field = function() {
		var selected_display_style = $('.mce-txt', '.mce-walkthecounty-display-style').text(),
			expected_display_styles = [ '- Select -', 'All Fields' ];

		if( -1 !== $.inArray( selected_display_style, expected_display_styles ) ) {
			$('.mce-walkthecounty-continue-button-title').closest('.mce-container' ).hide()
		} else {
			$('.mce-walkthecounty-continue-button-title').closest('.mce-container' ).show()
		}
	};
	...
	window.scForm = {
		...
		open: function( editor_id ) {
			...
			data = {
				action    : 'walkthecounty_shortcode',
							...
							for ( var id in required ) {
								if ( required.hasOwnProperty( id ) ) {
									...
									if ( typeof field !== 'undefined' && field.state.data.value === '' ) {
										...
										new WalkTheCounty.modal.WalkTheCountyErrorAlert({
											modalContent:{
												desc: required[ id ],
												cancelBtnTitle: WalkTheCounty.fn.getGlobalVar('ok'),
											}
										}).render();
			...
			if ( ! tinymce.get( window.wpActiveEditor ) ) {
				...
				if ( ! $( '#scTemp' ).length ) {
					...
					tinymce.init({
						mode     : "exact",
						elements : "scTemp",
						plugins  : ['walkthecounty_shortcode', 'wplink']
					});
				}

				setTimeout( function() { tinymce.execCommand( 'WalkTheCounty_Shortcode' ); }, 200 );
			} else {
				tinymce.execCommand( 'WalkTheCounty_Shortcode' );
			}
