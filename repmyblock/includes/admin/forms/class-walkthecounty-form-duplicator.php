...
if ( ! class_exists( 'WalkTheCounty_Form_Duplicator' ) ) {

	/**
	 * WalkTheCounty_Form_Duplicator class
	 */
	class WalkTheCounty_Form_Duplicator {
		...
		public function __construct() {
			...
			add_action( 'admin_action_walkthecounty_duplicate_form', array( $this, 'handler' ) );
		}
		...
		public function row_action( $actions, $post ) {
			...
			if ( isset( $_GET['post_type'] ) && 'walkthecounty_forms' === walkthecounty_clean( $_GET['post_type'] ) ) { // WPCS: input var ok.
				if ( current_user_can( 'edit_posts' ) ) {
					$actions['duplicate_form'] = sprintf(
						'<a href="%1$s">%2$s</a>',
						wp_nonce_url( add_query_arg(
							array(
								'action'  => 'walkthecounty_duplicate_form',
								'form_id' => $post->ID,
							),
							admin_url( 'admin.php' )
						), 'walkthecounty-duplicate-form' ),
						__( 'Duplicate', 'walkthecounty' )
					);
				}
			}
		...
		public function handler() {
			...
			if (
				...
				|| ( 'walkthecounty_duplicate_form' !== $_REQUEST['action'] )
			) {
				wp_die( esc_html__( 'Form ID not found in the query string', 'walkthecounty' ) );

			} elseif ( ! wp_verify_nonce( walkthecounty_clean( $_REQUEST['_wpnonce'] ), 'walkthecounty-duplicate-form' ) ) {

				wp_die( esc_html__( 'We\'re unable to recognize your session. Please refresh the screen to try again; otherwise contact your website administrator for assistance.', 'walkthecounty' ) );
			}
			// @codingStandardsIgnoreEnd

			$form_id      = walkthecounty_clean( $_REQUEST['form_id'] ); // @codingStandardsIgnoreLine
			$post_data    = get_post( $form_id );
			$current_user = wp_get_current_user();
			$error_notice = sprintf(
				/* translators: %s: Form ID */
				esc_html__( 'Cloning failed. Form with ID %s does not exist.', 'walkthecounty' ),
				absint( $form_id )
			);
			...
			if ( isset( $post_data ) && null !== $post_data ) {
				 ...
				 */
				do_action( 'walkthecounty_form_duplicated', $duplicate_form_id, $form_id );
		...
		private function duplicate_taxonomies( $new_form_id, $old_form ) {
			// Get the taxonomies of the post type `walkthecounty_forms`.
		...
		private function reset_stats( $new_form_id ) {
			...
			$meta_keys = array( '_walkthecounty_form_sales', '_walkthecounty_form_earnings' );
			 ...
			 * @since  2.2.0
			 */
			$meta_keys = apply_filters( 'walkthecounty_duplicate_form_reset_stat_meta_keys', $meta_keys );
	...
	new WalkTheCounty_Form_Duplicator();
}
