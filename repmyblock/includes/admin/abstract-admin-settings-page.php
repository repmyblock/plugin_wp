 ...
 * @subpackage  Classes/WalkTheCounty_Settings_Page
...
if ( ! class_exists( 'WalkTheCounty_Settings_Page' ) ) :
	 ...
	 * @sine 1.8
	 */
	class WalkTheCounty_Settings_Page {
		...
		public function __construct() {
			...
			$this->current_setting_page = walkthecounty_get_current_setting_page();

			// Get current section.
			$this->current_section = walkthecounty_get_current_setting_section();

			add_filter( "walkthecounty_default_setting_tab_section_{$this->id}", array( $this, 'set_default_setting_tab' ), 10 );
		...
		public function get_settings() {
			 ...
			 */
			$settings = apply_filters( 'walkthecounty_get_settings_' . $this->id, array() );
		...
		public function get_sections() {
			return apply_filters( 'walkthecounty_get_sections_' . $this->id, array() );
		}
		...
		public function output_sections() {
			...
			if ( $current_section && ! in_array( $current_section, array_keys( $sections ), true ) ) {
				echo wp_kses_post( '<div class="error"><p>' . __( 'Oops, this settings page does not exist.', 'walkthecounty' ) . '</p></div>' );
				$GLOBALS['walkthecounty_hide_save_button'] = true;
			...
			foreach ( $sections as $id => $label ) {
				 ...
				 * @since 2.0
				 */
				if ( apply_filters( "walkthecounty_hide_section_{$id}_on_{$this->id}_page", false, $sections, $this->id ) ) {
					continue;
				}

				$section_list[] = '<li><a href="' . admin_url( 'edit.php?post_type=walkthecounty_forms&page=' . $this->current_setting_page . '&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section === $id ? 'current' : '' ) . '">' . $label . '</a>';
			}

			echo wp_kses_post( sprintf(
				'<ul class="walkthecounty-subsubsub">%s</ul><br class="clear" /><hr>',
		...
		public function output() {
			if ( $this->enable_save ) {
				$GLOBALS['walkthecounty_hide_save_button'] = false;
			}
		...
		public function save() {
			 ...
			 * @since 2.1
			 */
			if ( apply_filters( "walkthecounty_save_options_{$this->id}_{$current_section}", true ) ) {
				WalkTheCounty_Admin_Settings::save_fields( $settings, 'walkthecounty_settings' );
			}
			 ...
			 * @since 1.8
			 */
			do_action( 'walkthecounty_update_options_' . $this->id . '_' . $current_section );
		}
		...
		private function get_heading() {
			$heading[]       = walkthecounty_get_admin_page_menu_title();
			$heading[]       = $this->label;
		...
		public function get_heading_html() {
			return sprintf(
				...
				implode( ' <span class="walkthecounty-settings-heading-sep dashicons dashicons-arrow-right-alt2"></span> ', $this->get_heading() )
			);
		}
	}
